"""
AIOMarine is Blazing fast async ORM/ODM in Python
...
def check_for_update():
    ...
    """
    try:
        response = requests.get("https://pypi.org/pypi/aiomarine/json").json()
        ...
        if sum(latest_digits) > sum(current_digits):
            message = f"New version of library aiomarine available: {latest_version}"
            ...
            print(
                f"[yellow]You use [bold]UNSTABLE[/bold] branch of aiomarine. Stable version: {latest_version}, your version: {__version__}[/yellow]\n"
            )
