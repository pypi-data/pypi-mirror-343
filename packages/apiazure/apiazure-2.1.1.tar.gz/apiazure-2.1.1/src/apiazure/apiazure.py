...
def get_branches_from_repo(API_URL: str, PROJECT: str, TOKEN: str, repo_name: str, headers: object = None, data: object = None, params: object = None):
    ...
    try:
        ...
        if repositoryId.status_code == 200:
            response = requests.get(url=f"{API_URL}/{quote(PROJECT)}/_apis/pipelines/{repositoryId['repoId']}/refs", headers=base_headers, params=base_parameters)
            if response.status_code == 200:
                json_response={
                            "function_code": 200,
