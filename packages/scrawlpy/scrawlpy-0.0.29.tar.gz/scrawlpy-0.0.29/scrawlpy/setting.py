...
def get_object_attrs(obj):
    ...
    def _is_valid_attr(key, value):
        """检查属性是否有效（非方法、非特殊属性）"""
        return not (callable(value) or isinstance(value, (classmethod, staticmethod, property)) or key.startswith("__"))

    def _collect_valid_attrs(source_obj, attrs_dict):
        """从 source_obj 收集有效属性到 attrs_dict"""
        for key, value in vars(source_obj).items():
            if _is_valid_attr(key, value):
    ...
    # 处理类或实例的继承链
    mro = obj.__mro__ if isinstance(obj, type) else obj.__class__.__mro__
    for base in mro:
        _collect_valid_attrs(base, attrs)

    # 如果是实例，额外收集实例属性
    if not isinstance(obj, type):
        _collect_valid_attrs(obj, attrs)
    ...
    def get_attributes(cls):
        ...
        for base in cls.__mro__:
            for key, value in vars(base).items():
                if not (
                    callable(value) or isinstance(value, (classmethod, staticmethod, property)) or key.startswith("__")
                ):
