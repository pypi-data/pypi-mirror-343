...
def dependencies(path, output, file, include_patterns, exclude_patterns, debug):
    ...
    Identifies import relationships between modules and files.
    """
    click.echo(f"Extracting dependencies from: {path}")
    ...
    click.echo(f"Dependencies extracted and saved to: {json_path}")

@cli.command('query-deps')
@click.argument('file_path', type=str)
@click.option('--dependency-file', '-d', default='output/python_dependencies.json',
...
def query_deps(file_path, dependency_file, direction):
    """
    Query dependency information for specific files.
    ...
    """
    from codex_arch.cli.query_deps_cmd import query_dependencies
    
    # Check if dependency file exists
    if not os.path.exists(dependency_file):
        click.echo(f"Error: Dependency file '{dependency_file}' not found.")
        click.echo(f"Run 'codex-arch dependencies' command first to generate dependencies.")
        return 1
    
    return query_dependencies(file_path, dependency_file, direction)
