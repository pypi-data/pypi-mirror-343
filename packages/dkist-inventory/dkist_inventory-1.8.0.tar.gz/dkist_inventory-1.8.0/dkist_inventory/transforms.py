...
def varying_spatial_model_from_headers(axes: list[int], parser: HeaderParser):
    ...
    if "crpix" in varying_axes:
        ...
        # The fits headers are correct for N images with a length one rastering axis.
        # We need to offset them so the 0th pixel is the first one.
        ...
        if lonind in varying_axes["crpix"]:
            crpix_offset[:, 0] = np.arange(crpix_table.shape[0])
        if latind in varying_axes["crpix"]:
            crpix_offset[:, 1] = np.arange(crpix_table.shape[1])
