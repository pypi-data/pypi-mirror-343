...
class TestSqlite3Client:
    ...
    async def test_get_suggestions(self, client):
        """Test get_suggestions returns both commands and table names"""
        # First call will populate the cache
        suggestions = await client.get_suggestions()
        
        # Verify we have both commands and tables
        assert any(item.endswith(" (COMMAND)") for item in suggestions)
        assert any(item.endswith(" (TABLE)") for item in suggestions)
        assert "test_table (TABLE)" in suggestions
