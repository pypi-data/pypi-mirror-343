...
class TestClientClass:
    ...
    async def test_get_suggestions(self, client):
        """Test get_suggestions returns properly formatted commands"""
        suggestions = await client.get_suggestions()
        expected_count = len(client.all_commands)
        assert len(suggestions) == expected_count
        assert all(item.endswith(" (COMMAND)") for item in suggestions)
