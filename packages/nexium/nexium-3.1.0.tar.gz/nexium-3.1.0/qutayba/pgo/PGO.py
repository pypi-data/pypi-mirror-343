...
"""Python level PGO handling in nexium."""

import os
import struct

from qutayba.__past__ import xrange
from qutayba.Options import getPythonPgoUnseenModulePolicy
from qutayba.Tracing import pgo_logger
...
def readPGOInputFile(input_filename):
    ...
    with open(input_filename, "rb") as input_file:
        ...
        if header != b"KAY.PGO":
            pgo_logger.sysexit(
                "Error, file '%s' is not a valid PGO input for this version of nexium."
                % input_filename
            )
