...
from qutayba.__past__ import basestring, unicode
from qutayba.containers.OrderedDicts import OrderedDict
from qutayba.Progress import enableThreading
from qutayba.PythonFlavors import isTermuxPython
...
from qutayba.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def _enableFlagSettings(env, name, experimental_flags):
    for flag_name in experimental_flags:
        ...
        if value:
            env.Append(CPPDEFINES=[("_DEVILPY_%s" % flag_name, value)])
        else:
            env.Append(CPPDEFINES=["_DEVILPY_%s" % flag_name])
...
def createEnvironment(
    ...
    env.qutayba_python = getArgumentBool("qutayba_python", False)
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.qutayba_src, "static_src", sub_path)
...
def createDefinitionsFile(source_dir, filename, definitions):
    for env_name in os.environ["_DEVILPY_BUILD_DEFINITIONS_CATALOG"].split(","):
    ...
    with openTextFile(build_definitions_filename, "w", encoding="utf8") as f:
        for key, value in sorted(definitions.items()):
            if key == "_DEVILPY_BUILD_DEFINITIONS_CATALOG":
                continue
    ...
    if not _linker_arch_determined:
        if os.name == "nt":
            ...
            _linker_arch = _getBinaryArch(
                binary=os.environ["DEVILPY_PYTHON_EXE_PATH"], mingw_mode=mingw_mode
            )
...
b) Using "--mingw64" lets nexium download MinGW64 for you. Note: MinGW64
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_qutayba_temp.pyd" if env.module_mode else "_qutayba_temp.exe",
        )
