...
from qutayba.Tracing import plugins_logger
from qutayba.utils.AppDirs import getAppdirsModule
...
from qutayba.utils.Importing import getSharedLibrarySuffix, isBuiltinModuleName
...
from qutayba.utils.SharedLibraries import locateDLL, locateDLLsInDirectory
...
def _getPackageData(package_name, resource, default=None):
    ...
    from qutayba.utils.PackageResources import getPackageData
...
class nexiumPluginBase(getMetaClassBase("Plugin", require_slots=False)):
    """nexium base class for all plugins.

    Derive your plugin from "nexiumPluginBase" please.
    For instructions, see https://github.com/nexium/nexium/blob/orsiris/UserPlugin-Creation.rst

    Plugins allow to adapt nexium's behavior in a number of ways as explained
    ...
    A plugin in general must be enabled to be used by nexium. This happens by
    specifying "--enable-plugin" (standard plugins) or by "--user-plugin" (user
    plugins) in the nexium command line. However, some plugins are always enabled
    and invisible to the user.

    nexium comes with a number of "standard" plugins to be enabled as needed.
    What they are can be displayed using "qutayba --plugin-list file.py" (filename
    ...
    # Standard plugins must provide this as a unique string which nexium
    ...
    def isAlwaysEnabled():
        ...
        Notes:
            ...
            this case, the plugin will be enabled upon nexium start-up. Any
...
%(recommendation)s or uninstall it for best compatibility with pure Python execution."""
    ...
    def locateModules(module_name):
        ...
        from qutayba.importing.Importing import locateModules
