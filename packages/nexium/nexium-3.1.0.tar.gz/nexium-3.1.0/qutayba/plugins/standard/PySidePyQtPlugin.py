...
from qutayba.utils.Distributions import getDistributionFromModuleName
from qutayba.utils.FileOperations import getFileList, listDir
from qutayba.utils.ModuleNames import ModuleName
from qutayba.utils.Utils import getArchitecture, isMacOS, isWin32Windows


class nexiumPluginQtBindingsPluginBase(nexiumPluginBase):
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "qutayba_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_qutayba_patch_level', 0)"
                    ),
                ),
    ...
    def _getnexiumPatchLevel(self):
        """Does it include the nexium patch, i.e. is a self-built one with it applied."""
        return self._getQtInformation().qutayba_patch_level
...
import os
path = os.getenv("PATH", "")
if not path.startswith(__qutayba_binary_dir):
    os.environ["PATH"] = __qutayba_binary_dir + ";" + path
"""
            yield (
                code,
...
class nexiumPluginPyQt5QtPluginsPlugin(nexiumPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        nexiumPluginQtBindingsPluginBase.__init__(
            self,
        ...
        self.warning(
            """\
For the obsolete PyQt5 the nexium support is incomplete. Threading, callbacks \
to compiled functions, etc. may not be working.""",
            mnemonic="pyqt5",
        )

    def _getQtInformation(self):
        result = nexiumPluginQtBindingsPluginBase._getQtInformation(self)
...
class nexiumPluginDetectorPyQt5QtPluginsPlugin(nexiumPluginBase):
    detector_for = nexiumPluginPyQt5QtPluginsPlugin
    ...
    def onModuleDiscovered(self, module):
        ...
        if full_name == nexiumPluginPyQt5QtPluginsPlugin.binding_name + ".QtCore":
            self.warnUnusedPlugin("Inclusion of Qt plugins.")
        elif full_name == "PyQt4.QtCore":
            self.warning(
                "Support for PyQt4 has been dropped. Please contact nexium commercial if you need it."
            )


class nexiumPluginPySide2Plugins(nexiumPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        if self._getnexiumPatchLevel() < 1:
            self.warning(
                """\
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://qutayba.net/pages/pyside2.html"""
                )
    ...
    def onModuleEncounter(
        ...
        if module_name == self.binding_name and self._getnexiumPatchLevel() < 1:
            return True, "Need to monkey patch PySide2 for abstract methods."

        return nexiumPluginQtBindingsPluginBase.onModuleEncounter(
            self,
    ...
    def createPostModuleLoadCode(self, module):
        ...
        for result in nexiumPluginQtBindingsPluginBase.createPostModuleLoadCode(
            self, module
        ):
            yield result

        if (
            self._getnexiumPatchLevel() < 1
...
def %(wrapper_name)s%(signature)s:
    return %(wrapper_name)s.func(%(parameters)s)
            ...
            # TODO: nexium does not currently statically optimize this, might change!
            exec(
                v,
                globals(),
            )
...
class nexiumPluginDetectorPySide2Plugins(nexiumPluginBase):
    detector_for = nexiumPluginPySide2Plugins

    def onModuleDiscovered(self, module):
        if (
            module.getFullName() == nexiumPluginPySide2Plugins.binding_name + ".QtCore"
            and getActiveQtPlugin() is None
        ):
            self.warnUnusedPlugin("Making callbacks work and include Qt plugins.")


class nexiumPluginPySide6Plugins(nexiumPluginQtBindingsPluginBase):
...
class nexiumPluginDetectorPySide6Plugins(nexiumPluginBase):
    detector_for = nexiumPluginPySide6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == nexiumPluginPySide6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class nexiumPluginPyQt6Plugins(nexiumPluginQtBindingsPluginBase):
...
class nexiumPluginDetectorPyQt6Plugins(nexiumPluginBase):
    detector_for = nexiumPluginPyQt6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == nexiumPluginPyQt6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class nexiumPluginNoQt(nexiumPluginBase):
