...
class nexiumPluginMultiprocessingWorkarounds(nexiumPluginBase):
    """This is to make multiprocessing work with nexium and use compiled code.
...
"""
                % ("__qutayba_binary_exe" if Options.isStandaloneMode() else "argv0"),
                """\
...
Monkey patching "multiprocessing" for compiled methods.""",
    ...
    def createFakeModuleDependency(module):
        ...
        # into the code and let nexium compile time optimize it, which will also
        # make sure joblib support is there for Python2 as well.
        if python_version >= 0x300:
            source_code += """
def __qutayba_freeze_support():
    import sys
...
__import__("multiprocessing.forking").forking.freeze_support()"""
    ...
    def getPreprocessorSymbols():
        if getModuleInclusionInfoByName("__parents_main__"):
            return {"_DEVILPY_PLUGIN_MULTIPROCESSING_ENABLED": "1"}
