...
nexium can detect some things that "pkg_resources" may not even be able to during
...
from qutayba.utils.Utils import withNoDeprecationWarning


class nexiumPluginResources(nexiumPluginBase):
...
class nexiumProvider(EggProvider):
    def _has(self, path):
...
register_loader_type(__qutayba_loader_type, nexiumProvider)
"""

        yield (
            code,
            """Registering nexium loader with "pkg_resources".""",
        )
