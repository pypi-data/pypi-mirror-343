...
from qutayba.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    """Get nexium package configuration. Merged from multiple sources."""
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            ...
            "standard.qutayba-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "qutayba.plugins.standard",
                "stdlib2.qutayba-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "qutayba.plugins.standard",
                "stdlib3.qutayba-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "qutayba.plugins.commercial", "commercial.qutayba-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    """Get the filename of the schema for nexium package configuration."""
    return os.path.join(
        os.path.dirname(__file__),
        "..",
        "..",
        "misc",
        "qutayba-package-config-schema.json",
    )
