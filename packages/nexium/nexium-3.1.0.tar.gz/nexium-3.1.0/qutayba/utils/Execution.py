...
from qutayba.__past__ import subprocess
...
class nexiumCalledProcessError(subprocess.CalledProcessError):
...
def check_output(*popenargs, **kwargs):
    ...
    if exit_code:
        ...
        raise nexiumCalledProcessError(exit_code, cmd, output=output, stderr=stderr)
...
def wrapCommandForDebuggerForExec(command, debugger=None):
    ...
    if debugger is None:
        debugger = os.getenv("DEVILPY_DEBUGGER_CHOICE")
