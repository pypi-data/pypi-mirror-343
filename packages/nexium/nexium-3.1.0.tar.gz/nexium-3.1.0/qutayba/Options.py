...
def _warnOSSpecificOption(option_name, *supported_os):
    if getOS() not in supported_os:
        if options.github_workflow_options:
            Tracing.options_logger.info(
                """\
Note: اختيار تشغيل النضام '%s' ماكو تأثير عليه %s."""
                % (option_name, getOS())
            )
        else:
            Tracing.options_logger.warning(
                """\
اختيار تشغيل النضا '%s' ماكو تأثير عليه %s."""
                % (option_name, getOS())
            )
...
'--follow-import-to=name' though."""
    ...
    if (
        ...
        Tracing.options_logger.warning(
            """لم تحدد متابعة أو تضمين أي شيء سوى الرئيسي %s. تحقق من الخيارات وتأكد من أن هذا هو المقصود."""
            % ("وحدة" if shallMakeModule() else "برمجة")
        )
