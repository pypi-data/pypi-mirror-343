...
def compileTree():
    ...
    general.info("تم الانتهاء من تجميع وتحسين مستوى بايثون.")

    if not Options.shallOnlyExecCCompilerCall():
        general.info("إنشاء كود المصدر لمترجم الواجهة الخلفية C.")

        reportMemoryUsage(
            "before_c_code_generation",
            (
                "إجمالي استخدام الذاكرة قبل إنشاء كود C:"
        ...
        if not os.path.isfile(os.path.join(source_dir, "__helpers.h")):
            general.sysexit("خطأ، لا يوجد دليل بناء سابق.")

    reportMemoryUsage(
        "before_running_scons",
        (
            "إجمالي استخدام الذاكرة قبل تشغيل الفحص"
            if Options.isShowProgress() or Options.isShowMemory()
            else None
        ),
    )
    ...
    general.info("تشغيل أداة مؤلف البيانات للتعامل الأمثل مع القيمة الثابتة.")

    runDataComposer(source_dir)

    Plugins.writeExtraCodeFiles(onefile=False)

    general.info("جار تجميع ملفات سي.")
...
def _main():
    ...
    general.info(
        "جار تجميع البايثون بمكتبة نصر %r اصدار بايثون (%s), %r لدرجة %r."
        % (
            getnexiumVersion(),
            getPythonFlavorName(),
            python_version_str,
            getCommercialVersion() or "غير محمل",
        )
    )
    ...
    reportMemoryUsage(
        "after_launch",
        (
            "إجمالي استخدام الذاكرة قبل المعالجة:"
            if Options.isShowProgress() or Options.isShowMemory()
            else None
        ),
    )
    ...
    if not result:
        general.sysexit(
            message="فشل بشكل غير متوقع في تجميع الواجهة الخلفية لـ فحص ملفات سي.",
    ...
    if Options.shallCreatePythonPgoInput():
        ...
        pgo_filename = OutputDirectories.getPgoRunInputFilename()
        general.info(
            "إعادة تشغيل التجميع باستخدام المعلومات المجمعة من '%s'."
            % pgo_filename
        )
    ...
    if Options.isStandaloneMode():
        ...
        if Options.isOnefileMode():
            ...
            if Options.isRemoveBuildDir():
                general.info("حذف المجلد '%s'." % dist_dir)
                ...
                general.info(
                    "أبقاء المجلد '%s' للفحص، لا حاجة لاستخدامه."
                    % dist_dir
                )
    ...
    if Options.isRemoveBuildDir():
        general.info("حذف المسار '%s'." % source_dir)
        ...
        general.info("أبقاء المسار لحفض التجميع بداخله '%s'." % source_dir)
...
not use compiled code while it exists."""
    ...
    general.info("تم الانتهاء من التجميع '%s'." % getReportPath(final_filename))
...
def main():
    try:
        ...
        try:
            ...
            general.warning("""تم منع كتابة التقرير بسبب مقاطعة المستخدم. لانه غبي وحمار""")
