...
class OurLogger(object):
    ...
    def sysexit_exception(self, message, exception, exit_code=1):
        self.my_print("فشل: %s" % message, style="red", file=sys.stderr)

        traceback.print_exc()
        self.sysexit("فشل:" + repr(exception), exit_code=exit_code, reporting=True)
...
class FileLogger(OurLogger):
    ...
    def debug(self, message, style=None):
        if self.file_handle:
            message = "تصحيح: %s" % (self.name, message)
