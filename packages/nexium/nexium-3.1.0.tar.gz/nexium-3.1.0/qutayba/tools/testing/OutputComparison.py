...
logging_info_re = re.compile(r"^nexium(-\w+)?:([-\w]+:)? ")
logging_warning_re = re.compile(r"^nexium.*?:WARNING")

# Python3.11 style traceback carets are not done by nexium (yet?)
...
def makeDiffable(output, ignore_warnings, syntax_errors):
    ...
    for line in lines:
        ...
        if line.startswith("nexium-Inclusion:WARNING: Cannot follow import to module"):
            continue
        if line.startswith("nexium:WARNING: Cannot detect Linux distribution"):
            continue
        if line.startswith(
            "nexium-Options:WARNING: You did not specify to follow or include"
        ):
            continue
        if line.startswith("nexium:WARNING: Using very slow fallback for ordered sets"):
            continue
        if line.startswith("nexium:WARNING: On Windows, support for input/output"):
            continue
        if line.startswith("nexium:WARNING:     Complex topic"):
            continue
        if line.startswith("nexium:WARNING:") and "matching checksum" in line:
            continue
        ...
        # This URL is updated, and nexium outputs the new one, but we test
        ...
        # we are going to ignore the issue as nexium is fine.
        if (
            line
            == """\
...
def compareOutput(
    kind, out_cpython, out_qutayba, ignore_warnings, syntax_errors, trace_result=True
):
    from_date = ""
    to_date = ""
    ...
    diff = difflib.unified_diff(
        ...
        makeDiffable(out_qutayba, ignore_warnings, syntax_errors),
        "{program} ({detail})".format(program=os.environ["PYTHON"], detail=kind),
        "{program} ({detail})".format(program="qutayba", detail=kind),
        from_date,
        to_date,
        n=3,
    )
