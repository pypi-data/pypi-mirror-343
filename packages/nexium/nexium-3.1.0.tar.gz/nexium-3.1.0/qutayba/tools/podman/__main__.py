...
from qutayba.tools.release.Release import (
    getBranchName,
...
from qutayba.utils.Utils import getArchitecture, isWin32Windows

from .Podman import getPodmanExecutablePath

containers_logger = OurLogger("nexium-Containers", base_style="blue")
...
def main():
    ...
    if options.command is None:
        options.command = "python3 -m qutayba --version"
    ...
    container_tag_name = "qutayba-build-%s-%s:latest" % (
    ...
    if options.network:
        command.append("--add-host=ssh.qutayba.net:116.202.30.188")
    else:
