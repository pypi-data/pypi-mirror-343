...
class HeartbeatClient:
    ...
    def get_all_public_data(self):
        try:
            ...
            logger.exception(f"get_all_public_data in exception: {e}")
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    logger.debug(f"get_entrypoint_config ok:{config}")
                    if 'config' in config:
                    ...
                    logger.exception(f"JSON解析错误: {e}, 原始响应: {response.text}")
            else:
                logger.error(
                    f"get_entrypoint_config failed:{response.text}"
                )  # 显示原始响应
        except Exception as e:
            logger.exception(f"get_entrypoint_config in exception: {e}")
    ...
    def get_agent_public_data(self):
        try:
            ...
            logger.exception(f"get_agent_public_data in exception: {e}")
    ...
    def get_agent_private_data(self):
        try:
            ...
            logger.exception(f"get_agent_private_data in exception: {e}")
