...
class CAClient:
    ...
    def __init__(self, ca_server, timeout):
    ...
    def send_csr_to_server(self, name):
        try:
            ...
            if response.status_code == 200:
                ...
                crt_name = f"aid/{name}/{name}.crt"
                with open(crt_name, "wb") as f:
                    f.write(response.json()["certificate"].encode('utf-8'))
                    csr_name = f"aid/{name}/{name}.csr"
                    self.__save_csr_to_file(csr, csr_name)
                    self.__save_private_key_to_file(name, private_key)
                logger.info(f'signed certificate successfully: {name} {csr_name}')  # 调试用
                return True
            else:
                logger.error(f"sign failed: {response.status_code} - {response.json()['error']}")  # 调试用
