...
class DBManager:
    ...
    def create_conversation(self, aid,session_id,identifying_code,name, type,to_aid_list:list):
        try:
            ...
            main_aid = to_aid_list[0] if to_aid_list else aid            
            # 修正参数数量，确保5个值对应5个占位符
    ...
    def insert_message(self, role,aid,conversation_id, message_aid, parent_message_id, to_aids, content, type, status):
        try:
            ...
            cursor.execute(f'''INSERT INTO {messages_table} (session_id, role,message_aid, parent_message_id, to_aids, content, type, status) VALUES (?,?,?,?,?,?,?,?)''',
