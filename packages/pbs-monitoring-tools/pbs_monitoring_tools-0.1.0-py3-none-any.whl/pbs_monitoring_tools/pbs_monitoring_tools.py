...
def get_cli_parser():
    ...
    parser.add_argument(
        "--no-errors", action="store_true", help="Print nothing if there is no errors."
    )

    return parser


def get_proxmox_api_handle(authinfo_file: str):
    """
...
def main():
    ...
    api_handle = get_proxmox_api_handle(arguments.auth_file)

    if not arguments.datastores:
        datastores = [
            datastore["store"] for datastore in get_datastores_list(api_handle)
        ]
    else:
