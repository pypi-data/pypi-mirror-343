...
def float32_to_hex(f):
    return format(struct.unpack(str("<I"), struct.pack(str("<f"), f))[0], "0>8X")


def int32_to_hex(i):
    return format(i, "0>8X")
...
class MeerstetterTEC1090(object):
    ...
    def get_stage_enable(self, address):
        param_no = 2010
        return self.get_param(address, param_no, 1, param_type="int")

    def set_stage_enable(self, address, stage_enable):
        param_no = 2010
        if stage_enable not in STAGE_ENABLE_CODES:
            raise ValueError(
                "stage_enable must be one of {}".format(STAGE_ENABLE_CODES.keys())
            )
        payload = "VS{param_no:0>4X}{instance:0>2X}{encoded_val}".format(
            param_no=param_no, instance=1, encoded_val=int32_to_hex(stage_enable)
        )
        frame_msg = self._assemble_frame(address, payload)
        ret_msg = self._send_frame(frame_msg)
        if ret_msg.startswith("+"):
