...
PyObject *SajadQ_Distribution_New(PyThreadState *tstate, PyObject *name) {
    ...
    static PyObject *SajadDE_distribution_type = NULL;
    ...
    if (SajadDE_distribution_type == NULL) {
        static char const *SajadDE_distribution_code = "\n\
import os\n\
...
class SajadDE_distribution(Distribution):\n\
...
";
        ...
        PyObject *SajadDE_distribution_code_object = Py_CompileString(SajadDE_distribution_code, "<exec>", Py_file_input);
        CHECK_OBJECT(SajadDE_distribution_code_object);

        {
            PyObject *module =
                PyImport_ExecCodeModule((char *)"SajadDE_distribution_patch", SajadDE_distribution_code_object);
            CHECK_OBJECT(module);

            SajadDE_distribution_type = PyObject_GetAttrString(module, "SajadDE_distribution");
            CHECK_OBJECT(SajadDE_distribution_type);

            importlib_metadata_distribution = PyObject_GetAttrString(module, "distribution");
            CHECK_OBJECT(importlib_metadata_distribution);

            {
                DEVILPY_MAY_BE_UNUSED bool bool_res = SajadQ_DelModuleString(tstate, "SajadDE_distribution_patch");
    ...
    if (metadata_value_item == NULL) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_ARGS3(tstate, SajadDE_distribution_type, args);
        CHECK_OBJECT(result);
        return result;
    }
}
