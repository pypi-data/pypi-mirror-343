...
{{#partial shownumusers}}
    ...
    <!-- Setting variable inside partial only affects partial scope -->
    {{#set halfusers 0 }}
    Partial Scope (after set): Half of the users is {{ halfusers }}
{{/partial}}

<!-- Do an inline query for older folks -->
<h2>All Adults</h2>
...
<!-- Render the partial, passing a parameter which overrides the global one *for that render* -->
Render with param:
{{#render shownumusers halfusers=3}}

<!-- Render again, it uses the global value -->
Render after param:
{{#render shownumusers}}

{{#dump users}}

<!-- Create a SQL view of minors -->
{{#view youngPeople from users WHERE age < :halfusers}}

{{#dump youngPeople}}

<h2>All Minors (Age < {{ halfusers }})</h2>
<table>
  <thead>
    <tr>
      <th>Name</th>
