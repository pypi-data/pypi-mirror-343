...
class PerRequestAssessmentResult(AssessmentResult):
    ...
    def to_mlflow_assessment_v2(
        ...
        return mlflow_eval.Assessment(
            ...
            metadata={_SPAN_ID: self.span_id} if self.span_id else None,
        )

    def to_mlflow_assessment_v3(self, trace_id: str, root_span_id: Optional[str]):
        """
        ...
        return mlflow_entities.Assessment(
            trace_id=trace_id,
            span_id=self.span_id or root_span_id,
...
class PerChunkAssessmentResult(AssessmentResult):
    ...
    def to_mlflow_assessment_v2(
        ...
        return [
            mlflow_eval.Assessment(
                ...
                error_message=rating.error_message,
                metadata={
                    _CHUNK_INDEX_KEY: position,
                    **({_SPAN_ID: self.span_id} if self.span_id else {}),
                },
            )
    ...
    def to_mlflow_assessment_v3(
        self, trace_id: str, root_span_id: Optional[str]
    ...
    def assessments(self) -> List[mlflow_entities.Assessment]:
        ...
        for metric in self.metric_results:
            ...
            converted_metric_results.append(
                metric.to_mlflow_assessment_v3(trace_id, root_span_id)
            )
        ...
        for assessment in self.assessment_results:
            if isinstance(assessment, PerRequestAssessmentResult):
                converted_assessment_results.append(
                    assessment.to_mlflow_assessment_v3(trace_id, root_span_id)
                )
