...
def _calculate_correction_factor_ms2(
    ...
    return np.nansum(
        np.array(comb.abundance.to_list()) * np.array(comb.p_ms2.to_list())
    )
...
def get_source_id2area_corrs(
    ...
    for source in sources:
        for norm_id, ids_sam in norm_id2ids.items():
            ...
            areas_corr_std, areas_corrs_sam = _solve_overlapping_areas(
                models_std, models_sam, fcorrs_std, fcorrs_sam
            )
            _update_areas_corr(source_id2area_corr, source, ids_std, areas_corr_std)
            _update_areas_corr(source_id2area_corr, source, ids_sam, areas_corrs_sam)
        ...
        for i, model_is in enumerate(models_is):
            for j, model in enumerate(models):
                ...
                if len(rts) and len(rts_is):
                    ...
                    area_is_o = np.trapz(ints_is_overlap, rts)
                    area_nl_o = np.trapz(ints_overlap, rts_is)
                else:
