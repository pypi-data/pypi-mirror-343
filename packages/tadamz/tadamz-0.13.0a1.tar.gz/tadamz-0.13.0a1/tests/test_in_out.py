...
import numpy as np
import emzed
from emzed import Table, PeakMap
...
def test_load_peaks_table(_paths, regtest):
    for path in _paths:
        t = in_out.load_peaks_table(path)
...
def test_get_classifier_path(_create):
    ...
    for i in range(3):
        ...
        x = in_out.get_classifier_path(classifier_name, path_to_folder=data_folder)
...
def test_save_classifier_0():
    ...
    with pytest.raises(AssertionError):
        in_out.save_classifier(None, classifier_name, path_to_folder=data_folder)


def test_load_save_targeted_wf_table_0():
    import sys

    print(sys.path)
    path = os.path.join(data_folder, "calibration_table.table")
    t = in_out.load_twf_table(path)
    print(t.col_names)
    path1 = path = os.path.join(data_folder, "calibration_table1.table")
    in_out.save_twf_table(t, path1, overwrite=True)
    t = in_out.load_twf_table(path1)
    os.remove(path1)
