...
import numpy as np
import emzed
from src.tadamz.in_out import load_peaks_table
...
def test_get_std_correction_factor_dict_1(result_table_ms2):
    ...
    exp = {2: {1: 0.04968291898704502}, 6: {5: 0.0}, 4: {3: 0.0}}
    equals = []
    for key in exp.keys():
        equals.extend(
            [abs(is_[key][key2] - exp[key][key2]) < 1e-14 for key2 in exp[key].keys()]
...
def test_get_sam_correctuion_factor_dict_1(result_table_ms2, t_iso_dist_ms2):
    cio._update_isotopologue_distribution_table(
        result_table_ms2,
        t_iso_dist_ms2,
        mztol_abs,
...
def test_isotopologue_overlays_3(result_table_ms2, t_iso_dist_ms2, kwargs_ms2):
    ...
    id2fcorr = {1: 0.04968291898704502, 2: 0, 3: 0, 4: 0.1, 5: 0, 6: 0}
    id2area = dict(zip(result_table_ms2.id, result_table_ms2.area_chromatogram))
    exps = []
    for id_, area, area_corr in zip(
        result_table_ms2.id,
        result_table_ms2.original_area_chromatogram,
        result_table_ms2.area_chromatogram,
    ):
        ida = id_ - 1 if (id_ / 2) == int(id_ / 2) else id_ + 1
        delta_rel = abs(area - (id2area[ida] * id2fcorr[id_]) - area_corr) / area_corr
        print(delta_rel)
...
def test_isotopologue_overlays_5(result_table_ms2, kwargs_ms2):
    cio.correct_isotopologue_overlays(result_table_ms2, kwargs_ms2, None)
    id2fcorr = {1: 0.04968291898704502, 2: 0, 3: 0, 4: 0.0, 5: 0, 6: 0}
    id2area = dict(zip(result_table_ms2.id, result_table_ms2.area_chromatogram))
    exps = []
    for id_, area, area_corr in zip(
        result_table_ms2.id,
...
def test_correct_isotopologue_overlays_0(
    result_table_ms1ms2, result_table_ms1, kwargs_ms2
):
    result_table_ms1.add_column_with_constant_value("mf_fragment", None, str)
    result_table_ms1.add_column_with_constant_value("adduct_fragment", None, str)
    cio.correct_isotopologue_overlays(result_table_ms1, kwargs_ms2)
...
def test_correct_isotopologue_overlays_1(
    result_table_ms1ms2, result_table_ms2, kwargs_ms2
):
    cio.correct_isotopologue_overlays(result_table_ms2, kwargs_ms2)
    # is2 = cio.correct_isotopologue_overlays(result_table_ms2, kwargs_ms2)
    cio.correct_isotopologue_overlays(result_table_ms1ms2, kwargs_ms2)
    is1 = result_table_ms2
    is12 = result_table_ms1ms2
    comb = is12.join(
        is1,
        (
