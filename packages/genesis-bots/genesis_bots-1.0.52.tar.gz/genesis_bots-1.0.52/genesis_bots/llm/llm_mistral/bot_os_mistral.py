...
class BotOsAssistantMistral(BotOsAssistantInterface):
   def __init__(self, name:str, instructions:str,
                tools:list[dict], available_functions={}, files=[], update_existing=False) -> None:
      logger.debug("BotOsAssistantMistral:__init__")
      api_key = os.environ["MISTRAL_API_KEY"]
      self.model_name = os.getenv("MISTRAL_MODEL_NAME", default="mistral-large-latest")
   ...
   def create_thread(self) -> str:
      ...
      self.thread_messages_map[thread] = [ChatMessage(role="system", content=self.instructions)]
      return thread
   ...
   def add_message(self, input_message:BotOsInputMessage):#thread_id:str, message:str, files):
      ...
      self.thread_messages_map[thread_id].append(ChatMessage(role="user", content=input_message.msg))
   ...
   def check_runs(self, event_callback):
      logger.debug("BotOsAssistantMistral:check_runs")
...
def test():
    m = BotOsAssistantMistral("Mistral","Talk like a pirate.",[])
