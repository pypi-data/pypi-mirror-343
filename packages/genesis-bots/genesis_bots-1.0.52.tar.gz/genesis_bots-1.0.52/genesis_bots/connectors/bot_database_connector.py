...
genesis_source = os.getenv("GENESIS_SOURCE", default="SQLite")


# Global variable to hold the bot connection
def bot_credentials(bot_id):
    """
    This function returns a single bot connection toa non-Snowflake db.
    ...
    try:
        if genesis_source == "Sqlite":
            ...
            connector = DatabaseConnector("Snowflake")
        else:
        ...
        if genbot_internal_project_and_schema is not None:
            genbot_internal_project_and_schema = (
                genbot_internal_project_and_schema.upper()
            )
        ...
        bot_config = connector.db_get_bot_database_creds(
        ...
        logger.info(
            f"Error getting bot credentials for {bot_config['bot_id']} : {str(e)}"
        )
