...
from ._exceptions import APIResponseValidationError, GitHubAPIError
from ._models import BaseModel, is_basemodel
from ._streaming import AsyncStream, Stream, extract_stream_chunk_type, is_stream_class_type
from ._types import NoneType
from ._utils import extract_type_arg, extract_type_var_from_base, is_annotated_type, is_given, is_type_alias_type
...
class BaseAPIResponse(Generic[R]):
    ...
    _is_sse_stream: bool
    _stream_cls: type[Stream[Any] | AsyncStream[Any]] | None
    ...
    def _parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        if self._is_sse_stream:
            ...
            stream_cls = cast("type[Stream[Any] | AsyncStream[Any]] | None", self._client._default_stream_cls)
        ...
        if (
            ...
            raise TypeError("Pydantic models must subclass our base model type, e.g. `from github_fluent_api import BaseModel`")

        if cast_to is not object and not origin is list and not origin is dict and not origin is Union and not issubclass(origin, BaseModel):
            raise RuntimeError(
...
class APIResponse(BaseAPIResponse[R]):
    ...
    def parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        ```py
        from github_fluent_api import BaseModel
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__(
            "The `stream` argument was set to `True` but the `stream_cls` argument was not given. See `github_fluent_api._streaming` for reference",
        )


class StreamAlreadyConsumed(GitHubAPIError):
    """
