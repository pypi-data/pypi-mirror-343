...
from typing_extensions import Literal, Self, get_origin, override
...
class BaseSyncPage(BasePage[_T], Generic[_T]):
    ...
    def iter_pages(self) -> Iterator[Self]:
        page = self
        while True:
            yield page
...
class BaseAsyncPage(BasePage[_T], Generic[_T]):
    ...
    async def iter_pages(self) -> AsyncIterator[Self]:
        page = self
        while True:
            yield page
            if page.has_next_page():
    ...
    async def get_next_page(self) -> Self:
        info = self.next_page_info()
        if not info:
            raise RuntimeError("No next page expected; please check `.has_next_page()` before calling `.get_next_page()`.")
...
class BaseClient(Generic[_HttpxClientT, _DefaultStreamT]):
    ...
    def __init__(
        ...
        *,
        ...
        if max_retries is None:  # pyright: ignore[reportUnnecessaryComparison]
            raise TypeError(
                "max_retries cannot be None. If you want to disable retries, pass `0`; if you want unlimited retries, pass `math.inf` or a very high number; if you want the default behavior, pass `github_fluent_api.DEFAULT_MAX_RETRIES`"
            )
    ...
    def _build_headers(self, options: FinalRequestOptions, *, retries_taken: int = 0) -> httpx.Headers:
        ...
        if "x-fluent-api-generator-retry-count" not in lower_custom_headers:
            headers["x-fluent-api-generator-retry-count"] = str(retries_taken)
        if "x-fluent-api-generator-read-timeout" not in lower_custom_headers:
            timeout = self.timeout if isinstance(options.timeout, NotGiven) else options.timeout
            if isinstance(timeout, Timeout):
                timeout = timeout.read
            if timeout is not None:
                headers["x-fluent-api-generator-read-timeout"] = str(timeout)
    ...
    def _idempotency_key(self) -> str:
        return f"fluent-api-generator-python-retry-{uuid.uuid4()}"
...
class SyncAPIClient(BaseClient[httpx.Client, Stream[Any]]):
    ...
    def _process_response(
        ...
        *,
        ...
        response: httpx.Response,
        stream: bool,
        stream_cls: type[Stream[Any] | AsyncStream[Any]] | None,
        retries_taken: int = 0,
    ) -> ResponseT:
...
def platform_headers(version: str, *, platform: Platform | None) -> dict[str, str]:
    return {
        "X-FluentAPIGenerator-Lang": "python",
        "X-FluentAPIGenerator-Package-Version": version,
        "X-FluentAPIGenerator-OS": str(platform or get_platform()),
        "X-FluentAPIGenerator-Arch": str(get_architecture()),
        "X-FluentAPIGenerator-Runtime": get_python_runtime(),
        "X-FluentAPIGenerator-Runtime-Version": get_python_version(),
    }
