...
def test_status_success(
    ...
    responses.add(
        responses.GET,
        f"{API_BASE_URL}/{GET_JOB_STATUS_ENDPOINT(job_id)}",
        json={
            "status_code": 200,
            "status": "success",
    ...
    synthex.jobs._current_job_id = job_id # type: ignore
    
    status = synthex.jobs.status()
...
def test_status_no_running_job_failure(
    ...
    # Ensure that no job is currently running by setting the current job ID to None.
