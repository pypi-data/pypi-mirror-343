    ...
    def _enhanced_browser_input_text(self, builtin, selenium_lib, locator, args, browser_name="unknown"):
        ...
        try:
            builtin.run_keyword_and_ignore_error("Execute JavaScript", 
                                              ...
                                              locator)
            # Brief pause to ensure element has settled
            builtin.run_keyword("Sleep", "0.3s")
        except:
            pass
        
        # Try to ensure element is visible and ready
        try:
            builtin.run_keyword_and_ignore_error("Wait Until Element Is Visible", locator, "2s")
        except:
            pass
        
        # Standard approach - try native input methods first
        standard_methods = [
            {"keyword": "Input Text", "args": [locator, text]},
            {"keyword": "Press Keys", "args": [locator, text]}
        ]
        
        for method in standard_methods:
            try:
                builtin.run_keyword(method["keyword"], *method["args"])
                return
            except Exception as e:
                logger.debug(f"{method['keyword']} failed: {e}")
        
        # Direct WebDriver approach
        try:
            ...
            logger.debug(f"WebDriver send_keys failed: {e}")
        
        # JavaScript approaches - with browser-specific handling
        js_methods = []
        
        # Simple approach for Edge and other browsers with issues
        if is_edge:
            js_methods.append("""
                try {
                    ...
                    console.error('Simple value set failed:', e);
                    return false;
                }
            """)
            
            # Simplest approach for Edge as fallback
            js_methods.append("""
                try {
                    var el = arguments[0];
        ...
        # More comprehensive approach for other browsers
        js_methods.append("""
            try {
                var el = arguments[0];
                ...
                // Try to trigger events if supported by browser
                try {
                    // Input event
                    ...
                    // Some browsers may not support this approach
                }
        ...
        # Try each JavaScript approach in order
        for js_code in js_methods:
            try:
                result = builtin.run_keyword("Execute JavaScript", js_code, locator, text)
                if result:
                    logger.info("Successfully used JavaScript to set input value")
                    return
            except Exception as e:
                logger.debug(f"JavaScript input approach failed: {e}")
        
        # Last resort approach - try to find element by CSS selector directly
        try:
            if locator.startswith("css="):
                css_selector = locator[4:]  # Remove "css=" prefix
                js_code = f"""
                    try {{
                        var elements = document.querySelectorAll('{css_selector}');
                        if (elements.length > 0) {{
                            elements[0].value = '{text.replace("'", "\\'")}';
                            return true;
                        }}
                        return false;
                    }} catch(e) {{
                ...
                """
                builtin.run_keyword("Execute JavaScript", js_code)
                logger.info("Used direct CSS selector JavaScript approach")
                return
        except Exception as e:
            logger.warn(f"All input methods failed: {e}")
            raise Exception(f"Failed to input text using all available methods: {e}")
