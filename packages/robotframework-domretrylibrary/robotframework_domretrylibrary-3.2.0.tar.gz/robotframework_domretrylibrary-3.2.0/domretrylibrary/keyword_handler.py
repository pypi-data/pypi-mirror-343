...
class KeywordHandler:
    ...
    def _get_ai_description(self, ai_description, primary_locator=None):
        ...
        # Check if we have the original variable name stored from _get_primary_locator
        ...
        # First check if this is a variable reference (not a direct locator)
        if not primary_locator.startswith(('id=', 'xpath=', 'css=', 'name=', 'class=', 'tag=', 'link=', 'partial link=')):
            # This is a variable name like 'CHI_FIELD', look for AI_{variable_name}
            ...
            # If we couldn't find the originating variable, try a more generic approach
            if not originating_var:
                # Check for direct AI variable matches (AI_xpath=value)
                ...
                # Try fuzzy matches as a last resort
                for var_name, var_value in all_vars.items():
                    if var_name.startswith('${AI_') and isinstance(var_value, str):
                        # For identical format locators, we need to match on the variable name portion
                        # Extract the base name from the AI variable (remove ${AI_ and })
                        base_name = var_name[5:-1]  # ${AI_CHI_FIELD} -> CHI_FIELD
                        
                        # Check if there's a variable with this name that matches our locator
                        base_var = "${" + base_name + "}"
    ...
    def _get_primary_locator(self, builtin, locator_var):
        # If this is a variable reference already resolved (${VAR} was passed)
        if isinstance(locator_var, str) and locator_var.startswith(('id=', 'xpath=', 'css=', 'name=', 'class=', 'tag=', 'link=', 'partial link=')):
            ...
            # Try to find which variable this locator came from by looking at all variables
            all_vars = builtin.get_variables()
            for var_name, var_value in all_vars.items():
                if var_name.startswith('${') and var_name.endswith('}') and var_value == locator_var:
                    ...
                    # Store the original variable name for later AI lookup
                    self.original_var_name = var_name[2:-1]  # Remove ${ and }
                    break
            return locator_var
