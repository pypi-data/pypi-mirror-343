...
class AIProcessor:
    ...
    def generate_locator(self, html, element_description, original_locator=None, page_url=None, selenium_lib=None):
        ...
        try:
            ...
            ai_locator = "//*"  # Very basic fallback
    ...
    def _call_openai_api(self, html, description, original_locator=None, chunk_info=None):
        ...
        system_message = """You are a web automation expert that helps find XPath selectors for web elements.
You MUST return a syntactically correct XPath selector that follows XPath specification.
ONLY return the raw XPath selector, nothing else - no explanations, no markdown formatting, no prefix.
...
MOST IMPORTANT: Return ONLY the raw XPath expression with no prefix or additional text."""
        
        user_message = f"""I need a precise XPath selector for: "{description}"\n\n"""
        ...
        user_message += """Return ONLY the raw XPath expression without any prefix.
