...
class Experiment:
    ...
    def evaluate(self, 
        ...
        Args:
            runs: List of runs requests to evaluate
        ...
        if not skip_sanity_checks:
            for challenge_slug in challenge_slugs:
                ...
                try:
                    ...
                    if not challenge['slug'] == challenge_slug:
                        raise ValueError(f"Challenge slug {challenge_slug} does not match challenge slug in API response {challenge['slug']}")
            ...
            for run in runs:
                for participant in run.participants:
                    participants.add(participant["agent"])

            print(f"Participants: {participants}")
            # do some sanity checks on all the participants
            for participant in participants:
                try:
                    participant_response = self._internal_api_client.agents.get(participant)
                    if not participant_response['username']:
                        raise ValueError(f"Participant agent username not found in API response")
                except Exception as e:
                    raise ValueError(f"Failed to query participant {participant} from Kradle") from e
    ...
    def _create_and_monitor_run(self, run_idx: int, run_request: RunRequest, max_retries: int = 3) -> Run:
        run_id = None
        run = None
        ...
        if run_id:
            ...
            self._logger.log_info(f"Run #{run_idx} id {run_id} "
        ...
        while True:
            ...
            try:
                ...
                if run_status != run.status:
                    ...
                    self._logger.log_info(f"Run #{run_idx} id: {run_id}"
                                          f"status: {run_status} (elapsed: {'0' if not run.total_time else f'{run.total_time/1000:.1f}s'})")
                    self.runs[run_id] = run
