...
#====================================================================================================================================================
    def AddLog(self, inst_type:TService):
        from ...Utilites.ext_reflection import ExtReflection
        
        if ExtReflection.is_class(inst_type):
            self._serviceDescriptor[inst_type] = ServiceDescriptorFWDI.create_log(inst_type, ServiceLifetime.Singleton)
    ...
    def __add(self, lifetime:ServiceLifetime, base_type:type[TService]=None, inst_type:type[TService]=None):
        ...
        if ExtReflection.is_class(base_type):
            if base_type != None and inst_type != None:
                self._serviceDescriptor[base_type] = ServiceDescriptorFWDI.create(base_type, inst_type, lifetime)
            elif base_type != None and inst_type == None:
                self._serviceDescriptor[base_type] = ServiceDescriptorFWDI.create_from_type(base_type, lifetime)
        else:
            if lifetime == ServiceLifetime.Singleton:
                self._serviceDescriptor[base_type] = ServiceDescriptorFWDI.create_from_instance(base_type, lifetime)
            else:
