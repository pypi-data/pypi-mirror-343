...
class ServiceDescriptorFWDI():
    ...
    @classmethod
    def create_log(cls: type[_CLS_], inst:T, lifetime:ServiceLifetime)-> _CLS_:
        new_instance = cls()
        new_instance.ImplementationType = type(inst)
        new_instance.ServiceType = new_instance.ImplementationType
        new_instance.ImplementationType = ServiceDescriptorFWDI.__set_inject_log_v1(inst)
        new_instance.Lifetime = lifetime

        return new_instance

    def __set_inject_v2(cls:abc.ABCMeta):
                    ...
                    match type_call:
                        case TypeMethod.Instance:
                            if inspect.iscoroutinefunction(value):
                                ...
                                setattr(cls, attr, ExtReflection._inject_inst_sync(value))
    ...
    def __set_inject_log_v1(cls:abc.ABCMeta):
