...
def select_best_gpu():
    if torch.cuda.is_available():
        # Obtenez le nombre de GPU disponibles
        ...
        print(f"Le meilleur GPU est GPU {best_gpu} avec {max_memory / (1024 ** 2):.2f} MB disponibles.")
...
def generate_2Dacoustic_field_KWAVE(folderPathBase,depth_end, angle_deg, active_listString, c0=1540, num_elements = 192, num_cycles = 4, element_width = 0.2/1000, depth_start = 0, f_US = 6e6, f_aq = 180e6, f_saving = 10e6, plotExcitation=True):
...
def calculate_envelope_squared(acoustic_field):
    ...
    Parameters:
    - acoustic_field (numpy.ndarray): Input acoustic field data. This should be a time-domain signal.

    Returns:
    - envelope (numpy.ndarray): The squared analytic envelope of the acoustic field. This represents the instantaneous power of the signal.
    """
    if (len(acoustic_field.shape) != 2 and len(acoustic_field.shape) != 3):
        raise ValueError("Input acoustic field must be a 2D or 3D array.")
    if len(acoustic_field.shape) == 2:
        return np.abs(hilbert(acoustic_field, axis=0))**2
    if len(acoustic_field.shape) == 3:
        EnveloppeField = np.zeros_like(acoustic_field)
        for y in range(acoustic_field.shape[2]):
            for z in range(acoustic_field.shape[1]):
                EnveloppeField[:, z, y, :] = np.abs(hilbert(acoustic_field[:, z, y, :], axis=1))
        return EnveloppeField  

def generate_3Dacoustic_field_KWAVE(folderPathBase,depth_end, angle_deg, active_list_hex, c0=1540, num_elements = 192, num_cycles = 4, element_width = 0.2/1000, element_height = 6/1000, depth_start = 0, f_US = 6e6, f_aq = 180e6, f_saving = 10e6, IsSaving=True):
    ...
    execution_options = SimulationExecutionOptions(
        is_gpu_simulation=select_best_gpu()[0],
        device_num = select_best_gpu()[1])  # True si GPU disponible
