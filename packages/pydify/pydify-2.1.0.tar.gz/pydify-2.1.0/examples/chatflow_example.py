...
def example_send_message_streaming():
    ...
    def on_node_started(data):
        print(
            f"节点开始: ID={data.get('node_id')}, 类型={data.get('node_type')}, 标题={data.get('title')}"
        )

    def on_node_finished(data):
        print(
            f"节点完成: ID={data.get('node_id')}, 状态={data.get('status')}, 耗时={data.get('elapsed_time', 0)}秒"
        )
        if data.get("outputs"):
    ...
    def on_workflow_finished(data):
        print(
            f"工作流完成: ID={data.get('id')}, 状态={data.get('status')}, 总耗时={data.get('elapsed_time', 0)}秒"
        )
        if data.get("outputs"):
...
def example_message_feedback():
    ...
    like_result = client.message_feedback(
        message_id=message_id, user=USER_ID, rating="like", content="这个解释非常清晰！"
    )
...
def example_get_suggested_questions():
    ...
    # 先发送一条消息
    response = client.send_message(
        query="什么是工作流编排？", user=USER_ID, response_mode="blocking"
    )
...
def example_rename_conversation():
    ...
    # 创建一个新会话
    response = client.send_message(
        query="请概述一下工作流编排的主要优势", user=USER_ID, response_mode="blocking"
    )
    ...
    result = client.rename_conversation(
        conversation_id=conversation_id, user=USER_ID, name="工作流编排优势讨论"
    )
...
def example_text_to_audio():
    ...
    # 从消息ID转语音
    response = client.send_message(
        query="工作流编排有哪些主要组件？", user=USER_ID, response_mode="blocking"
    )
