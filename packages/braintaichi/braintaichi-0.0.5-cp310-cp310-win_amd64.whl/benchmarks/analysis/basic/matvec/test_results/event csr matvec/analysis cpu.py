...
combined_csv_path = "event csr matvec/event_csrmv_cpu.csv"
...
avg_speedup_df = combined_df.groupby(['values type', 'events type', 'transpose', 'shape[0]', 'shape[1]']).mean().reset_index()

# Setting up the plots for heatmaps based on different backends, values types, and events types
plt.figure(figsize=(14, 20))  # Adjusting figure size for eight subplots

# Creating subplots for each combination of backend, values type, and events type
for i, (values_type, events_type, transpose) in enumerate([(v, e, t)  for v in avg_speedup_df['values type'].unique() for e in avg_speedup_df['events type'].unique() for t in avg_speedup_df['transpose'].unique()]):
    plt.subplot(4, 2, i + 1)

    # Filtering data and creating a pivot table for the heatmap
    filtered_df = avg_speedup_df[(avg_speedup_df['values type'] == values_type) & (avg_speedup_df['events type'] == events_type) & (avg_speedup_df['transpose'] == transpose)]
    heatmap_data = filtered_df.pivot(index="shape[0]", columns="shape[1]", values="speedup")

    # Creating the heatmap
    sns.heatmap(heatmap_data, annot=True, fmt=".2f", cmap='YlGnBu')
    plt.title(f'values type: {values_type}, events type: {events_type}, transpose: {transpose}')
    plt.xlabel('shape[0]')
    plt.ylabel('shape[1]')

# Adding an overall title with adjusted position
plt.suptitle('[CPU] event csr matvec taichi speedup over brainpylib\n'
             'speedup = (brainpylib time / taichi aot time) - 1', fontsize=16)

# Adjust layout to make room for the title
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.savefig('event csr matvec/event csrmv cpu.png')
# plt.show()
