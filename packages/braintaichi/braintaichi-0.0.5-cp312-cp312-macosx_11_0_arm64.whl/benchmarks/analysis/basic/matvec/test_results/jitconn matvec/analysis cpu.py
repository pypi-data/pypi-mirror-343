...
combined_csv_path = "jitconn matvec/jitconn_matvec_cpu.csv"
...
def calculate_speedup(df):
    # Calculate the mean of 'taichi aot time' and 'brainpy time' for each row
    ...
    # Calculate speedup, if 'brainpy time' is smaller than 'taichi aot time', speedup is negative
...
avg_speedup_df = combined_df.groupby(['type', 'transpose', 'outdim_parallel', 'shape[0]', 'shape[1]']).mean().reset_index()

# Setting up the plots for heatmaps based on different backends, values types, and events types
plt.figure(figsize=(22, 18))  # Adjusting figure size for eight subplots

# Creating subplots for each combination of backend, values type, and events type
for i, (_type, transpose, outdim_parallel) in enumerate([(_t, t, o)  for _t in avg_speedup_df['type'].unique() for t in avg_speedup_df['transpose'].unique() for o in avg_speedup_df['outdim_parallel'].unique()]):
    plt.subplot(3, 4, i + 1)

    # Filtering data and creating a pivot table for the heatmap
    filtered_df = avg_speedup_df[(avg_speedup_df['type'] == _type) & (avg_speedup_df['transpose'] == transpose) & (avg_speedup_df['outdim_parallel'] == outdim_parallel)]
    heatmap_data = filtered_df.pivot(index="shape[0]", columns="shape[1]", values="speedup")

    # Creating the heatmap
    sns.heatmap(heatmap_data, annot=True, fmt=".2f", cmap='YlGnBu')
    plt.title(f'type: {_type}, transpose: {transpose}, outdim_parallel: {outdim_parallel}')
    plt.xlabel('shape[0]')
    plt.ylabel('shape[1]')

# Adding an overall title with adjusted position
plt.suptitle('[CPU] jitconn matvec taichi speedup over brainpylib\n'
             'speedup = (brainpylib time / taichi aot time) - 1', fontsize=16)

# Adjust layout to make room for the title
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.savefig('jitconn matvec/jitconn_matvec_cpu.png')
# plt.show()
