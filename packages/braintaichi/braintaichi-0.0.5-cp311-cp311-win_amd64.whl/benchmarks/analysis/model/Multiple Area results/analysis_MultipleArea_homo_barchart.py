...
combined_df = pd.read_csv(os.path.join(PATH, 'benchmark_MultipleArea_gpu.csv'))
...
combined_df = combined_df[combined_df['syn_func_type'] == 'event_mv_prob_homo']
combined_df = combined_df[combined_df['conn_num'] == 80]

# print(combined_df)

plt.figure(figsize=(10, 6))  # 调整图形大小
sns.lineplot(x=combined_df['neuron_num'], y=combined_df['speedup'], marker='o', linestyle='-', color='darkorange', linewidth=2.5)
plt.xlabel('神经元个数', fontsize=16, fontproperties=font)
plt.ylabel('加速比', fontsize=16, fontproperties=font)
# plt.title('[CPU] Multiple Area network speedup over origin brainpy op', fontsize=18)
plt.grid(True)  # 显示网格
plt.savefig(os.path.join(PATH, 'benchmark_MultipleArea_linechart_gpu.png'))
# plt.show()
