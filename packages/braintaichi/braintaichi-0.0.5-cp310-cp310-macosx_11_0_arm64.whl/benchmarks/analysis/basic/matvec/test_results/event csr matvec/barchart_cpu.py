...
combined_df = pd.read_csv("event csr matvec/event_csrmv_cpu.csv")
...
plt.title('[CPU] event csr matvec taichi speedup over brainpylib' + '\n' +
          'speedup = (brainpylib time / taichi aot time) - 1')
plt.savefig('event csr matvec/event csrmv cpu dist.png')
