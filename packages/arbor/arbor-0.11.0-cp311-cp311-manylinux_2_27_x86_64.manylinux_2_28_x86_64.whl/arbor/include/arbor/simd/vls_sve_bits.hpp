...
static constexpr unsigned vls_sve_width = /64;

// Check required compiler features for VLS_SVE
#if (!defined(__ARM_FEATURE_SVE_BITS) || __ARM_FEATURE_SVE_BITS != )
#error "Vector length specific scalable vector extension (VLS_SVE) not enabled - consider setting ARB_SVE_LENGTH="
#endif
...
using fvuint32_t  = svuint32_t __attribute__((arm_sve_vector_bits()));
using fvuint64_t  = svuint64_t __attribute__((arm_sve_vector_bits()));
using fvint32_t   = svint32_t __attribute__((arm_sve_vector_bits()));
using fvint64_t   = svint64_t __attribute__((arm_sve_vector_bits()));
using fvfloat64_t = svfloat64_t __attribute__((arm_sve_vector_bits()));
using fvbool_t    = svbool_t __attribute__((arm_sve_vector_bits()));
