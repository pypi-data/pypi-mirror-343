"""
This module handles the import of dumpserver flows generated by old versions.

The flow file version is decoupled from the dumpserver release cycle (since
...
def migrate_flow(flow_data: Dict[Union[bytes, str], Any]) -> Dict[Union[bytes, str], Any]:
    while True:
        ...
        # Historically, we used the dumpserver minor version tuple as the flow format version.
        ...
        if flow_version == version.FLOW_FORMAT_VERSION:
            ...
            raise ValueError(
                "{} cannot read files with flow format version {}{}.".format(
                    version.MITMPROXY,
                    flow_version,
                    ", please update dumpserver" if should_upgrade else ""
                )
            )
    return flow_data
