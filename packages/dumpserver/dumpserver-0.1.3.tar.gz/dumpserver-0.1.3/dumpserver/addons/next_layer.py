...
For example, if dumpserver runs as a regular proxy, we first need to determine if
new clients start with a TLS handshake right away (Secure Web Proxy) or send a plaintext HTTP CONNECT request.
This addon here peeks at the incoming bytes and then makes a decision based on proxy mode, dumpserver options, etc.
...
In that case it's not necessary to modify dumpserver's source, adding a custom addon with a next_layer event hook
that sets nextlayer.layer works just as well.
"""
import re
from typing import Type, Sequence, Union, Tuple, Any, Iterable, Optional, List

from dumpserver import ctx, exceptions, connection
from dumpserver.net.tls import is_tls_record_magic
from dumpserver.proxy.layers.http import HTTPMode
from dumpserver.proxy import context, layer, layers
from dumpserver.proxy.layers import modes
from dumpserver.proxy.layers.tls import HTTP_ALPNS, parse_client_hello
