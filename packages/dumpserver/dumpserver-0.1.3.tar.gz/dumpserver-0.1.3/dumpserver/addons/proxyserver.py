...
from dumpserver import command, ctx, exceptions, flow, http, log, master, options, platform, tcp, websocket
from dumpserver.flow import Flow
from dumpserver.proxy import commands, events, server_hooks
from dumpserver.proxy import server
from dumpserver.proxy.layers.tcp import TcpMessageInjected
from dumpserver.proxy.layers.websocket import WebSocketMessageInjected
from dumpserver.utils import asyncio_utils, human
...
class Proxyserver:
    ...
    def load(self, loader):
        loader.add_option(
            ...
            "Determine when server connections should be established. When set to lazy, dumpserver "
            "tries to defer establishing an upstream connection as long as possible. This makes it possible to "
            "use server replay while being offline. When set to eager, dumpserver can detect protocols with "
        ...
        loader.add_option(
            ...
            Disabling this option makes dumpserver vulnerable to HTTP smuggling attacks.
            """,
        )
