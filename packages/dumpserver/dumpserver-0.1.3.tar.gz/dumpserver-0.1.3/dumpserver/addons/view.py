...
import dumpserver.flow
from dumpserver import command
from dumpserver import ctx
from dumpserver import exceptions
from dumpserver import hooks
from dumpserver import connection
from dumpserver import flowfilter
from dumpserver import http
from dumpserver import io
from dumpserver import tcp
from dumpserver.utils import human
...
class _OrderKey:
    ...
    def generate(self, f: dumpserver.flow.Flow) -> typing.Any:  # pragma: no cover
        pass
...
class OrderRequestStart(_OrderKey):
    def generate(self, f: dumpserver.flow.Flow) -> float:
        return f.timestamp_start


class OrderRequestMethod(_OrderKey):
    def generate(self, f: dumpserver.flow.Flow) -> str:
...
class OrderKeySize(_OrderKey):
    def generate(self, f: dumpserver.flow.Flow) -> int:
        if isinstance(f, http.HTTPFlow):
            size = 0
...
class View(collections.abc.Sequence):
    ...
    def _bisect(self, f: dumpserver.flow.Flow) -> int:
        v = self._view.bisect_right(f)
        return self._rev(v - 1) + 1

    def index(self, f: dumpserver.flow.Flow, start: int = 0, stop: typing.Optional[int] = None) -> int:
    ...
    def getvalue(self, flow: dumpserver.flow.Flow, key: str, default: str) -> str:
        """
    ...
    def setvalue_toggle(
        self,
        flows: typing.Sequence[dumpserver.flow.Flow],
        key: str
    ) -> None:
        """
    ...
    def duplicate(self, flows: typing.Sequence[dumpserver.flow.Flow]) -> None:
        """
    ...
    def remove(self, flows: typing.Sequence[dumpserver.flow.Flow]) -> None:
        """
    ...
    def resolve(self, flow_spec: str) -> typing.Sequence[dumpserver.flow.Flow]:
        """
    ...
    def load_file(self, path: dumpserver.types.Path) -> None:
        """
    ...
    def add(self, flows: typing.Sequence[dumpserver.flow.Flow]) -> None:
        """
    ...
    def get_by_id(self, flow_id: str) -> typing.Optional[dumpserver.flow.Flow]:
        """
    ...
    def update(self, flows: typing.Sequence[dumpserver.flow.Flow]) -> None:
        """
...
class Focus:
    ...
    def __init__(self, v: View) -> None:
        self.view = v
        self._flow: typing.Optional[dumpserver.flow.Flow] = None
    ...
    @property
    def flow(self) -> typing.Optional[dumpserver.flow.Flow]:
        return self._flow

    @flow.setter
    def flow(self, f: typing.Optional[dumpserver.flow.Flow]):
...
class Settings(collections.abc.Mapping):
    ...
    def __getitem__(self, f: dumpserver.flow.Flow) -> dict:
