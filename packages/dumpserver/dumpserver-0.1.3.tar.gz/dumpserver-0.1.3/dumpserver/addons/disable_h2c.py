...
class DisableH2C:
    ...
    def process_flow(self, f):
        if f.request.headers.get('upgrade', '') == 'h2c':
            dumpserver.ctx.log.warn("HTTP/2 cleartext connections (h2c upgrade requests) are currently not supported.")
        ...
        if is_connection_preface:
            f.kill()
            dumpserver.ctx.log.warn("Initiating HTTP/2 connections with prior knowledge are currently not supported.")
