...
`dumpserver.http.HTTPFlow.websocket` attribute set.
...
class WebSocketMessage(serializable.Serializable):
    ...
    Fragmented WebSocket messages are reassembled by dumpserver and then
    ...
    dumpserver stores all message contents as `bytes`. If you need a `str`, you can access the `text` property
    on text messages:
    ...
    dropped: bool
    """True if the message has not been forwarded by dumpserver, False otherwise."""
    injected: bool
...
class WebSocketData(stateobject.StateObject):
    ...
    This is typically accessed as `dumpserver.http.HTTPFlow.websocket`.
    """
