...
import dumpserver.types
from dumpserver import exceptions, command_lexer
from dumpserver.command_lexer import unquote
...
def typename(t: type) -> str:
    ...
    to = dumpserver.types.CommandTypes.get(t, None)
    if not to:
...
class Command:
    ...
    def __init__(self, manager: "CommandManager", name: str, func: typing.Callable) -> None:
        ...
        for name, parameter in self.signature.parameters.items():
            ...
            if not dumpserver.types.CommandTypes.get(parameter.annotation, None):
                raise exceptions.CommandError(f"Argument {name} has an unknown type {t} in {func}.")
        if self.return_type and not dumpserver.types.CommandTypes.get(self.return_type, None):
    ...
    def call(self, args: typing.Sequence[str]) -> typing.Any:
        ...
        if ret is None and self.return_type is None:
            return
        typ = dumpserver.types.CommandTypes.get(self.return_type)
        assert typ
...
class CommandManager:
    ...
    def parse_partial(
        ...
        next_params: typing.List[CommandParameter] = [
            CommandParameter("", dumpserver.types.Cmd),
            CommandParameter("", dumpserver.types.CmdArgs),
        ]
        ...
        for part in parts:
            ...
            if expected and expected.kind is inspect.Parameter.VAR_POSITIONAL:
                ...
                expected = CommandParameter("", dumpserver.types.Unknown)

            arg_is_known_command = (
                    expected.type == dumpserver.types.Cmd and part in self.commands
            )
            arg_is_unknown_command = (
                    expected.type == dumpserver.types.Cmd and part not in self.commands
            )
            command_args_following = (
                    next_params and next_params[0].type == dumpserver.types.CmdArgs
            )
            ...
            to = dumpserver.types.CommandTypes.get(expected.type, None)
            valid = False
            if to:
                try:
    ...
    def execute(self, cmdstr: str) -> typing.Any:
        ...
        command_name, *args = [
            unquote(part.value)
            for part in parts
            if part.type != dumpserver.types.Space
        ]
...
def parsearg(manager: CommandManager, spec: str, argtype: type) -> typing.Any:
    ...
    """
    t = dumpserver.types.CommandTypes.get(argtype, None)
    if not t:
...
def argument(name, type):
    """
        ...
        specific types such as dumpserver.types.Choice, which we cannot annotate
