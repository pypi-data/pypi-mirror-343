...
from dumpserver import http, options as moptions, tls
from dumpserver.proxy.context import Context
from dumpserver.proxy.layers.http import HTTPMode
from dumpserver.proxy import commands, events, layer, layers, server_hooks
from dumpserver.connection import Address, Client, Connection, ConnectionState
from dumpserver.utils import asyncio_utils
from dumpserver.utils import human
from dumpserver.utils.data import pkg_data
...
class ConnectionHandler(metaclass=abc.ABCMeta):
    ...
    def server_event(self, event: events.Event) -> None:
        ...
        try:
            ...
            self.log(f"dumpserver has crashed!\n{traceback.format_exc()}", level="error")
    ...
    async def handle(reader, writer):
        ...
        def tls_start_client(tls_start: tls.TlsData):
            ...
            ssl_context.use_privatekey_file(
                pkg_data.path("../test/dumpserver/data/verificationcerts/trusted-leaf.key")
            )
            ssl_context.use_certificate_chain_file(
                pkg_data.path("../test/dumpserver/data/verificationcerts/trusted-leaf.crt")
            )
