...
from dumpserver import flow, http
from dumpserver.connection import Connection, Server
from dumpserver.net import server_spec
from dumpserver.net.http import status_codes, url
from dumpserver.net.http.http1 import expected_http_body_size
from dumpserver.proxy import commands, events, layer, tunnel
from dumpserver.proxy.layers import tcp, tls, websocket
from dumpserver.proxy.layers.http import _upstream_proxy
from dumpserver.proxy.utils import expect
...
class HttpStream(layer.Layer):
    ...
    def check_body_size(self, request: bool) -> layer.CommandGenerator[bool]:
        ...
        if max_total_size is not None and expected_size > max_total_size:
            ...
            err_msg = f"{'Request' if request else 'Response'} body exceeds dumpserver's body_size_limit."
