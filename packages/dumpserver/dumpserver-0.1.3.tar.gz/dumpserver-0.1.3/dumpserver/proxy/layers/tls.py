...
from dumpserver.tls import ClientHello, ClientHelloData, TlsData
from dumpserver import certs, connection
from dumpserver.proxy import commands, events, layer, tunnel
from dumpserver.proxy import context
from dumpserver.proxy.commands import StartHook
from dumpserver.proxy.layers import tcp
...
class TlsStartClientHook(StartHook):
    """
    TLS negotation between dumpserver and a client is about to start.

    An addon is expected to initialize data.ssl_conn.
    (by default, this is done by `dumpserver.addons.tlsconfig`)
    """
    data: TlsData
...
class TlsStartServerHook(StartHook):
    """
    TLS negotation between dumpserver and a server is about to start.
    ...
    def receive_handshake_data(self, data: bytes) -> layer.CommandGenerator[Tuple[bool, Optional[str]]]:
        ...
        try:
            ...
            if last_err == ('SSL routines', 'tls_process_server_certificate', 'certificate verify failed'):
                ...
                err = (
                    f"The remote server and dumpserver cannot agree on a TLS version to use. "
                    f"You may need to adjust dumpserver's tls_version_server_min option."
                )
            else:
                err = f"OpenSSL {e!r}"
            return False, err
...
class ClientTLSLayer(_TLSLayer):
    ...
    def on_handshake_error(self, err: str) -> layer.CommandGenerator[None]:
        ...
        if err.startswith("Cannot parse ClientHello"):
            ...
            err = (
                f"Client and dumpserver cannot agree on a TLS version to use. "
                f"You may need to adjust dumpserver's tls_version_client_min option."
            )
