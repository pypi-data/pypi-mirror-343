...
from dumpserver import contentviews, ctx, flow, flowfilter, http
from dumpserver.contentviews import base
from dumpserver.net.encoding import decode
...
class ProtoParser:
    ...
    class Field:
        ...
        As dumpserver sees the data passing by as raw protobuf message, it only knows the
...
#       This function is generic enough, to consider moving it to dumpserver.contentviews.base
def format_table(
...
# hacky fix for dumpserver issue:
#
# dumpserver handles Exceptions in the contenview's __call__ function, by
...
# with invalid data.
# When the flow data gets invalidated in the edit mode, dumpserver re-calls
# the generator functions outside the contentviews '__call__' method.
#
# This happens in the 'safe_to_print' function of 'dumpserver/contentvies/__init__.py'
#
...
#        yield clean_line
#
# In result, dumpserver crashes if the generator functions raise Exception to indicate
# data parsing errors.
