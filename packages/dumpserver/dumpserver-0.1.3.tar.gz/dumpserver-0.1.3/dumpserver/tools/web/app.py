...
import dumpserver.tools.web.master
from dumpserver import certs, command, contentviews
...
from dumpserver import version
from dumpserver.http import HTTPFlow
from dumpserver.tcp import TCPFlow, TCPMessage
from dumpserver.utils.emoji import emoji
from dumpserver.utils.strutils import always_str
from dumpserver.websocket import WebSocketMessage
...
def flow_to_json(flow: dumpserver.flow.Flow) -> dict:
    """
...
class RequestHandler(tornado.web.RequestHandler):
    ...
    @property
    def view(self) -> "dumpserver.addons.view.View":
        return self.application.master.view

    @property
    def master(self) -> "dumpserver.tools.web.master.WebMaster":
        return self.application.master

    @property
    def flow(self) -> dumpserver.flow.Flow:
...
class FlowHandler(RequestHandler):
    ...
    def put(self, flow_id):
        flow: dumpserver.flow.Flow = self.flow
        flow.backup()
        try:
            for a, b in self.json.items():
                if a == "request" and hasattr(flow, "request"):
                    request: dumpserver.http.Request = flow.request
                    for k, v in b.items():
                        if k in ["method", "scheme", "host", "path", "http_version"]:
                            ...
                            if request.trailers is not None:
                                ...
                                request.trailers = dumpserver.http.Headers()
                            for trailer in v:
                    ...
                    response: dumpserver.http.Response = flow.response
                    for k, v in b.items():
                        if k in ["msg", "http_version"]:
                            ...
                            if response.trailers is not None:
                                ...
                                response.trailers = dumpserver.http.Headers()
                            for trailer in v:
...
class Application(tornado.web.Application):
    master: "dumpserver.tools.web.master.WebMaster"

    def __init__(self, master: "dumpserver.tools.web.master.WebMaster", debug: bool) -> None:
        self.master = master
        super().__init__(
