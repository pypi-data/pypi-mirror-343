...
from dumpserver import exceptions, master
from dumpserver import options
from dumpserver import optmanager
from dumpserver.tools import cmdline
from dumpserver.utils import debug, arg_check
...
def assert_utf8_env():
    ...
    if "utf" not in spec:
        print(
            "Error: dumpserver requires a UTF console environment.",
            file=sys.stderr
        )
        print(
...
def run(
    ...
    async def main() -> T:
        ...
        if "-R" in sys.argv and sys.argv[sys.argv.index("-R") + 1].startswith("http"):
            print("To use dumpserver in reverse mode please use --mode reverse:SPEC instead")
...
def mitmproxy(args=None) -> typing.Optional[int]:  # pragma: no cover
    if os.name == "nt":
        ...
        assert_utf8_env()
    from dumpserver.tools import console
    run(console.master.ConsoleMaster, cmdline.mitmproxy, args)
    return None


def mitmdump(args=None) -> typing.Optional[int]:  # pragma: no cover
    from dumpserver.tools import dump
...
def mitmweb(args=None) -> typing.Optional[int]:  # pragma: no cover
    from dumpserver.tools import web
