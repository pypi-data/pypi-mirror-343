...
class CompletionState(typing.NamedTuple):
    completer: Completer
    parsed: typing.Sequence[dumpserver.command.ParseResult]


class CommandBuffer:
    def __init__(self, master: dumpserver.master.Master, start: str = "") -> None:
        self.master = master
        self.text = start
    ...
    def render(self):
        ...
        if not parts:
            ...
            for p in parts:
                if p.valid:
                    if p.type == dumpserver.types.Cmd:
            ...
            if remaining:
                if parts[-1].type != dumpserver.types.Space:
                    ret.append(("text", " "))
    ...
    def cycle_completion(self, forward: bool = True) -> None:
        if not self.completion:
            ...
            if parts and parts[-1].type != dumpserver.types.Space:
                ...
                return
            ct = dumpserver.types.CommandTypes.get(type_to_complete, None)
            if ct:
...
class CommandEdit(urwid.WidgetWrap):
    ...
    def __init__(self, master: dumpserver.master.Master, text: str) -> None:
