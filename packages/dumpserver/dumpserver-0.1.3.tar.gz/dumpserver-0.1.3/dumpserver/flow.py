...
from dumpserver import stateobject
...
class Error(stateobject.StateObject):
    ...
    which is represented by a normal `dumpserver.http.Response` object. This class is
...
class Flow(stateobject.StateObject):
    ...
    See also:
     - dumpserver.http.HTTPFlow
     - dumpserver.tcp.TCPFlow
    """
    client_conn: connection.Client
    """The client that connected to dumpserver."""

    server_conn: connection.Server
    """
    The server dumpserver connected to.

    Some flows may never cause dumpserver to initiate a server connection,
    for example because their response is replayed by dumpserver itself.
    ...
    intercepted: bool
    """
    If `True`, the flow is currently paused by dumpserver.
    ...
    This attribute indicates if this flow has been replayed in either direction.
     ...
     - a value of `request` indicates that the request has been artifically replayed by dumpserver to the server.
    ...
    def timestamp_start(self) -> float:
        ...
        *Read-only:* Start time of the flow.
        ...
        `dumpserver.connection.Client.timestamp_start` or `dumpserver.http.Request.timestamp_start`.
        """
