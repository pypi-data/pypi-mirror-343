...
class EthereumServiceProviderConnector(ServiceProviderConnector):
    ...
    def __init__(
        ...
        self.connectors[Chain.ethereum_sepolia] = sep_provider_list
                ...
                # Configure Arbitrum Sepolia testnet connectors
        arb_sep_provider_list = []
        if hasattr(app_config, 'arbitrum_sepolia_rpc_url'):
            print("i am here")
            self.arbitrum_sepolia_rpc_url = app_config.arbitrum_sepolia_rpc_url
            arb_sep_provider_list.append(EthereumRPCProvider(self.arbitrum_sepolia_rpc_url))
        # You can add Etherscan-like broadcaster here if Arbitrum Sepolia is supported
        sep_provider_list.append(EtherscanBroadcaster('https://api-sepolia.arbiscan.io/api', etherscan_api_token))
        self.connectors[Chain.arbitrum_sepolia] = arb_sep_provider_list
