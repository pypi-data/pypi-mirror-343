...
import { clientIsConnected } from '../jovyanClient';
...
export class JovyanCellController {
  ...
  public async createCodeStream(prompt: string): Promise<AsyncIterable<string> | null> {
    if (!clientIsConnected()) {
      console.debug('Jovyan client not connected. Prompting for auth.');
      const userCancelled = await showAuthReminderDialog(this._notebookController.settingRegistry);
      if (userCancelled) {
