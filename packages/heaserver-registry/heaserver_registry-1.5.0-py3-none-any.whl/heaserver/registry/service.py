...
from heaobject.root import Share, ShareImpl, Permission, DesktopObjectDict, DefaultPermissionGroup
...
async def get_all_collections(request: web.Request) -> web.Response:
    ...
    async for obj in mongoservicelib.get_all_desktop_objects_gen(request, MONGODB_COMPONENT_COLLECTION, type_=Component):
        for resource in obj.resources:
            ...
            for group in resource.collection_accessor_groups:
                ...
                share__.group = group
                share__.permissions = [Permission.VIEWER, Permission.CHECK_DYNAMIC]
            ...
            if await coll.has_permissions(DefaultPermissionGroup.ACCESSOR_PERMS, context):
...
async def _get_collection(request: web.Request) -> web.Response:
    ...
    async for obj in mongoservicelib.get_all_desktop_objects_gen(request, MONGODB_COMPONENT_COLLECTION,
        ...
        for resource in (r for r in obj.resources if r.resource_type_name == request.match_info['id']):
            ...
            if await c.has_permissions(DefaultPermissionGroup.ACCESSOR_PERMS, context):
