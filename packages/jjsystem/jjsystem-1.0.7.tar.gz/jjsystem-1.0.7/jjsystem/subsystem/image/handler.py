...
from jjsystem.subsystem.image.resource import Image as ImageJJSystem, \
    QualityImage
    ...
    def verify_size_resolution_image(folder, filename):
        ...
        try:
            ...
            with Image.open(image_path) as img:
                if img.height > ImageJJSystem.MAX_ALLOWED_SIZE[0] or \
                   img.width > ImageJJSystem.MAX_ALLOWED_SIZE[1]:
                    messageError = 'Image must be less than {}px and less\
                        than {}px high!'.format(
                            ImageJJSystem.MAX_ALLOWED_SIZE[0],
                            ImageJJSystem.MAX_ALLOWED_SIZE[1])
        except Exception:
