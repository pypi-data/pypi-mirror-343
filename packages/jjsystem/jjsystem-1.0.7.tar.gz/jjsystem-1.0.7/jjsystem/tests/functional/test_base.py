...
class JJSystemTest(object):

    def setUp(self):
        super(JJSystemTest, self).setUp()
        app = jjsystem.SystemFlask()
        app.config['TESTING'] = True
        self.app = app.test_client()

        response = self.app.post(
            '/tokens',
            data=json.dumps({'domain_name': 'default',
    ...
    def check_resource(self, resource, ref=None):
        ...
        if ref:
            ...
            attrs = (
                attr for attr in self.required_attributes if attr not in self.hidden_attributes)
            for attr in attrs:
...
class RetrieveTest(JJSystemTest):
