...
from hestia_earth.validation.utils import _filter_list_errors, get_term_lookup_value, is_permanent_crop
...
def validate_no_tillage(practices: list):
    ...
    operation_tillage_practices = [
        (index, practice)
        for index, practice in enumerate(practices) if _practice_is_tillage(practice)
    ]
    ...
    return _filter_list_errors([
        ...
        } for index, p in operation_tillage_practices
    ]) if no_value == 100 else (
        {
            'level': 'error',
            'dataPath': '.practices',
            'message': 'must contain a tillage practice',
            'params': {
                'term': operation_tillage_practices[0][1].get('term')
            }
        } if all([
            len(operation_tillage_practices) > 0,
            len(tillage_practices) == 0,
        ]) else True
    )
