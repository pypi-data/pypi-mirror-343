...
class StateMachine:
    ...
    def Execute(self, context : nuclio_sdk.Context, event : nuclio_sdk.Event):
        ...
        return nuclio_sdk.Response(event)
        print(event)
        print(event.body)
        prev_state = self.current_state
        state = self.states[self.current_state]
        try:
            self.mts_state.UpdateMtsState(self.mts_state, event)
            self.current_state = state.Action(context, self.mts_state)
            self.context.logger.info(f"State {prev_state} executed")
            result = self.mts_state

            return nuclio_sdk.Response(result)
        except Exception as e:
            context.logger.error_with(f"State {prev_state} failed with exception {e}")
            return nuclio_sdk.Response(f"State {prev_state} failed with exception {e}")