...
struct ExponentialDist {
  ...
  ExponentialDist(const double lam=1.0,const double mx=1.0)
...
public:
  ...
  const SimResult run(const int seed) {
    rng_.seed(seed);
    const int N = node_count(), M = activity_count();
    ...
    for(auto i=0;i<M;++i) {
      ...
      if(di>=0) {
        ...
        if(extra>0.0) { extended_durations_[i] = delta + extra;}
      }
    }
    // init propagate
    std::copy(earliest_allowed_.begin(), earliest_allowed_.end(), realized_ts_.begin());
    std::fill(cause_.begin(), cause_.end(), -1);

    // actual propagation
    for(auto n_index = 0; n_index<N; ++n_index) {
...
PYBIND11_MODULE(_core,m){
  ...
  py::class_<SimResult>(m,"SimResult")
    ...
    .def_readonly("durations",      &SimResult::durations,      "Per-link durations (incl. extra)")
