...
PYBIND11_MODULE(_core, m) {
    ...
    py::class_<SimResult>(m, "SimResult")
        ...
        .def_property_readonly("cause_event", [](const SimResult &r) {
            // use 64-bit ints for events
            return py::array_t<long long>(
                { r.cause_event.size() },
                { sizeof(long long) },
                reinterpret_cast<const long long*>(r.cause_event.data())
            );
        })
        ;
