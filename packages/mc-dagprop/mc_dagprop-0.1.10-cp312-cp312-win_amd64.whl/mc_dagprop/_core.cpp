...
PYBIND11_MODULE(_core, m) {
    ...
    py::class_<SimResult>(m, "SimResult")
        ...
        .def_property_readonly("cause_event", [](const SimResult &r) {
            // we'll expose this as 64?bit ints
            py::array_t<long long> arr(r.cause_event.size());
            auto *buf = arr.mutable_data();
            std::memcpy(buf,
                        r.cause_event.data(),
                        sizeof(int) * r.cause_event.size()); // OK since sizeof(int)==4 or 8
            return arr;
        })
        ;
