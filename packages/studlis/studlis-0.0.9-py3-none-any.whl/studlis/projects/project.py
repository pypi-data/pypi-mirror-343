...
#
    ...
    async def update_validation_async(self,data,session_data):
        ...
        if user_validation:
            ...
            update_query = "UPDATE validation SET data = ?, confidence = ?,ts=?,comment=?,validation_state=1"
            update_query += " WHERE study_id = ? AND validator = ?"
            await self.async_db.execute(update_query, update_values)
        else:
            # Insert a new validation
            update_query = "INSERT INTO validation (data,confidence,ts,comment,study_id,validator,validation_state) VALUES (?,?,?,?,?,?,1)"
            await self.async_db.execute(update_query, update_values)
