...
class RpcMethod(Generic[T]):
    ...
    def __call__(self, request: Message, **kwargs) -> T:
...
class RpcStreamingMethod(Generic[T]):
    ...
    def __call__(self, request: Message, **kwargs) -> Generator[T, None, None]:
        return self.method(request, **kwargs)
...
def coerce_rpc_error(
    ...
    if isinstance(e, grpc.Call):
        ...
        if exceptions_for_status is not None and status in exceptions_for_status:
            ...
            return BeakerServerUnavaibleError(e.details())
    return e
...
class ServiceClient:
    ...
    def http_request(
        ...
        def make_request(session: requests.Session) -> requests.Response:
            ...
            request_data: str | bytes | io.BufferedReader | None = None
    ...
    def rpc_request(
        ...
        try:
            return method(request, metadata=self._rpc_call_metadata)
    ...
    def rpc_paged_request(
        ...
        try:
            response = method(request, metadata=self._rpc_call_metadata)
            yield response
    ...
    def rpc_streaming_request(
        ...
        try:
            yield from method(request, metadata=self._rpc_call_metadata)
        except RpcError as e:
            raise coerce_rpc_error(e, exceptions_for_status)
    ...
    def resolve_experiment_id(self, experiment: str | pb2.Experiment) -> str:
    ...
    def resolve_workload_id(self, workload: str | pb2.Workload) -> str:
        if isinstance(workload, pb2.Workload):
            return workload.experiment.id or workload.environment.id
    ...
    def _log_and_wait(self, retries_so_far: int, err: Exception) -> None:
        ...
        self.logger.debug("Request failed with: %s\nRetrying in %d seconds...", err, retry_in)
        time.sleep(retry_in)
