...
class WorkloadClient(ServiceClient):
    ...
    def is_experiment(self, workload: pb2.Workload) -> bool:
        return workload.experiment.ByteSize() > 0

    def is_environment(self, workload: pb2.Workload) -> bool:
        return workload.environment.ByteSize() > 0
    ...
    def _resolve_task(
        ...
        if task is None:
            if not workload.experiment.tasks:
                return None
            task = workload.experiment.tasks[0]
        else:
            for other_task in workload.experiment.tasks:
                if other_task.id == task.id:
                    break
                else:
                    raise ValueError("invalid task, task is not part of workload")
        return task
