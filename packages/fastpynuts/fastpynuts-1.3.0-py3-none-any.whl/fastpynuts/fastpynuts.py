...
class NUTSfinder:
    ...
    **Note**: Points-in-polyon tests via `shapely` may suffer from floating-point precision issues for points on the boundary of regions.
    ...
    def from_web(cls, scale=1, year=2021, epsg=4326, level=None, datadir=".data", force_reload=False, temporary=False, **kwargs):
        ...
        ----------
        scale : int, optional
            The level of detail/resolution of the NUTS data, one of {1, 3, 10, 20, 60}, by default 1
        year : int, optional
            Which year's data to download, one of {2024, 2021, 2016, 2013, 2010, 2006, 2003}, by default 2021
        epsg : int, optional
            CRS of data, one of {4326, 3035, 3857}, by default 4326
        level : int, optional
            Which level, one of {None, 0, 1, 2, 3}, default None
            If level is specified, `min_level` and `max_level` passed to `kwargs` are ignored.
        datadir : str, optional
            _description_, by default ".data"
        force_reload : bool, optional
            _description_, by default False
        temporary : bool, optional
            Save NUTS data to temporary file
