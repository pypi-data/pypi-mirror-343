...
class ApiClient:
    ...
    async def score_vacancy(self, user_requirements: str, vacancy_text: str) -> RelevanceOutput:
    ...
    async def send_telegram_message(self, user_id: int, message: str, 
                                  parse_mode: str = "HTML", 
                                  disable_web_page_preview: bool = False) -> ResponseMessage:
        ...
        try:
            async with self.session.post(url, json=notification.model_dump()) as response:
                return await self._process_model_response(response, ResponseMessage)
        except ClientError as e:
            raise ConnectionError(f"Connection error: {str(e)}")
    ...
    async def get_notification(self, user_id: int, job_id: str) -> Optional[Notification]:
        ...
        url = f"{self.base_url}/notifications/{user_id}/{job_id}"
        try:
            async with self.session.get(url) as response:
                if response.status == 404:
                    return None
    ...
    async def save_notification(self, notification: Notification) -> ResponseMessage:
        ...
        url = f"{self.base_url}/notifications/save"
        try:
