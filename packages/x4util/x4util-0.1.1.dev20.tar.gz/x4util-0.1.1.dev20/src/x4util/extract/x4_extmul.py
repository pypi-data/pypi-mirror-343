...
from x4util.conversion import x4_x4toj4, x4_poipoi, x4_j4tox4

def main():
  args=get_args(ver)
  extract_dataset(*get_input(args))


def extract_dataset(file_inp,file_dict,data_id,file_out,force0):
...
# Conversion to J4 format by X4TOJ4
  ...
  x4_x4toj4.main(file_inp,file_dict,file_js0,force,chkrid,keykeep,keepflg,outstr)
...
# Conversion to J4 format without pointer structure by POIPOI
  if data_id=="all":
    ...
    os.mkdir(dir_js1)
    x4_poipoi.main(file_js0,file_dict,data_id,dir_js1,force,keykeep,delpoin,keep001)
  else:
    file_js1 = "exfor1.json"
    x4_poipoi.main(file_js0,file_dict,data_id,file_js1,force,keykeep,delpoin,keep001)
...
def get_args(ver):
  ...
  parser=argparse.ArgumentParser(\
   ...
   epilog="example: x4_extmul.py -i exfor.txt -d dict_9130.json -e R0046.002.1 -o exfor_out.txt")
...
def get_input(args):
  ...
  if file_inp is None:
    file_inp=input("EXFOR input file [exfor.txt] -----------> ")
    if file_inp=="":
      file_inp="exfor.txt"
  ...
  if data_id is None:
    data_id=input("EXFOR dataset ID [R0046.002.1] ---------> ")
    if data_id=="":
      data_id="R0046.002.1"
  ...
  if file_out is None:
    file_out=input("EXFOR output file [exfor_out.txt] ------> ")
  if file_out=="":
