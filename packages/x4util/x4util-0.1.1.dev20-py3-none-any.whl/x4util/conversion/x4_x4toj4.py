...
def main():
  args=get_args(ver)
  convert_to_json(*get_input(args))

def convert_to_json(file_x4,file_dict,file_j4,force0,chkrid0,keykeep0,keepflg0,outstr0):
  global x4_json, dict_json
  global force, chkrid, keykeep, keepflg, outstr
  ...
  if outstr==True:
    x4_json={
     "name"           : "J4 - EXFOR in JSON (number as string)",
     "time_stamp"     : time_out
    }
  else:
    x4_json={
     "name"           : "J4 - EXFOR in JSON",
     "time_stamp"     : time_out
    }
...
def get_args(ver):
  ...
  parser=argparse.ArgumentParser(
   ...
   epilog="example: x4_x4toj4.py -i exfor.txt -d dict_9130.json -o exfor.json")
  ...
  parser.add_argument("-k", "--keykeep",
   help="keywords to be kept", default="all", nargs="+")
  parser.add_argument("-g", "--keepflg",
   help="keep flag at column 80", action="store_true")
  ...
  parser.add_argument("-o", "--file_j4",
   help="name of output EXFOR JSON file")
...
def get_input(args):
  ...
  force0=args.force
  chkrid0=args.chkrid
  keykeep0=args.keykeep
  keepflg0=args.keepflg
  outstr0=args.outstr

  file_x4=args.file_x4
  if file_x4 is None:
    file_x4=input("EXFOR file [exfor.txt] ------------------> ")
    if file_x4=="":
      file_x4="exfor.txt"
  ...
  if file_j4 is None:
    file_j4=input("EXFOR JSON file [exfor.json] ------------> ")
  if file_j4=="":
    file_j4="exfor.json"
  ...
  return file_x4,file_dict,file_j4,force0,chkrid0,keykeep0,keepflg0,outstr0
...
def read_system_identifier(lines):
  sys_id="ENDENTRY"
  entry="00000"
  ansan="00000.000"
  nentry=-1
...
#      }
      ...
      for keyword in bib_text:
        if keykeep!="all":
          if keyword not in keykeep and keyword!="REACTION":
            continue
  ...
  if keyword=="ANG-SEC" or \
    ...
    for index, item in enumerate(codes):
      ...
      if keyword=="ADD-RES":
        ...
        if not re.compile("^\d+\.$").search(item):
          msg="Noninteger coded under FLAG: "+item
          col1=line.find(item)+1
          col2=col1+len(item)-1
          print_error_3(ansan,keyword,msg,line,col1,col2,11)
...
def anal_decaydata(ansan,keyword,line,lines,codes):
  ...
  if len(codes)>2: # radiation fields exist
    for i in range(len(codes)//3):
      ...
      radiation["radiation_type"]=codes[j]
...
def anal_levelprop(ansan,keyword,line,codes,code_str):
  ...
  if len(codes)<2 or len(codes)>4:
    ...
    for i, item in enumerate(codes):
      if i>0:
        if re.compile("^E-LVL=|^E-EXC=|^LVL-NUMB=|^IAS-NUMB=").search(item):
          arr=item.split("=")
          ...
          level_property["value"]=str2float(arr[1],line)
          level_properties.append(level_property)
...
def check_date(ansan,keyword,date,line):
  ...
  if keyword=="TRANS"   or keyword=="MASTER" or\
   ...
   if len(char)!=8:
     msg="6 or 8 digits date are expected: "+str(date)

  else:
    if len(char)!=2 and len(char)!=4 and len(char)!=6 and len(char)!=8:
      msg="2, 4, 6 or 8 digits date are expected: "+str(date)
