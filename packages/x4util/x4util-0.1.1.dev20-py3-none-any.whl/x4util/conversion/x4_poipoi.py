...
def main():
  args=get_args(ver)
  remove_pointers(*get_input(args))

def remove_pointers(file_inp,file_dict,data_id,file_out,force0,keykeep0,delpoin0,keep0010):
  global x4_json, dict_json
  global force, keykeep, delpoin, keep001
      ...
      if "SUBENT" in subentry:
        ...
        for reaction in x4_json["subentries"][nsubent]["REACTION"]:
          if data_id=="all":
            ...
            poipoi(nsubent,pointer,file_out)
...
def poipoi(nsubent,pointer,file_out):

  if keep001==True:
    name="J4 - EXFOR in JSON without pointer with common subentry"
  else:
    name="J4 - EXFOR in JSON without pointer without common subentry"
...
# BIB section of condensed JSON file when preservation of 001 choosen
  if keep001==True:
    ...
    bib_condensed=dict() 
    bib_condensed=condense_bib(nsubents,pointer)
...
def condense_bib(nsubents,pointer):
...
def get_args(ver):
  ...
  parser=argparse.ArgumentParser(\
   ...
   epilog="example: x4_poipoi.py -i exfor.json -d dict_9130.json -e 22742.004.1 -o exfor_poi.json")
...
def get_input(args):
  ...
  if file_inp is None:
    file_inp=input("Input JSON EXFOR file [exfor.json] ------> ")
    if file_inp=="":
      file_inp="exfor.json"
  if not os.path.exists(file_inp):
    print(" ** File "+file_inp+" does not exist.")
  while not os.path.exists(file_inp):
    file_inp=input("Input EXFOR file in JSON [exfor.json] ------> ")
    if file_inp=="":
      file_inp="exfor.json"
    if not os.path.exists(file_inp):
      print(" ** File "+file_inp+" does not exist.")
  ...
  if data_id=="all":
    if file_out is None:
      file_out=input("Output JSON EXFOR file directory [json] -> ")
    if file_out=="":
      file_out="json"
    ...
    if file_out is None:
      file_out=input("Output JSON EXFOR file [exfor_poi.json] -> ")
    if file_out=="":
  ...
  return file_inp,file_dict,data_id,file_out,force0,keykeep0,delpoin0,keep0010
...
def read_x4json(file_inp):
  ...
  if x4_json["name"]!="J4 - EXFOR in JSON" and\
     x4_json["name"]!="J4 - EXFOR in JSON (number as string)" :
