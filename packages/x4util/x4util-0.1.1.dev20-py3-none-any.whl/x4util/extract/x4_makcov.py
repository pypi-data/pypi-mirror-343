...
def main():
  args=get_args(ver)
  to_covariance(*get_input(args))

def to_covariance(file_j4,file_hed,file_dict,data_id,file_tab,file_log,x_min,x_max,force0,shape0,outfrc0):
  ...
  if x4_json_full["name"]!="J4 - EXFOR in JSON without pointer without common subentry":
    msg="This is a JSON EXFOR file processed by POIPOI with option -1."
    print_error_fatal(msg,x4_json_full["name"])
...
# get base unit
  ...
  (dymin,dymax)=print_table(file_tab,an,san,pointer,x,dx,y,dy,cors,x_uni,y_uni,author,year,institute,shape,outfrc)
...
def print_table(file_tab,an,san,pointer,x,dx,y,dy,cors,x_uni,y_uni,author,year,institute,shape,outfrc):
  dymin=[100]*len(cors)
  dymax=[0]*len(cors)
...
def read_x4json(file_j4):
  ...
  if not re.compile("^J4 - EXFOR in JSON").search(x4_json["name"]):
...
def read_dict(file_dict):
  ...
  if dict_json["name"]!="EXFOR/CINDA Dictionary in JSON":
...
def get_args(ver):
  ...
  parser=argparse.ArgumentParser(\
   ...
   epilog="example: x4_makcov.py -i exfor.json -j exfor_hed.txt -d dict_9130.json -e 22742.004.1 -o exfor_tab.txt")
...
def get_input(args):
  ...
  if file_j4 is None:
    file_j4=input("JSON EXFOR file [exfor.json] ------------> ")
    if file_j4=="":
      file_j4="exfor.json"
  ...
  file_tab=args.file_tab
  if file_tab is None:
    file_tab=input("EXFOR table file [exfor_tab.txt] --------> ")
  if file_tab=="":
    file_tab="exfor_tab.txt"
  if os.path.isfile(file_tab):
    msg="File '"+file_tab+"' exists and must be overwritten."
    print_error(msg,"",force0)

  file_log=args.file_log
  if file_log is None:
    file_log=input("EXFOR table file [exfor_tab.log] --------> ")
  if file_log=="":
    file_log="exfor_tab.log"
  ...
  return file_j4,file_hed,file_dict,data_id,file_tab,file_log,x_min,x_max,force0,shape0,outfrc0
