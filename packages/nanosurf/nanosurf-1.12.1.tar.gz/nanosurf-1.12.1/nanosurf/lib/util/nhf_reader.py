...
class _CalNode():
    def __init__(self, uuid:str, sink_name:str, unit:str, map_type:_CalMapType, map_par:list[float], right_dir:bool=True, source_name:str= None):
        self.uuid = uuid
...
class NHFDataset():
    ...
    def _need_conversion_to_physical_unit(self, target_signal_id:str=None) -> bool:
        """ Returns True if data conversion to target signal is not yet done. If target is omitted default signal is unsed. 
        """
        if target_signal_id is None:
            target_signal_id = self.get_default_signal()

        if extract_unit(self.attribute['signal_unit']) and extract_unit(self.attribute['signal_calibration_unit']) == 'int':
            return False
        else:
    ...
    def _find_calibration_node_by_uuid(self, uuid:str) -> _CalNode:
        nodes = self.parent_segment._file_hdl.cal_group.calibration_nodes
        for node in nodes:
            if node.uuid == uuid:
                return node
        return None
    
    def _find_calibration_node_by_sink_id(self, sink_id:str) -> _CalNode:
        nodes = self.parent_segment._file_hdl.cal_group.calibration_nodes
        for node in nodes:
            if node.sink_name == sink_id:
                return node
        return None
    
    def _find_calibration_node_by_unit(self, sink_unit) -> _CalNode:
        sink_unit = extract_unit(sink_unit)
        nodes = self.parent_segment._file_hdl.cal_group.calibration_nodes
        for node in nodes:
            if extract_unit(node.unit) == sink_unit:
                return node
        return None
    ...
    def signals(self) -> set[str]:
        ...
        if "signal_calibration_source" in self.attribute:
            cal_node = self._find_calibration_node_by_uuid(self.attribute["signal_calibration_source"])
            cal_targets.add(cal_node.sink_name)
            ...
            while (attr_node_name:=f"signal_calibration_source_{index}") in self.attribute:
                cal_node = self._find_calibration_node_by_uuid(self.attribute[attr_node_name])
                cal_targets.add(cal_node.sink_name)
                index += 1
        elif "signal_selected" in self.attribute:
            cal_targets.add(self.attribute["signal_selected"])
        elif "signal_id" in self.attribute:
            cal_targets.add(self.attribute["signal_id"])
        return cal_targets
    ...
    def units(self) -> set[str]:
        ...
        if "signal_calibration_source" in self.attribute:
            cal_node = self._find_calibration_node_by_uuid(self.attribute["signal_calibration_source"])
            cal_targets_units.add(extract_unit(cal_node.unit))
        elif "signal_calibration_source_0" in self.attribute:
            index = 0
            while (attr_node_name:=f"signal_calibration_source_{index}") in self.attribute:
                cal_node = self._find_calibration_node_by_uuid(self.attribute[attr_node_name])
            ...
            cal_targets_units.add(extract_unit(self.attribute["signal_unit"]))
        elif "signal_id" in self.attribute:
            cal_targets_units.add(extract_unit(self.attribute["signal_unit"]))
        return cal_targets_units

    def get_signal_from_unit(self, unit:str) -> str:
        node = self._find_calibration_node_by_unit(extract_unit(unit))
        if node is not None:
            return node.sink_name
        return ""
...
class NHFSegment():
    ...
    def read_channel(self, ch:typing.Union[str, int], as_matrix:bool=False, with_signal:str=None, with_unit:str=None) -> NHFDataset:
        ...
        if (with_unit is not None) and self._file_hdl.version() < (2,0):
            raise ValueError("Parameter 'with_unit' is not supported by file version below 2.0")
...
class NHFCalibrationGroup():
    ...
    def build_calibration_graph(self, from__id_name:str, target_id_name:str) -> list[int]:
        calibration_graph:list[int] = []
        
        node_source_name = from__id_name
        exclude_sink_name = ""
        while (node_index:=self.find_calibration_node_by_name(node_source_name, exclude_sink_name)) >= 0:
            calib_node = self.calibration_nodes[node_index]
            calibration_graph.append(node_index)
            if calib_node.sink_name == target_id_name:
                return calibration_graph        
            node_source_name = calib_node.sink_name
            exclude_sink_name = calib_node.source_name
        return []

    def find_calibration_node_by_name(self, source_name:str, exclude_sink_name:str) -> int:
        for node_index, node in enumerate(self.calibration_nodes):
            if (node.source_name == source_name) and (node.sink_name != exclude_sink_name):
                    return node_index
        return -1
            ...
            if signal_invertibility in ["left", "two_way"]:
                self.calibration_nodes.append(
                    _CalNode(
                        ...
                        map_type = calib_map_type,
                        map_par = signal_mapping_parameters,
                        right_dir = signal_invertibility == "left",
