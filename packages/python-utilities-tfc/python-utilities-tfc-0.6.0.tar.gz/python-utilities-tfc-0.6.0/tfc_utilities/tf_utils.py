...
def lock_workspace(workspace_id, token):
    """
...
def unlock_workspace(workspace_id, token):
    """
...
def push_state_to_terraform_cloud(state_path, workspace_id, token):
    """
...
def download_workspace_state(org_name, workspace_name, token, output_path="downloaded.tfstate"):
    """
    ...
    workspace_id = get_workspace_id(org_name, workspace_name, token)
...
def get_workspace_id(org_name, workspace_name, token):
    """
    ...
    while True:
        response = requests.get(f"{base_url}?page[number]={page}", headers=headers)
        if response.status_code != 200:
...
def get_variable_id(api_token, organization, workspace_id, target_variable_name,url="https://app.terraform.io"):
    """
...
def update_variable(api_token, organization, workspace_id, variable_id, new_value,url="https://app.terraform.io"):
    """
...
def get_workspace_runs(api_token, workspace_id,url="https://app.terraform.io"):
    """
    Retrieve the list of runs for a specific Terraform Cloud workspace.
    ...
    Returns:
        list: A list of runs associated with the specified workspace, if the request is successful.
        None: If the request fails, logs the error and returns None.

    Raises:
        None: This function does not raise exceptions directly but logs errors in case of failed requests.
    """
    url = f'{url}/api/v2/workspaces/{workspace_id}/runs'
    ...
    if response.status_code == 200:
        ...
        logging.info(f'Error retrieving runs: {response.status_code}\n{response.text}')
        return None

def discard_run(api_token, run_id,url="https://app.terraform.io"):
    ...
    if response.status_code == 202:
        ...
        logging.info(f"Cannot discard run {run_id}: Transition not allowed.")
    else:
        logging.info(f'Error discarding run {run_id}: {response.status_code}\n{response.text}')

def cancel_run(api_token, run_id,url="https://app.terraform.io"):
    ...
    if response.status_code == 202:
        ...
        logging.info(f"Cannot cancel run {run_id}: Transition not allowed.")
    else:
        logging.info(f'Error canceling run {run_id}: {response.status_code}\n{response.text}')

def trigger_run(api_token, organization, workspace_id, auto_apply,url="https://app.terraform.io"):
    ...
    if response.status_code == 201:
        ...
        if run_id:
            ...
            runs = get_workspace_runs(api_token, workspace_id)
            if runs:
...
def get_run_status(api_token, organization, run_id,url="https://app.terraform.io"):
    headers = {
        'Authorization': f'Bearer {api_token}',
        'Content-Type': 'application/vnd.api+json'
    }
...
def monitor_run(api_token, organization, run_id,url="https://app.terraform.io"):
    """
    Monitor the status of a Terraform run until it completes.
    """
    while True:
        status = get_run_status(api_token, organization, run_id)
        if status in ['pending', 'planning', 'applying', 'policy_checking', 'plan_queued', "post_plan_running", "apply_queued" ]:
...
def get_run_outputs(api_token, run_id, url="https://app.terraform.io"):
    """
...
def get_plan_summary(api_token, run_id, url="https://app.terraform.io"):
    """
    Retrieves the plan summary (additions, changes, deletions) from a Terraform Cloud run.
    ...
    Returns:
        dict: A dictionary containing 'add', 'change', 'destroy' counts.
    """
    headers = {
        'Authorization': f'Bearer {api_token}',
        'Content-Type': 'application/vnd.api+json'
...
def apply_run(api_token, run_id, url="https://app.terraform.io"):
    """
