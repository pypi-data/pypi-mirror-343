...
from nnunet_extend.training.loss.compound_losses import DC_and_CE_and_CBDC_loss, DC_and_CE_and_CL_M_DC_loss
from nnunet_extend.registry.registry import TRAINERS

@TRAINERS.register_module()
class SoftcbDiceTrainer(nnUNetTrainer):
    def _build_loss(self):
        if self.label_manager.has_regions:
            ...
            loss = SoftcbDiceLoss(iter_=10, smooth=1e-3)
...
@TRAINERS.register_module()
class CompoundcbDiceTrainer(nnUNetTrainer):
    def _build_loss(self):
        if self.label_manager.has_regions:
            loss = DC_and_BCE_loss({},
                                   {'batch_dice': self.configuration_manager.batch_dice,
...
@TRAINERS.register_module()
class SoftclMDiceTrainer(nnUNetTrainer):
    def _build_loss(self):
        if self.label_manager.has_regions:
            loss = DC_and_BCE_loss({},
                                   {'batch_dice': self.configuration_manager.batch_dice,
            ...
            loss = SoftclMDiceLoss(iter_=10, smooth=1.0)
...
@TRAINERS.register_module()
class CompoundclMDiceTrainer(nnUNetTrainer):
    def _build_loss(self):
        if self.label_manager.has_regions:
            loss = DC_and_BCE_loss({},
                                   {'batch_dice': self.configuration_manager.batch_dice,
