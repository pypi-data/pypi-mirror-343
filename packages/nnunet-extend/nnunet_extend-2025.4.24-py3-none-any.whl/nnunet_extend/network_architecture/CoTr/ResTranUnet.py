...
import numpy as np
from nnunet_extend.network_architecture.CoTr import CNNBackbone
from nnunet_extend.network_architecture.CoTr.neural_network import SegmentationNetwork
from nnunet_extend.network_architecture.CoTr.DeTrans.DeformableTrans import DeformableTransformer
from nnunet_extend.network_architecture.CoTr.DeTrans.position_encoding import build_position_encoding
...
class U_ResTran3D(nn.Module):
    def __init__(self, in_channels=1, norm_cfg='BN', activation_cfg='ReLU', img_size=None, num_classes=None, weight_std=False):
        ...
        self.backbone = CNNBackbone.Backbone(in_channels=in_channels, depth=9, norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std)
...
class ResTranUnet(SegmentationNetwork):
    ...
    ResTran-3D Unet
    """
    def __init__(self, in_channels=1, norm_cfg='BN', activation_cfg='ReLU', img_size=None, num_classes=None, weight_std=False, deep_supervision=False):
        super().__init__()
        self.do_ds = False
        self.U_ResTran3D = U_ResTran3D(in_channels, norm_cfg, activation_cfg, img_size, num_classes, weight_std) # U_ResTran3D
...
if __name__ == '__main__':
    model = ResTranUnet(norm_cfg='IN', activation_cfg='ReLU', img_size=(64, 64, 64), num_classes=3, weight_std=False, deep_supervision=True)
    model = model.cuda()
