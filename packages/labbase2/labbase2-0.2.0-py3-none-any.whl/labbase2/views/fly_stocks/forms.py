...
class FilterFlyStocks(FilterForm):
    ...
    short_genotype = StringField(
        ...
        validators=[Optional()],
        filters=[strip_input],
        render_kw=render.custom_field | {"placeholder": "m6[3xcs]"},
        description="The genotype of the fly stock.",
    )
    chromosome_x = StringField(
        label="Chromosome X", validators=[Optional()], render_kw=render.custom_field
    )
    chromosome_y = StringField(
        label="Chromosome Y", validators=[Optional()], render_kw=render.custom_field
    )
    chromosome_2 = StringField(
        label="Chromosome 2", validators=[Optional()], render_kw=render.custom_field
    )
    chromosome_3 = StringField(
        label="Chromosome 3", validators=[Optional()], render_kw=render.custom_field
    )
    chromosome_4 = StringField(
        label="Chromosome 4", validators=[Optional()], render_kw=render.custom_field
    )
    source = StringField(
        label="Source",
        validators=[Optional()],
        filters=[strip_input],
        render_kw=render.custom_field | {"placeholder": "anne uv"},
...
class EditFlyStock(EditEntityForm):
    ...
    documentation = TextAreaField(
        ...
        validators=[Optional(), Length(max=2048)],
        filters=[strip_input],
        render_kw=render.custom_field | {"placeholder": "Documentation"},
    )
    reference = StringField(
        "Reference",
        validators=[Optional(), Length(max=512)],
