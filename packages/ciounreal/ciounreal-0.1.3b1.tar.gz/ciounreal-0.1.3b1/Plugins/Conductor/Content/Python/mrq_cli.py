...
def main():
    ...
    # Only connect to Perforce if all related environment variables were set
    p4 = None
    p4_environ = ["P4PORT", "P4USER", "P4PASSWD"]
    
    for ev in p4_environ:
        if os.environ.get(ev):
            p4_environ.remove(ev)
    
    if p4_environ:
        p4_formatted = ", ".join(p4_environ)
        unreal.log(f'MRQ CLI: The following P4 environment variable(s) are not set: {p4_formatted}. Skipping Perforce sync.')
    else:
         try:
            p4 = perforce.PerforceConnection().p4
            p4.client = app.get_workspace_name(project_name=unreal_utils.get_project_name())
         except Exception as e:
            unreal.log_error(f'MRQ CLI: Perforce connection error: {str(e)}')
