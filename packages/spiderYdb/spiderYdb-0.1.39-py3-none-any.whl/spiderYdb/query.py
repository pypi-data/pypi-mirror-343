                    ...
                    if param.foreign_key:
                        for ii in param.foreign_key:
                            if ii.split('.')[0] == self.table.table_name:
                                ...
                                joins[f'JOIN {t1.table_name} ON '] = joins.get(f'JOIN {t1.table_name} ON ', [])
                                joins[f'JOIN {t1.table_name} ON '].append(f'{t1.table_name}.{i} = {ii}')
        ...
        if self.join_many:
            ...
            for i in result:
                ...
                for join in self.join_many:
                    ...
                    lst.append({param: i[param] for param in i if param.startswith(f"{join}.")})
