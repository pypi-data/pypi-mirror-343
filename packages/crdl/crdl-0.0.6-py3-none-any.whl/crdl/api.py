...
class CrunchyrollAPI:
    ...
    def get_drm_auth(self, asset_id: str) -> Optional[Dict]:
        """
        Get DRM authentication data from Crunchyroll
    
        Args:
            asset_id: Asset ID extracted from the MPD URL
        
        Returns:
            Optional[Dict]: Dictionary containing custom_data and token for DRM license requests
        """
        if not self.access_token or not self.account_id:
            logger.error("Missing access token or account ID for DRM auth")
            return None
        ...
        try:
            ...
            # Send the request
            logger.info(f"Requesting DRM auth for asset_id: {asset_id}")
            response = requests.post(self.config.DRM_AUTH_URL, headers=headers, json=request_data)
            
            # Status code 201 means Created, which is a success for this endpoint
            if response.status_code != 200 and response.status_code != 201:
                logger.error(f"DRM auth request failed with status code: {response.status_code}")
                logger.error(f"Response: {response.text}")
                return None
            
            # Parse the response
            auth_data = response.json()
            logger.info(f"DRM auth successful")
            
            if 'custom_data' in auth_data and 'token' in auth_data:
                logger.info(f"Received custom_data and token for DRM authentication")
                return {
                    "custom_data": auth_data.get("custom_data"),
                    "token": auth_data.get("token")
                }
            else:
                logger.error(f"Missing custom_data or token in response")
                return None
                
        except Exception as e:
            logger.error(f"Error getting DRM auth: {str(e)}", exc_info=True)
            return None 