...
from typer_repyt.cache.exceptions import CacheError, CacheInitError
from typer_repyt.cache.manager import CacheManager, clear_directory
...
class TestSettingsManager:
    ...
    def test_init__raises_exception_on_fail(self, mocker: MockerFixture):
        mocker.patch("typer_repyt.cache.manager.Path.mkdir", side_effect=RuntimeError("BOOM!"))
    ...
    def test_store_bytes__raises_error_if_write_fails(self, fake_cache_path: Path, mocker: MockerFixture):
        mocker.patch("typer_repyt.cache.manager.Path.write_bytes", side_effect=RuntimeError("BOOM!"))
        test_path = Path("jawa/ewok/hutt")
        manager = CacheManager("test")
        data = b"test data"
    ...
    def test_store_bytes__raises_error_if_chmod_fails(self, fake_cache_path: Path, mocker: MockerFixture):
        mocker.patch("typer_repyt.cache.manager.Path.chmod", side_effect=RuntimeError("BOOM!"))
        test_path = Path("jawa/ewok/hutt")
        manager = CacheManager("test")
        data = b"test data"
        mode = 0o141
    ...
    def test_load_bytes__fails_if_read_fails(self, fake_cache_path: Path, mocker: MockerFixture):
        mocker.patch("typer_repyt.cache.manager.Path.read_bytes", side_effect=RuntimeError("BOOM!"))
        test_path = Path("jawa/ewok/hutt")
        manager = CacheManager("test")
        data = b"test data"
        full_path = fake_cache_path / test_path
    ...
    def test_clear_path__raises_error_if_unlink_fails(self, mocker: MockerFixture):
        mocker.patch("typer_repyt.cache.manager.Path.unlink", side_effect=RuntimeError("BOOM!"))
        test_path = Path("jawa/ewok/hutt")
        manager = CacheManager("test")
    ...
    def test_clear_all__raises_error_if_clear_directory_fails(self, mocker: MockerFixture):
        mocker.patch("typer_repyt.cache.manager.clear_directory", side_effect=RuntimeError("BOOM!"))
        manager = CacheManager("test")
