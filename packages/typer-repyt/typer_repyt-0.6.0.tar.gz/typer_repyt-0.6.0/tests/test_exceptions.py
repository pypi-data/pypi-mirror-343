...
from typer_repyt.exceptions import RepytError, handle_errors
from typer_repyt.cache.manager import CacheManager
from typer_repyt.cache.attach import attach_cache
...
class TestHandleErrors:
    ...
    def test_handle_errors__handles_repyt_error_by_default(self):
        cli: typer.Typer = typer.Typer()

        @cli.command()
        @handle_errors("What happened?")
        def _():
            raise RepytError("Boring conversation anyway")
        ...
        check_output(
            ...
            exit_code=RepytError.exit_code,
            prog_name="test",
        )
    ...
    def test_handle_errors__raises_runtime_error_if_format_fails(self, mocker: MockerFixture):
        mocker.patch("typer_repyt.exceptions.reformat_exception", side_effect=ValueError("BOOM!"))
    ...
    def test_handle_errors__uses_subject_and_footer_from_repyt_error_if_provided(self):
        cli: typer.Typer = typer.Typer()
