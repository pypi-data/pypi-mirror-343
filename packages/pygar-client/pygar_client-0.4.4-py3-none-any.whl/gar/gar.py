...
class GARClient:
    ...
    def register_heartbeat_handler(self, handler: Callable[[int], None]):
    ...
    def register_default_handlers(self):
        ...
        self.register_subscription_status_handler(
            lambda name, status: self.logger.info(
                "Subscription %s status: %s", name, status
            )
        )
    ...
    def check_heartbeat(self):
        ...
        if time.time() > cutoff:
            self.logger.warning(
                "Heartbeat failured; previous heartbeat: %.3fs",
    ...
    def subscribe(
        ...
        include_unknown_keys: Optional[bool] = False,
    ):
