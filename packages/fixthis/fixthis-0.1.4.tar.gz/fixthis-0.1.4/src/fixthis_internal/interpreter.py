...
from fixthis_internal.number_converter import parse_english_number
...
class FixVisitor(NodeTransformer):
    def visit_Name(self, node):
        if not isinstance(node.ctx, Load):
            return node
        return Call(
            func=Name(id="___fixthis_internal_name___", ctx=Load()),
            args=[
...
def ___fixthis_internal_name___(varname, *scopes):
    ...
    if len(varname) > 0 and varname[0] == "_":
        try:
            return ucd.lookup(varname[1:].replace("_", " ").upper())
        except:
            ...
    try:
        return parse_english_number(varname)
    except:
        return varname
