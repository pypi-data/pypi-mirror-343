...
# Store original data to arrays xxx_ori
    ...
    def write_stn(self, file, data1, x, y, *, data2=None, day=None, month=None,
                  year=None, time=None, direction=0):
        """
...
#    minute after the end of the former line.
                ...
                for ix, xx in enumerate(x[::-1]):
                    ...
                    if isinstance(time, np.ndarray):
                        t = time[:, -(ix+1)]-np.array(time[:, -(ix+1)]/86400.,
                                                      dtype=int)*86400.
                    else:
                        t = start[-(ix+1)] + tref
                    t = t[index][::-1]
    ...
    def read_base(self, infile, year=None):
        ...
        if lines[0][0] == "*":
            ...
            for k in range(2, nlines):
                ...
                self.hour_base.append(int(line[3][0:2]))
                self.jday_base.append(self.u.date2julian(
