...
def map_topic_prompts(row):
    trf_row = row.copy()
    trf_row['prompt_content'] = GENERAL_PURPOSE_DOCUMENTS_TOPICS.format(
...
def map_html_prompts(row):
    trf_row = row.copy()
    trf_row['prompt_html'] = GENERAL_PURPOSE_DOCUMENTS_HTML.format(
                    document_type=row['document_type'],
                    audience=row['audience'],
                    tone=row['tone'],
    ...
    def generate(
        ...
        **kwargs
        ...
        Args:
            ...
            tone (str, optional): Desired tone of the document
