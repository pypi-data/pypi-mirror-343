...
def extract_audio_from_video(video: Union[Clip, str], output_filename: Union[str, None] = None):
    """
...
def set_audio_in_video(video: Union[Clip, str], audio: Union[AudioClip, str], output_filename: Union[str, None] = None):
    """
    ...
    if output_filename:
        # TODO: Check extension, please
        # tmp_output_filename = Temp.create_filename('tmp_output.mp4')
...
def set_audio_in_video_ffmpeg(
    ...
    if (
        ...
        not FileValidator.file_is_audio_file(audio_filename) or
        not video_filename or
        not FileValidator.file_is_video_file(video_filename)
    ):
