...
class TitledGreenScreenImage:
    ...
    def __init__(self, title, width = 1344, height = 756):
        if width <= 0 or width > MOVIEPY_SCENE_DEFAULT_SIZE[0] or height <= 0 or height > MOVIEPY_SCENE_DEFAULT_SIZE[1]:
    ...
    def save(self, output_filename):
        ...
        if self.title:
            ...
            position = ((MOVIEPY_SCENE_DEFAULT_SIZE[0] / 2) - (w / 2), MOVIEPY_SCENE_DEFAULT_SIZE[1] - ((MOVIEPY_SCENE_DEFAULT_SIZE[1] - self.height) / 2) - (h / 2 + PADDING))
    ...
    def __generate_green_screen(self):
        ...
        filename = Temp.create_filename('tmp_gs_shape.png')
        img.save(filename)

        return filename
    
    def insert_video(self, video_filename, output_filename):
        tmp_filename = Temp.create_filename('tmp_gs.png')
