...
def sub_command_for_def(string, command, definition, num_args):
    ...
    if num_args > 0:
        ...
        while match:
            ...
            subbed_definition = re.compile("|".join(re.escape(key) for key in sub_for_args.keys())).sub(lambda match: sub_for_args[match.group(0)], definition)
    ...
    # If no args
        ...
        pattern = re.escape(command) + r"(?=\b|_|\^|\{|\}|\(|\)|\[|\]|$|\s|~|\$|\d)"
