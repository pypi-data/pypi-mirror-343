...
class ManagePortGroup():
    ...
    def reservePortGroup(self, sessionId=None, overallSummaryFile=None, user=None, stage=None, task=None, env=None):
        """ 
                    ...
                    if getSessionIdInWaitingList:
                        ...
                        for index, inQueue in enumerate(portGroupData['waitList']):
                            if inQueue['sessionId'] == getSessionIdInWaitingList:
                                ...
                                if sessionLog and self.keystackLogger: self.keystackLogger.info('portGroup={self.portGroup}: Giving priority to the same sessionId from the waitList')
                                
                        if foundSessionIdInWaitList is False:
                            if sessionLog and self.keystackLogger: self.keystackLogger.info('portGroup={self.portGroup}: getSessionIdInWaitingList=None. Getting the top-of-waitList to active-user: portGroupData["waitList"][0]}')
