                ...
                for index, portGroupObj in enumerate(portGroups):
                    ...
                    if portGroupObj['ports'] == {}:
                        ...
                        for device in portGroupObj['ports'].keys():
                            ...
                            portsDropdown += f"<div class='dropdown-toggle textBlack' data-toggle='dropdown' role='button' aria-haspopup=true aria-expanded=false><a class='textBlack openPortConfigsClass' href='#' portGroup={portGroupObj['name']} type='button'>{len(portlist)}&ensp;Ports</a></div>" 
                    ...
                    html += f'<td class="paddingLeft5px">{portsDropdown}</td>'
                    ...
                    if ManagePortGroup(domain=domain, portGroup=portGroup).isPortGroupAvailable():
                        ...
                        if ports != {}:
                            ...
                            for device in ports.keys():
                                ...
                                #     # Get all the devices. Then loop through each device's portMgmt to search
                                #     # for the portGroup and delete it from portGroups list.
...
class AmINextPortGroup(APIView):
    def post(self, request):
        ...
        return Response(data={'result': amINext, 'status': status, 'errorMsg': errorMsg}, status=statusCode)    
