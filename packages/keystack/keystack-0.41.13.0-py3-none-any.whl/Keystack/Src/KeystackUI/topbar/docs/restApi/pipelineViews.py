            ...
            try:
                ...
                for eachSession in pipelines:
                    ...
                    if isEnvsOnHold is False and os.path.exists(testResultsPath):
                        execSubprocessInShellMode(f'sudo rm -r {testResultsPath}')        
...
class SavePipeline(APIView):
    ...
    def post(self,request):
        ...
        # request.data: 
        #     {'pipelineName': 'abc', 'remoteController': '192.168.28.10:28028', 'playbook': 'Samples/pythonSample', 'debug': True, 'emailResults': False, 'awsS3': False, 'jira': False, 'pauseOnFailure': False, 'holdEnvsIfFailed': False, 'abortTestOnFailure': False, 'includeLoopTestPassedResults': False, 'sessionId': 'hgee2', 'domain': 'Communal', 'reconfigs': 'demoChanges'}
...
class AddJobSchedule(APIView):                                           
    ...
    def post(self, request):
        ...
        reconfigs                    = request.data.get('reconfigs', None)
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            try:
                ...
                newJob += f'{debugMode}&domain={domain}&reconfigs={reconfigs}&holdEnvsIfFailed={holdEnvsIfFailed}&abortTestOnFailure={abortTestOnFailure}'
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            try:
                ...
                for eachCron in cronjobs:
                    ...
                    # 13 17 * * * keystack curl -d "mainController=192.168.28.10:28028&remoteController=192.168.28.10:28028&reservationUser=Hubert Gee&sessionId=&playbook=DOMAIN=Communal/Samples/advance&awsS3=False&jira=False&pauseOnFailure=False&debug=None&domain=Communal&reconfigs=&holdEnvsIfFailed=False&abortTestOnFailure=False&includeLoopTestPassedResults=False&scheduledJob=playbook=DOMAIN=Communal/Samples/advance minute=13 hour=17 day=* month=* dayOfWeek=*&removeJobAfterRunning=False&webhook=true" -H "Content-Type: application/x-www-form-urlencoded" -X POST http://192.168.28.10:28028/api/v1/playbook/runPlaybook
...
class GetTestcasesInProgress(APIView):
    ...
    def post(self, request, data=None):
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            # /opt/KeystackTests/Results/DOMAIN=Communal/PLAYBOOK=Samples-advance/11-22-2024-16:06:49:754527_8767         
            if os.path.exists(resultsPath):
                timestampFolderName = resultsPath.split('/')[-1]
                resultsMetaDataPath = f'{resultsPath}/.Data/ResultsMeta'
                        
                for index, (testcase, cmdLineArgs) in enumerate(eval(testcaseSortedOrderList)):
                    # testcase: /opt/KeystackTests/Playlist/Demo/Bringups/bringupDut1.yml
                    # cmdLineArgs: '' <-- empty string
                    
                    # /opt/KeystackTests/Results/DOMAIN=Communal/PLAYBOOK=Samples-advance/11-22-2024-16:06:49:754527_8767/.Data/ResultsMeta/opt/KeystackTests/Playlist/Demo/Bringups/bringupDut1.yml
                    ...
                    for testcaseIteration in glob(f'{testcaseMetaDataPath}*'):
                        ...
                        if testcaseMetaData:  
                            ...
                            # If the testcase is still running, and possibly on pauseOnFailure.
                            testcaseResult = 'Not-Ready'
                            status = 'Unknown'                          
...
class GetTestcasesInProgress_backup(APIView):
    swagger_schema = None
...
class GetReconfigsDropdownForPipeline(APIView):
    swagger_schema = None
    ...
    def post(self, request, data=None):
        ...
        Description:
            Internal usage only
            Reconfig selection for "pipeline test parameters"
        
        POST /api/vi/pipelines/reconfigs/getReconfigsDropdownForPipeline
        
        ---
        Examples:
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/getReconfigsDropdownForPipeline
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/x-www-form-urlencoded" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/getReconfigsDropdownForPipeline
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/json" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/getReconfigsDropdownForPipeline
            
            session = requests.Session()
            response = session.request('post', 'http://localhost:8000/api/v1/pipelines/reconfigs/getReconfigsDropdownForPipeline')
        """
        statusCode = HtmlStatusCodes.success
        errorMsg = None
        status = 'success'
        ...
        reconfigsHtml = '<label for="reconfigs"><strong>Select a reconfig file:</strong></label>&emsp;'
        reconfigsHtml += '<select id="reconfigs">'
        reconfigsHtml += '<option value="" selected="selected">Select</option>'
                
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            params = {}
            restApi = '/api/v1/pipelines/reconfigs/getReconfigsDropdownForPipeline'
            response, errorMsg , status = executeRestApiOnRemoteController('post', remoteControllerIp, restApi, params, 
                                                                  user, webPage=Vars.webpage, action='GetReconfigsDropdownForPipeline')  
            reconfigsHtml = response.json()['reconfigs']
            
        else:             
            for reconfigFile in glob(f'{GlobalVars.reconfigsFolder}/*.yml'):
                reconfigFileName = reconfigFile.split('/')[-1].split('.')[0]
                reconfigsHtml += f'<option value="{reconfigFileName}">{reconfigFileName}</option>'
               
        reconfigsHtml += '</select>'
        
        return Response(data={'reconfigs':reconfigsHtml, 'errorMsg': errorMsg, 'status': status}, status=statusCode)
   
 
class GetReconfigsList(APIView):
    swagger_schema = None
    ...
    def post(self, request, data=None):
        ...
        Description:
            Internal usage only
            Get reconfig selection for "modifying testcase" configurations
        
        POST /api/vi/pipelines/reconfigs/getReconfigsList
        
        ---
        Examples:
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/getReconfigsList
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/x-www-form-urlencoded" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/getReconfigsList
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/json" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/getReconfigsList
            
            session = requests.Session()
            response = session.request('post', 'http://localhost:8000/api/v1/pipelines/reconfigs/getReconfigsList')
        """
        statusCode = HtmlStatusCodes.success
        errorMsg = None
        status = 'success'
        ...
        reconfigsHtml = '<select id="reconfigsList" onchange="getReconfigContents(value)">'
        reconfigsHtml += '<option value="" selected="selected">Select Reconfig File</option>'
                
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            params = {}
            restApi = '/api/v1/pipelines/reconfigs/getReconfigsList'
            response, errorMsg , status = executeRestApiOnRemoteController('post', remoteControllerIp, restApi, params, 
                                                                  user, webPage=Vars.webpage, action='GetReconfigsList')  
            reconfigsHtml = response.json()['reconfigsList']
            
        else:      
            for reconfigFile in glob(f'{GlobalVars.reconfigsFolder}/*.yml'):
                reconfigFileName = reconfigFile.split('/')[-1].split('.')[0]
                reconfigsHtml += f'<option value="{reconfigFile}">{reconfigFileName}</option>'
                
        reconfigsHtml += '</select>'
        
        return Response(data={'reconfigsList': reconfigsHtml, 'errorMsg': errorMsg, 'status': status}, status=statusCode)

    
class DeleteReconfigs(APIView):
    swagger_schema = None

    @verifyUserRole(webPage=Vars.webpage, action='DeleteReconfigs', exclude=['engineer'])
    def post(self, request, data=None):
        """
        Description:
            Internal usage only
        ...
        POST /api/vi/pipelines/reconfigs/delete
        
        ---
        Examples:
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/delete
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/x-www-form-urlencoded" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/delete
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/json" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/delete
            
            session = requests.Session()
            response = session.request('post', 'http://localhost:8000/api/v1/pipelines/reconfigs/delete)
        """
        statusCode = HtmlStatusCodes.success
        errorMsg = None
        status = 'success'
        user = AccountMgr().getRequestSessionUser(request)
        mainControllerIp, remoteControllerIp = getMainAndRemoteControllerIp(request)
        reconfigsFullPath = request.data.get('reconfigFullPath', None)
                
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            params = {'reconfigFullPath': reconfigsFullPath}
            restApi = '/api/v1/pipelines/reconfigs/delete'
            response, errorMsg , status = executeRestApiOnRemoteController('post', remoteControllerIp, restApi, params, 
                                                                  user, webPage=Vars.webpage, action='DeleteReconfigs')  
        else:
            try:
                if os.path.exists(reconfigsFullPath):
                    os.remove(reconfigsFullPath)
                    SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='DeleteReconfigs', msgType='Success',
                                              msg=f'Deleted reconfigs: {reconfigsFullPath}', forDetailLogs='')
                else:
                    SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='DeleteReconfigs', msgType='Info',
                                              msg=f'No reconfig file: {reconfigsFullPath}', forDetailLogs='')
            except Exception as errMsg:
                errorMsg = str(errMsg)
                status = 'failed'
                SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='DeleteReconfigs', msgType='Error',
                                          msg=errorMsg, forDetailLogs=traceback.format_exc(None, errMsg))
                                
        return Response(data={'errorMsg': errorMsg, 'status': status}, status=statusCode)
    

class GetReconfigParams(APIView):
    swagger_schema = None

    @verifyUserRole(webPage=Vars.webpage, action='GetReconfigParams', exclude=['engineer'])
    def post(self, request, data=None):
        """
        Description:
            Internal usage only
            Get all the reconfig params to help user create a new reconfig file
        
        POST /api/vi/pipelines/reconfigs/getParams
        
        ---
        Examples:
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/getParams
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/x-www-form-urlencoded" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/getParams
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/json" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/getParams
            
            session = requests.Session()
            response = session.request('post', 'http://localhost:8000/api/v1/pipelines/reconfigs/getParams)
        """
        statusCode = HtmlStatusCodes.success
        status = 'success'
        errorMsg = None
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            params = {}
            restApi = '/api/v1/pipelines/reconfigs/getParams'
            response, errorMsg , status = executeRestApiOnRemoteController('post', remoteControllerIp, restApi, params, 
                                                                  user, webPage=Vars.webpage, action='GetReconfigParams')  
        else:
            try:
                currentDir = os.path.abspath(os.path.dirname(__file__))
                reconfigsTemplateYmlFile = f'{currentDir}/reconfigsTemplate.yml'
                reconfigContents = readFile(reconfigsTemplateYmlFile)
            except Exception as errMsg:
                errorMsg = str(errMsg)
                status = 'failed'
                SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='GetReconfigParams', msgType='Error',
                                          msg=errorMsg, forDetailLogs=traceback.format_exc(None, errMsg))
                                
        return Response(data={'reconfigParams': reconfigContents, 'errorMsg': errorMsg, 'status': status}, status=statusCode)


class SaveNewReconfigsToFile(APIView):
    swagger_schema = None

    @verifyUserRole(webPage=Vars.webpage, action='SaveNewReconfigsToFile', exclude=['engineer'])
    def post(self, request, data=None):
        """
        Description:
            Internal usage only
            Save new reconfigs to a file
        
        POST /api/vi/pipelines/reconfigs/saveNewReconfigsToFile
        
        ---
        Examples:
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/saveNewReconfigsToFile
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/x-www-form-urlencoded" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/saveNewReconfigsToFile
            
            curl -H "API-Key: iNP29xnXdlnsfOyausD_EQ" -H "Content-Type: application/json" -X POST http://192.168.28.7:8000/api/v1/pipelines/reconfigs/saveNewReconfigsToFile
            
            session = requests.Session()
            response = session.request('post', 'http://localhost:8000/api/v1/pipelines/reconfigs/saveNewReconfigsToFile)
        """
        statusCode = HtmlStatusCodes.success
        errorMsg = None
        status = 'success'
        user = AccountMgr().getRequestSessionUser(request)
        mainControllerIp, remoteControllerIp = getMainAndRemoteControllerIp(request)
        reconfigsFilename = request.data.get('reconfigName', None)
        textArea = request.data.get('textarea', None)
        
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            params = {'reconfigName': reconfigsFilename, 'textarea': textArea}
            restApi = '/api/v1/pipelines/reconfigs/saveNewReconfigsToFile'
            response, errorMsg , status = executeRestApiOnRemoteController('post', remoteControllerIp, restApi, params, 
                                                                  user, webPage=Vars.webpage, action='SaveNewReconfigsToFile')  
        else:
            try:
                reconfigFileFullPath = f'{GlobalVars.reconfigsFolder}/{reconfigsFilename}.yml'
                if os.path.exists(reconfigFileFullPath):
                    raise Exception(f'There is already a reconfig with same filename already: {reconfigsFilename}')
                
                with open(reconfigFileFullPath, 'w') as fileObj:
                    fileObj.write(textArea)
                 
                chownChmodFolder(GlobalVars.reconfigsFolder,
                                 user=GlobalVars.user, userGroup=GlobalVars.userGroup, permission=770, stdout=False)
                
                SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='SaveNewReconfigsToFile', msgType='Success',
                                          msg=reconfigFileFullPath, forDetailLogs='')
                                   
            except Exception as errMsg:
                errorMsg = str(errMsg)
                status = 'failed'
                SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='SaveNewReconfigsToFile', msgType='Error',
                                          msg=errorMsg, forDetailLogs=traceback.format_exc(None, errMsg))
