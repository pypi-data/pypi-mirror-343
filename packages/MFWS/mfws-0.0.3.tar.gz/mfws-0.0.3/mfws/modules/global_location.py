...
class LineScanStitch:
    ...
    def get_offset_by_jitter(self, jit):
        if self.line_scan == 0:
            ...
            _oo = [i for i in list(offset[1::2, :]) if i[0] != self._placeholder]
            if len(_oo) == 0: offset_odd = np.nan
            else: offset_odd = np.round(np.mean(_oo, axis = 0))

            _oe = [i for i in list(offset[::2, :]) if i[0] != self._placeholder]
            if len(_oe) == 0: offset_even = np.nan
            else: offset_even = np.round(np.mean(_oe, axis = 0))
        ...
        for item in stitch_list:
            ...
            if np.max(stitch_mask) == 0:
                ...
                if len(loc_list) >= 1:
                    ...
                    # ilog.info("{}:{} no stitch neighbor".format(row, col))
                ...
                if flag is False:
                    ...
                    for src in neighbor:
                        if dst[0] < src[0]:  # down
                            ...
                            ilog.info('[{}-{}] fix_loc error!'.format(dst[0], dst[1]))
                    if len(tem_loc) > 0:
