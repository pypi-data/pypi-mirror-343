...
def test_improved_node_edge_centrality():
    # test empty hypergraph
    H = xgi.Hypergraph()
    assert xgi_optimized.algorithms.centrality.improved_node_edge_centrality(H) == (dict(), dict())

    # Test no edges
    H.add_nodes_from([0, 1, 2])
    nc, ec =  xgi_optimized.algorithms.centrality.improved_node_edge_centrality(H)
    assert set(nc) == {0, 1, 2}
    for i in nc:
        assert np.isnan(nc[i])
    assert ec == dict()
