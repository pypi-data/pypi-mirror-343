...
def find_openssl_on_windows():
    """
    Tries to detect OpenSSL installation path on Windows with multiple fallback mechanisms.
    Returns a tuple of (include_dir, lib_dir, lib_name)
    """
    # 1. Try environment variable approach first (current method)
    ...
    potential_locations = [
        # OpenSSL installed from official installer
        ...
        # Chocolatey OpenSSL
        os.path.join(os.environ.get('ChocolateyInstall', r'C:\ProgramData\chocolatey'), 'lib', 'openssl'),
        # Scoop
        os.path.expanduser(r'~\scoop\apps\openssl'),
    ]
    
    # Check version-specific subdirectories in each location
    ...
    for base_location in potential_locations:
        ...
        for subdir in version_subdirs:
            ...
            # Check if this is a valid OpenSSL installation
            openssl_h = os.path.join(include_dir, "openssl", "opensslv.h")
            if os.path.exists(openssl_h) and os.path.exists(lib_dir):
                # Determine which library file exists
    ...
    # 3. Try to detect from the current PATH
    try:
        ...
        if which_openssl:
            ...
            if os.path.exists(os.path.join(include_dir, "openssl", "opensslv.h")):
                for lib_name in ['libcrypto.lib', 'libeay32.lib', 'crypto.lib']:
                    if os.path.exists(os.path.join(lib_dir, lib_name)):
                        return include_dir, lib_dir, lib_name
    except Exception:
    ...
    # If we get here, we couldn't find OpenSSL
    error_message = (
        "OpenSSL not found. Please install OpenSSL and set one of:\n"
        "1. Set OPENSSL_CONF environment variable to the openssl.cnf path\n"
        "2. Install OpenSSL in a standard location\n"
        "3. Install via package manager like vcpkg ('vcpkg install openssl:x64-windows')\n"
    ...
    def build_extension(self, ext):
        ...
        if sys.platform != "win32":
            ...
            try:
                ...
                ext.extra_link_args.append(lib_name)
                ext.extra_link_args.append('/LIBPATH:' + lib_dir)
