...
class SparkClient:
    ...
    def get_feature_table(self, table_name):
        ...
        table_properties = {row.key: row.value for row in properties}
        # 获取主键信息
        primary_key_str = table_properties.get("primaryKeys")
        primary_keys = primary_key_str.split(",") if primary_key_str else []
        # 获取表ID
        table_id = table_properties.get("table_id", table_name)
        description = table.description or table_properties.get("comment", "")
        ...
        if partition_info:
            ...
            if partition_str.startswith("struct<"):
                # 使用更高效的正则表达式提取字段名
                import re
                partition_columns = re.findall(r'([^:,\s]+):', partition_str[7:-1])
