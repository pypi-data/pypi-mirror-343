...
class SparkClient:
    ...
    def get_feature_table(self, table_name):
        ...
        for row in table_details:
            ...
            elif row.col_name == "_partition":
                partition_info = row.data_type
                if partition_info.startswith("struct<") and partition_info.endswith(">"):
                    # 提取字段定义部分(去掉struct<>)
                    fields_str = partition_info[7:-1]
                    partition_field = fields_str.split(":")[0].strip() if fields_str else ""
                    partition_columns = [partition_field] if partition_field else []
