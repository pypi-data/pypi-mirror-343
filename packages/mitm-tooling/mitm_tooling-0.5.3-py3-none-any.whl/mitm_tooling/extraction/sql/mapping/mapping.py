...
class ColumnContentProvider(pydantic.BaseModel):
    ...
    def from_static(cls, name: str, value: str, dt: MITMDataType = MITMDataType.Text) -> Self:
        ce = sa.literal(value, dt.sa_sql_type_cls).alias(name)
...
class HeaderEntryProvider:
    ...
    def apply_session(self, db_session: Session) -> list[HeaderEntry]:
    ...
    def apply_iterable(self, distinct: Iterable[tuple[str, str]]) -> list[HeaderEntry]:
        return [HeaderEntry(concept=self.concept, kind=kind, type_name=type_name, attributes=tuple(self.attributes),
                            attribute_dtypes=tuple(self.attribute_dtypes)) for kind, type_name in distinct]


@dataclasses.dataclass
class InstancesPostProcessor:
    transforms: list[TableTransforms] = dataclasses.field(default_factory=list)
...
class InstancesProvider:
    ...
    def apply_session(self, db_session: Session) -> pd.DataFrame:
    ...
    def apply_session_chunked(self, db_session: Session, chunk_size: int) -> Iterable[pd.DataFrame]:
        tm = self.virtual_view.table_meta
