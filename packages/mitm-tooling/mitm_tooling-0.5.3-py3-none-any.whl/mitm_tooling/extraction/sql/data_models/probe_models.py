...
class NumericSummaryStatistics(pydantic.BaseModel):
    ...
    @staticmethod
    def EMPTY():
        return NumericSummaryStatistics(count=0, mean=np.nan, min=np.nan, max=np.nan, std=np.nan, percentile_25=np.nan, percentile_50=np.nan, percentile_75=np.nan)
...
class SampleSummary(pydantic.BaseModel):
    ...
    summary_statistics: NumericSummaryStatistics | CategoricalSummaryStatistics | DatetimeSummaryStatistics | None = None
