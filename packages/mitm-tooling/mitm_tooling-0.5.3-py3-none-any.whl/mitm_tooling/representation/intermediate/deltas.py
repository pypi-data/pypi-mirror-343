...
from .header import HeaderEntry, Header
...
def diff_header_entry(a: HeaderEntry, b: HeaderEntry) -> TypeUpdate | None:
    ...
    for i, ((attr_a, dt_a), (attr_b, dt_b)) in enumerate(
            zip(zip(a.attributes, a.attribute_dtypes), zip(b.attributes, b.attribute_dtypes))):
        if attr_a != attr_b or dt_a != dt_b:
            deltas.append(AttributeUpdate(index=i, name=attr_b, dt=dt_b))
    k_a, k_b = a.get_k(), b.get_k()
    x = k_b - k_a
    if x > 0:
        for j, (attr_b, dt_b) in zip(range(k_a, k_b), zip(b.attributes[k_a:], b.attribute_dtypes[k_a:])):
            deltas.append(AttributeInsertion(index=j, name=attr_b, dt=dt_b))
    elif x < 0:
        for j, (attr_a, dt_a) in zip(range(k_b, k_a), zip(a.attributes[k_b:], a.attribute_dtypes[k_b:])):
            deltas.append(AttributeDeletion(index=j, name=attr_a))
    return TypeUpdate(deltas=deltas)
