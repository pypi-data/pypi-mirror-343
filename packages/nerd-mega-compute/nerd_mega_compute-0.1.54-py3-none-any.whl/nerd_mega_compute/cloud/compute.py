...
def compute_data_fingerprint(data):
    """
    Compute a unique fingerprint for data to check for duplicates.
    Uses sampling approach for large data.
    
    Args:
        data: The data to fingerprint
        
    Returns:
        str: SHA-256 hexdigest fingerprint or None if fingerprinting fails
    """
    try:
        import numpy as np
        
        # Special handling for numpy arrays (common in astronomy data)
        if isinstance(data, np.ndarray):
            h = hashlib.sha256()
            # Include shape and dtype in fingerprint
            h.update(str(data.shape).encode())
            h.update(str(data.dtype).encode())
            
            # For large arrays, sample values instead of using whole array
            if data.size > 1_000_000:  # 1 million elements
                sample_size = 100_000  # Sample 100k elements
                
                # Sample start, middle, and end of array
                h.update(data.flat[:sample_size].tobytes())
                mid_point = data.size // 2
                h.update(data.flat[mid_point:mid_point+sample_size].tobytes())
                h.update(data.flat[-sample_size:].tobytes())
            else:
        ...
        # For bytes objects
            ...
            h = hashlib.sha256()
            # Use data length as part of fingerprint
            data_len = len(data)
            h.update(str(data_len).encode())
            
            # Sample data for very large objects
            if data_len > 10*1024*1024:  # 10MB
                ...
                h.update(data[middle_pos:middle_pos+sample_size])
                # Last 1MB
                h.update(data[-sample_size:])
            else:
                h.update(data)
        ...
        # For other objects, pickle first then fingerprint
        # This handles pandas DataFrames, astropy Tables, etc.
        pickled = pickle.dumps(data, protocol=4)
        return compute_data_fingerprint(pickled)
        
    except Exception as e:
        debug_print(f"Error computing fingerprint: {e}")
        return None

def get_cached_data_reference(fingerprint):
    """
    Check if data with the given fingerprint exists in cache
    
    Args:
        fingerprint: The data fingerprint to look up
        
    Returns:
        dict: The reference object if found, None otherwise
    """
    if not fingerprint:
        return None
    ...
    if fingerprint in data_cache:
        ...
        debug_print(f"Found cached data: {cached_info['dataId']}")
        ...
        return {
            ...
            'value': {
                'data_reference': cached_info['dataId'],
                's3Uri': cached_info.get('s3Uri', ''),
                'sizeMB': cached_info.get('sizeMB', '')
            }
        }
    return None

def cache_data_reference(fingerprint, upload_result):
    """
    Store data reference in the fingerprint cache
    
    Args:
        fingerprint: The data fingerprint
        upload_result: The result from upload_nerd_cloud_storage
    """
    if not fingerprint:
        return
    ...
    data_cache[fingerprint] = {
        ...
        'sizeMB': upload_result.get('sizeMB', ''),
        'uploadDate': time.strftime('%Y-%m-%d %H:%M:%S'),
        'lastAccessed': time.strftime('%Y-%m-%d %H:%M:%S')
    }
    save_data_cache(data_cache)
    debug_print(f"Cached data reference: {upload_result['dataId']} with fingerprint: {fingerprint}")
...
def serialize_with_chunking(obj):
    try:
        ...
        # For large data that might get uploaded to cloud storage,
        # check if we've already uploaded identical data before
        if is_large_data(obj, threshold_mb=3):
            debug_print("Large data detected, computing fingerprint...")
            
            # Compute data fingerprint for deduplication
            fingerprint = compute_data_fingerprint(obj)
            
            # Check if we've already uploaded this exact data
            if fingerprint:
                cached_reference = get_cached_data_reference(fingerprint)
                if cached_reference:
                    debug_print(f"Found identical data already in cloud storage!")
                    return cached_reference
                    
            # Data not found in cache, proceed with upload
            debug_print("Data not in cache, uploading...")
            upload_result = upload_nerd_cloud_storage(obj)
            
            # Handle upload failures gracefully
            if upload_result is None:
                debug_print("Upload failed, falling back to standard serialization")
                # Standard serialization for fallback
            ...
            if fingerprint:
                cache_data_reference(fingerprint, upload_result)
        ...
        # Handle functions or callable objects differently
        if callable(obj):
            # Special case for partial objects (common in parallel processing)
            if isinstance(obj, functools.partial):
                debug_print(f"Serializing partial function: {obj.func.__name__}")
                return {
                    'type': 'callable',
                    'function_type': 'partial',
                    'func_name': obj.func.__name__,
                    'func_module': obj.func.__module__,
                    'args': serialize_with_chunking(obj.args),
                    'keywords': serialize_with_chunking(obj.keywords)
                }
            # For regular functions
            else:
                func_name = getattr(obj, '__name__', 'unknown_function')
                func_module = getattr(obj, '__module__', '__main__')
                debug_print(f"Serializing function: {func_name} from module {func_module}")
                return {
                    'type': 'callable',
...
# Add functions to manage the cache from outside the module
def list_cached_data():
    """List all data fingerprints in the cache"""
