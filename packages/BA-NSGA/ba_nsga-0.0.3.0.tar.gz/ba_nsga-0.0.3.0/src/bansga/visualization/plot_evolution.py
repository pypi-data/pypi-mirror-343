...
class EvolutionPlotter:
    ...
    def _plot_stall_and_dataset_size(self) -> None:
        ...
        plt.step(generations_arr, stall_count_objetive_cumsum/np.max(stall_count_objetive_cumsum) * np.max(stall_arr), where='post', label="(relative) Cumulative objetive Stall", alpha=0.2)
        stall_count_information_cumsum = stall_count_information_arr
        plt.step(generations_arr, stall_count_information_cumsum/np.max(stall_count_information_cumsum) * np.max(stall_arr), where='post', label="(relative) Cumulative information Stall", alpha=0.2)
