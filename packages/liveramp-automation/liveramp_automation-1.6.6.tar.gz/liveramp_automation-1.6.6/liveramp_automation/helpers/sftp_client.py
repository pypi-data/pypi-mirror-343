...
class SFTPClient:
    ...
    def __enter__(self):
        ...
        """
        # Check if the hostname is "files.liveramp.com" and either:
        # - `self.cnopts` is not set (None or False), or
        # - `self.cnopts.hostkeys` is not set (empty or None).
        if self.hostname == "files.liveramp.com" and (
            not self.cnopts or not self.cnopts.hostkeys
        ):
            # Construct the full path to the "known_hosts" file from current dir.
            known_hosts = os.path.join(os.path.dirname(__file__), "known_hosts")
            # Create a new `pysftp.CnOpts` object, passing the path to the "known_hosts" file.
            # This ensures that the SFTP client uses the specified "known_hosts" file for host key verification.
            self.cnopts = pysftp.CnOpts(knownhosts=known_hosts)
