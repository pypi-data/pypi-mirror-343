...
class RunJob:
    ...
    def execute_windows_command(self, src: str, dest: str) -> bool:
        ...
        for attempt in range(self.watch_config.checksum_parameters.max_retries):
            ...
            logging.info(
                {
                    "Action": "Copy complete. Validating checksum",
                    "Source": src,
                    "Destination": dest,
                    "Attempt": attempt + 1,
                }
            ...
            if Path(src).is_dir():
                ...
                src_checksum = self.calculate_crc32c(src)
                dest_checksum = self.calculate_crc32c(dest_file)
    ...
    def run_job(self) -> None:
        ...
        try:
            ...
            if self.config.delete_modalities_source_after_success:
                ...
                logging.info({"Action": "Modality source deleted"} | self.config.log_tags, extra={'weblog':True})
