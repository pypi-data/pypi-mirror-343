...
\paragraph*{Medicare Costs}
	...
	MAGI, we simply use $G_{(n-2)} + Q_{(n-2)} + e_{(n-2)}$, i.e., gross taxable ordinary income
	...
	\begin{figure}[t]
	    ...
	    \caption{\small Modeling a piecewise constant function with special ordered set of type 1 (SOS1).
	...
	We use binary variables $z_{qn}^m \in \{0, 1\}$ with the constraints
	\begin{equation}
		\label{Eq:SOS1}
		\sum_q z_{qn}^m = 1,
	\end{equation}
	...
	\end{equation}
	as only one $z_{qn}^m$ can be non-zero for a given $n$.
        The determination of which $z_{qn}^m$ is active is achieved by enforcing the following
	$(N_n - n_m +1)$ constraints
	\begin{equation}
		\label{Eq:MedBracket}
		\sum_{q\neq 0} z_{qn}^m \bar{\mathcal{L}}_{qn} \le G_{(n-2)} + Q_{(n-2)} + e_{(n-2)}
		   < \sum_{q\neq N_q -1} z_{qn}^m \bar{\mathcal{L}}_{(q+1)n},
	\end{equation}
        for all years $n \ge n_m$ when Medicare is required.
	The end points $\mathcal{L}_0 = 0$ and $\mathcal{L}_{N_q} = \mathcal{M}$ are not included
        as they generate constraints
	that are true for all values considered and do not bring additional information.
	Eq.~(\ref{Eq:MedBracket}) merely states that the MAGI from two years before
	...
	Eq.~(\ref{Eq:MedBracket}) constraints can also be implemented individually
	with $(N_q -1)(N_n - n_m + 1)$ constraints as
	\begin{eqnarray}
		\label{Eq:MedBracket2}
		z_{qn}^m \bar{\mathcal{L}}_{qn} &\le& G_{(n-2)} + Q_{(n-2)} + e_{(n-2)},
			\qquad q\neq 0, \nonumber\\
		z_{qn}^m \bar{\mathcal{L}}_{(q+1)n} + (1 - z_{qn}^m)\mathcal{M} &>& G_{(n-2)} + Q_{(n-2)} + e_{(n-2)},
			\qquad q \neq N_q-1, \nonumber\\
