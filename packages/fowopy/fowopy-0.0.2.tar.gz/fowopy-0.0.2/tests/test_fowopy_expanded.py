...
Typical usage example:
    ...
    pytest test_pyfowo.py

"""

config = FOWOConfig(backoff=False)
...
def test_environment_variable():
    ...
    # Retrieve the environment variable or use default
    fowo_base_url = os.getenv("FOWO_API_BASE_URL") or "http://0.0.0.0:8000"

    # Print the value of the environment variable
    print(f"API_BASE_URL: {fowo_base_url}")
    # Check if the environment variable is set to a valid URL
    assert fowo_base_url.startswith("http://") or fowo_base_url.startswith("https://")
...
def test_bulk_player_file_parquet():
    ...
    config = FOWOConfig(bulk_file_format="parquet")
    client = FOWOClient(config)
