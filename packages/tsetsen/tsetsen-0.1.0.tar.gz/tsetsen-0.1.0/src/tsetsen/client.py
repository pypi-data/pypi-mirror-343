...
try:
    ...
    if 'runtime_version' in str(e):
        ...
        if os.path.exists(pb2_path):
            ...
            raise ImportError("Generated protobuf files not found. Run script/generate_grpc.py first.")
    else:
        raise ImportError(
...
class Client:
    ...
    def __init__(
        ...
        self.endpoint = endpoint or os.environ.get("TSETSEN_API_ENDPOINT", self.DEFAULT_ENDPOINT)
    ...
    def generate_speech(
        ...
        Raises:
            ...
            TsetsenError: For other API errors.
        """
        logger.debug(f"Generating speech for text ({len(text)} chars) with voice {voice_id}...")
    ...
    def check_status(self, request_id: str) -> CheckStatusResponse:
        ...
        try:
            ...
            self._handle_grpc_error(e, f"Error checking status for request {request_id}")
    ...
    def get_user_balance(self) -> GetUserBalanceResponse:
        ...
        try:
            # Convert to gRPC request
            grpc_request = pb2.GetUserBalanceRequest(
                api_key=request.api_key
            )
