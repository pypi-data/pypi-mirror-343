...
def _apply_gaussian_convolution(
    df: pl.DataFrame,
    fwhm: float,
    grid_points: int = 500,
...
def create_simulated_spectrum_chart(
    ...
    _gaussians, _grid = _apply_gaussian_convolution(
        bar_spectrum,
        fwhm,
        grid_points,
        energy_column,
    ...
    _simulated_spectrum_df = _compute_total_spectrum(_gaussians, _grid)
    ...
    _sum_chart = (
        ...
        .mark_line()
        .encode(x=energy_column, y=intensity_column)
    )

    final_chart = alt.layer(*_individual_charts, _sum_chart).resolve_scale(
        color="independent"
    )
