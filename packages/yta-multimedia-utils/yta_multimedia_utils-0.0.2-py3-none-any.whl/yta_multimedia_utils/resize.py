...
def get_cropping_points_to_keep_aspect_ratio(
    ...
    """
    if (
        not PythonValidator.is_tuple(original_size) or
        not PythonValidator.is_tuple(new_size) or
        len(original_size) != 2 or
        len(new_size) != 2
    ):
        raise Exception('The provided "original_size" and/or "new_size" parameters are not tuples of 2 elements.')
    
    if (
        not NumberValidator.is_positive_number(original_size[0]) or
        not NumberValidator.is_positive_number(original_size[1]) or
        not NumberValidator.is_positive_number(new_size[0]) or
        not NumberValidator.is_positive_number(new_size[1])
    ):
...
def get_size_to_keep_aspect_ratio(
    ...
    if (
        not NumberValidator.is_positive_number(size[0]) or
        not NumberValidator.is_positive_number(size[1])
    ):
