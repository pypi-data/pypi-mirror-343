...
from LabToolbox import my_cov, my_mean, my_var, my_line, y_estrapolato, n_cifre, riscrivi_con_cifre_decimali, format_value_auto, propagate_uncertainty
...
def model_fit(x, y, sy, f, p0, sx = None, xlabel="x [ux]", ylabel="y [uy]", showlegend = True, legendloc = None, 
    ...
    for i, param in enumerate(popt):
        digits = np.abs(n_cifre(errors[i])) + 1
        if n_cifre(errors[i]) >= 0:
            sigma1 = round(errors[i], -int(digits) + 2)
        else:
            sigma1 = round(errors[i], digits)
        mean1 = riscrivi_con_cifre_decimali(sigma1, param)
