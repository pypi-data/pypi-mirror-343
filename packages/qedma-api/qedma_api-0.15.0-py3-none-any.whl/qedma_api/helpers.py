...
def adapt_to_backend(  # type: ignore[no-any-unimported]
    ...
    *,
    ...
    for q in _observables_to_qubits(observables):
        qmap.setdefault(q, q)
...
def _get_transpiled_qmap(  # type: ignore[no-any-unimported]
    ...
    for q in _observables_to_qubits(transpiled_observables):
...
def _pauli_to_qubits(pauli: models.Pauli) -> list[int]:
    return [int(q) for q in re.findall(r"[XYZ](?P<qubit>[0-9]+)", pauli)]


def _observables_to_qubits(observables: list[models.Observable]) -> set[int]:
    all_qubits = set()

    for o in observables:
        o_qubits = set().union(*(_pauli_to_qubits(p) for p in o.root.keys()))
        all_qubits.update(o_qubits)
...
def _remove_idle_wires(  # type: ignore[no-any-unimported]
    ...
    observables_qubits = _observables_to_qubits(observables)
    qc_out = qc.copy()
