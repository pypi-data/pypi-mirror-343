...
class Genmst:
    ...
    def insert_or_update_genmst(self, column, connection, con_type):
        ...
        if missing_keys:
            ...
            SYSTEM_EVENT_SERVICES_COUNT = cursor.fetchone()[0]
            logger.log(f"Inside GEN SYSTEM_EVENT_SERVICES {SYSTEM_EVENT_SERVICES_COUNT}")
            cursor.close()

            if SYSTEM_EVENT_SERVICES_COUNT <= 0:
                logger.log(f"insert in SYSTEM_EVENT_SERVICES")   
            ...
            SYSTEM_SERVICE_ARGS_COUNT = cursor.fetchone()[0]
            logger.log(f"Inside GEN SYSTEM_SERVICE_ARGS {SYSTEM_SERVICE_ARGS_COUNT}")
            cursor.close()

            if SYSTEM_SERVICE_ARGS_COUNT <= 0:
                logger.log(f"insert in SYSTEM_SERVICE_ARGS")  
