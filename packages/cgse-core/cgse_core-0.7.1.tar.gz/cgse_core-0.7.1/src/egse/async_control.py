...
CONTROL_SERVER_DEVICE_COMMANDING_PORT = 55557
CONTROL_SERVER_SERVICE_COMMANDING_PORT = 55558
...
async def is_control_server_active(endpoint: str = None, timeout: float = 0.5) -> bool:
    ...
    with AsyncControlClient(endpoint) as client:
                    ...
                    message = zmq_error_response(
                        {
                            ...
                            'metadata': {
                                'data': data.decode(),
                                'file': filename, 'lineno': lineno, 'function': function_name
                            },
                        }
                    )
...
class AsyncControlClient:
    ...
    async def ping(self) -> str | None:
        response = await self._send_request(self.service_command_socket, "ping")
        if response['success']:
...
async def control_server_test():
    ...
    response = await client.do({"command": "xxx"})
    print(f"command xxx: {response = }")

    is_active = await is_control_server_active(endpoint=f'tcp://localhost:{CONTROL_SERVER_SERVICE_COMMANDING_PORT}')
