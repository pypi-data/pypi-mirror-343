...
def generate_info_json(filepath: str, prompt: str):
    """
    ...
    img_meta = get_image_metadata(str(filepath)) if filepath.exists() else {"dimensions": {"width": 0, "height": 0}, "file_size": 0.0}
...
def get_seo_data(info,
    ...
    if thumbnail_score:
        ...
        basename = os.path.basename(info['thumbnail']['file_path'])
        filename,ext = os.path.splitext(basename)
