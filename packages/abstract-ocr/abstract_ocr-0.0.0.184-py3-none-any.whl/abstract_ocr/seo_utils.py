...
def generate_with_bigbird(text: str, task: str = "title", model_dir: str = "allenai/led-base-16384") -> str:
    try:
        ...
        return ""
def generate_media_url(fs_path: str,domain=None,repository_directory=None) -> str | None:
    fs_path = os.path.abspath(fs_path)
    if not fs_path.startswith(repository_directory):
        return None
    rel_path = fs_path[len(repository_directory):]
...
def generate_info_json(filepath=None,
    ...
    url_path = filepath
    title_prompt = metadata.get('title') or generate_with_bigbird(f"Video of {filename.replace('-', ' ')} with the video text f{alt_text}", task="title")
    description_prompt = metadata.get('description') or generate_with_bigbird(f"Video of {filename.replace('-', ' ')} with the video text f{alt_text}", task="description")
    caption_prompt = metadata.get('caption') or generate_with_bigbird(f"Video of {filename.replace('-', ' ')} with the video text f{alt_text}", task="caption")
    ...
    caption = alt_text
    if generator:
        gen = generator(description_prompt, max_length=100, num_return_sequences=1)[0]
        description = gen.get('generated_text', '')[:150]
        gen = generator(title_prompt, max_length=100, num_return_sequences=1)[0]
        title = gen.get('generated_text', '')[:150]
        gen = generator(caption_prompt, max_length=100, num_return_sequences=1)[0]
        caption = gen.get('generated_text', '')[:150]
    ...
    info = {
        ...
        "schema": {
            ...
            "url": generate_media_url(filepath,domain=domain,repository_directory=repository_dir),
            "contentUrl": generate_media_url(video_path,domain=domain,repository_directory=repository_dir),
            "width": width,
            "height": height,
        ...
        "social_meta": {
            "og:image": generate_media_url(filepath,domain=domain,repository_directory=repository_dir),
            "og:image:alt": alt_text,
            "twitter:card": "summary_large_image",
            "twitter:image": generate_media_url(filepath,domain=domain,repository_directory=repository_dir)
        }
    }
    return info
