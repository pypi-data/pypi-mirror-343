...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/test_salt_winrepo/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/test_salt_winrepo/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/test_salt_winrepo/*"
            )
            ...
            #    "--include=src/saltext/test_salt_winrepo/*,tests/*"
            # )
        finally:
