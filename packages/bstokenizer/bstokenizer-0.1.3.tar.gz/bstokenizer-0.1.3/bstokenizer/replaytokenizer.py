...
class BeatSaberReplayTokenizer:
    ...
    def tokenize(
        self,
        replay_data: Union[Bsor, bytes, BinaryIO, str],
        ...
        if "controller_offsets" not in exclude and bsor_data.controller_offsets:
            if not isinstance(
                bsor_data.controller_offsets, list
            ):  # Handle different types based on Bsor implementation
                controller_token = self._process_controller_offset(
                    bsor_data.controller_offsets
                )
    ...
    def _process_vr_object(self, vr_obj: VRObject) -> Dict:
        ...
        """
        return {
            "position": {"x": vr_obj.x, "y": vr_obj.y, "z": vr_obj.z},
            "rotation": {
                "x": vr_obj.x_rot,
                "y": vr_obj.y_rot,
                "z": vr_obj.z_rot,
    ...
    def _process_note(self, note: Note) -> Dict:
        ...
        token = {
            ...
            "scoringTypeName": lookup_dict_scoring_type.get(
                note.scoringType, "Unknown"
            ),
    ...
    def detokenize(self, tokens: List[Dict]) -> Bsor:
        ...
        if TokenType.CONTROLLER_OFFSET.value in token_groups:
            token = token_groups[TokenType.CONTROLLER_OFFSET.value][
                0
            ]  # Should only be one
            if "raw" in token and isinstance(token["raw"], ControllerOffsets):
                ...
                controller_offsets.right = self._create_vr_object(
                    token.get("right", {})
                )
