...
class BeatSaberMapTokenizer:
    ...
    def _seconds_to_beats(self, seconds: float) -> float:
        ...
        for i, (change_beat, change_bpm) in enumerate(sorted_changes):
            ...
            change_time = prev_time + ((change_beat - time_changes[-1][0]) * 60 / time_changes[-1][2])
    ...
    def tokenize(self, map_data: Union[str, Dict], exclude: Optional[Set[str]] = None) -> List[Dict]:
        """
        ...
        self.version = map_data.get("_version", None) # _version is used in v2 maps, we'll have to convert.
        ...
        if "colorNotes" not in exclude and "colorNotes" in map_data:
            for note in map_data["colorNotes"]:
                ...
                processed_note = customDataHandler.parse_custom_data(TokenType.COLOR_NOTE, note)
                tokens.append({
        ...
        if "bombNotes" not in exclude and "bombNotes" in map_data:
            for bomb in map_data["bombNotes"]:
                ...
                processed_bomb = customDataHandler.parse_custom_data(TokenType.BOMB_NOTE, bomb)
                tokens.append({
        ...
        if "obstacles" not in exclude and "obstacles" in map_data:
            for obstacle in map_data["obstacles"]:
                ...
                processed_obstacle = customDataHandler.parse_custom_data(TokenType.OBSTACLE, obstacle)
                tokens.append({
                    ...
                    "duration_seconds": (processed_obstacle["d"] * 60) / self._get_bpm_at_beat(processed_obstacle["b"]),
        ...
        if "sliders" not in exclude and "sliders" in map_data:
            for slider in map_data["sliders"]:
                ...
                processed_slider = customDataHandler.parse_custom_data(TokenType.SLIDER, slider)
                tokens.append({
        ...
        if "burstSliders" not in exclude and "burstSliders" in map_data:
            for burst in map_data["burstSliders"]:
                ...
                processed_burst = customDataHandler.parse_custom_data(TokenType.BURST_SLIDER, burst)
                tokens.append({
        ...
        if "basicBeatmapEvents" not in exclude and "basicBeatmapEvents" in map_data:
            for event in map_data["basicBeatmapEvents"]:
                ...
                processed_event = customDataHandler.parse_custom_data(TokenType.BASIC_EVENT, event)
                tokens.append({
                    "type": TokenType.BASIC_EVENT.value,
                    "beat": processed_event.get("b", event.get("b", 0)),
                    "time": self._beats_to_seconds(processed_event.get("b", event.get("b", 0))),
                    ...
                    "custom_data": processed_event.get("customData", event.get("customData", {})),
