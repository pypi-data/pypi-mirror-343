...
class MarkdownSOPParser(SOPParserBase):
    ...
    def parse(self, file_path: str) -> Optional[SOPDocument]:
        ...
        try:
            ...
            # Extract references
            references = []
            refs_match = re.search(r'^## References\s+([\s\S]+?)(?=^##|$)', content, re.MULTILINE)
            if refs_match:
            ...
            # Extract attachments
            attachments = []
            attach_match = re.search(r'^## Attachments\s+([\s\S]+?)(?=^##|$)', content, re.MULTILINE)
            if attach_match:
                attach_text = attach_match.group(1).strip()
                for line in attach_text.split('\n'):
                    if line.strip().startswith('- '):
            ...
            print(f"Created document with title: {doc.title} and {len(doc.steps)} steps")
            return doc
