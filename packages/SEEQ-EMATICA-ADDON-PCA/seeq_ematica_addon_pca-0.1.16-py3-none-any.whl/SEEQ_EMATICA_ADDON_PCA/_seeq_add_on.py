...
class PCA_Widget_Addon:
    ...
    def __init__(self, host, url, workbook_id, worksheet_id):
        ...
        self.error_plot = go.Figure(data=[trace], layout=layout)
        ...
        self.barchart = go.Figure(data=[trace], layout=layout)
            ...
            for iteration, X in enumerate(X_table_test):
                try:
                    ...
                    layout = go.Layout(
                        ...
                        xaxis=dict(title='Observation Index', type='date'),
                        yaxis=dict(title="Hotelling's T^2 Contribution"))

                    # Create figure and display it
                    hotel = go.Figure(data=[trace], layout=layout)
                    ...
                    x_labels = [str(name) for name in original_names]
                    y_values = [float(val) for val in bar_var]
                    trace = go.Bar(
                        x=x_labels,  # Categories
                        y=y_values,  # Values
                        marker=dict(color='blue')  # Optional: Set bar color
                    )
                    ...
                    barchart = go.Figure(data=[trace], layout=layout)
