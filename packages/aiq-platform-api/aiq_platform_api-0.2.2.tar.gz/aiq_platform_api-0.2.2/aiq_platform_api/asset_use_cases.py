...
def list_asset_tags(
    client: AttackIQRestClient, asset_id: str, limit: Optional[int] = 10
):
    logger.info(f"Listing up to {limit} tags for asset with ID: {asset_id}")
    tag_count = 0

    for tagged_item in TaggedItemUtils.get_tagged_items(
        client, "asset", asset_id, limit=limit
    ):
        tag_count += 1
...
def main():
    ...
    if asset_id:
        ...
        logger.warning(
            "ATTACKIQ_ASSET_ID environment variable is not set. Skipping asset-specific operations."
        )
