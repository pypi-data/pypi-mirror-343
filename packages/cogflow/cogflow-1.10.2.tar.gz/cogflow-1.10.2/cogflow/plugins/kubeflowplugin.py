            ...
            def wrapped_func(*args, **kwargs):
                ...
                try:
                    ...
                    for attempt in range(10):
                        pod = v1.read_namespaced_pod(name=pod_name, namespace=namespace)
                        run_id = pod.metadata.labels.get("pipeline/runid")
                        if run_id:
                            print(f"✅ run_id fetched on attempt {attempt + 1}: {run_id}")
                            return run_id

                        print(f"⏳ run_id not found yet (attempt {attempt + 1}), retrying in {5}s...")
                        time.sleep(5)
                    print("podname", pod_name)
                    print("ns", namespace)
                    print("pod yaml", pod)
