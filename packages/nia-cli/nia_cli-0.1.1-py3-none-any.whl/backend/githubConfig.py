...
def get_installation_token(installation_id: str) -> str:
    ...
    This is a SYNCHRONOUS function - DO NOT use with 'await'!
    ...
    IMPORTANT: This is a synchronous function. Do not use with 'await'.
    """
    app_id = os.getenv("GITHUB_APP_ID")
    private_key = os.getenv("GITHUB_APP_PRIVATE_KEY")
    
    if not app_id or not private_key:
        raise ValueError("GITHUB_APP_ID and GITHUB_APP_PRIVATE_KEY environment variables must be set")
    
    try:
        # Create a JWT to authenticate as the app
        now = int(time.time())
        payload = {
            "iat": now,
        ...
        logging.info(f"Requesting installation token for installation_id: {installation_id}")
        resp = requests.post(url, headers=headers)
        resp.raise_for_status()
        
        data = resp.json()
        logging.info("Successfully obtained installation token")
        return data["token"]
        
    except Exception as e:
        logging.error(f"Error getting installation token: {str(e)}")
        raise

def list_installation_repos(installation_token: str) -> list:
    """
    List repositories accessible to a GitHub App installation.
    
    Args:
        installation_token: The installation access token
