...
from .inputs import ListArtifactoryItemsInput, SearchArtifactoryItemsInput, GetArtifactoryItemInfoInput
...
def artifactory_list_items(repository: str, path: str = "/") -> Union[List[Dict[str, Any]], Dict[str, str]]:
    ...
    try:
        ...
        # Build the API URL
        api_url = f"{ARTIFACTORY_URL.rstrip('/')}/api/storage/{input_data.repository}{clean_path}"
        ...
        if response.status_code == 200:
            ...
            if "children" in data:
                ...
                result = {"uri": data.get("uri", ""), "created": data.get("created", ""), "size": data.get("size", 0)}
...
def artifactory_search_items(query: str, repositories: Optional[List[str]] = None) -> Union[List[Dict[str, Any]], Dict[str, str]]:
    ...
    """
    logger.debug(f"artifactory_search_items called with query: '{query}', repositories: {repositories}")
...
def artifactory_get_item_info(repository: str, path: str) -> Union[Dict[str, Any], Dict[str, str]]:
