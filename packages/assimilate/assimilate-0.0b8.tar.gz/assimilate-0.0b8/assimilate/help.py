...
class Overview(HelpMessage):
    DESCRIPTION = "overview of assimilate"

    @staticmethod
    def help():
        text = dedent(
            """
            Assimilate is a simple command line utility to orchestrate backups.
            It is built on Borg, which is a powerful and fast de-duplicating
            backup utility for managing encrypted backups, however it can be
            rather clumsy to use directly.  With Assimilate, you specify all the
            details about your backups once in advance, and then use a very
            simple command line interface for your day-to-day activities.  The
            details are contained in ~/.config/assimilate.  That directory will
            contain a file (settings) that contains shared settings, and then
            another file for each of your backup configurations.

            Use of Assimilate does not preclude the use of Borg directly on the
            same repository.  The philosophy of Assimilate is to provide
            commands that you would use often and in an interactive manner with
            the expectation that you would use Borg directly for the remaining
            commands.  In fact, Assimilate provides the borg command to make
            that easier.
            """
        ).strip()
        return text
...
class Precautions(HelpMessage):
    DESCRIPTION = "what everybody should know before using assimilate"
    ...
    def help():
        text = dedent(
            ...
            its permissions so that it is not readable by others:
                ...
                chmod 700 ~/.config/assimilate/settings
            ...
            built in to Assimilate, but its use is optional (it need not be
            installed).
