...
from tqsdk_zq.utils import check_subprocess_running, run_subprocess_async, kill_subprocess
...
def init_zq_server_config():
    ...
    tmp_path = config.ZQ_CONFIG_ZQ_SERVER_FILE.with_suffix('.tmp')
    with open(tmp_path, 'w') as f:
        json.dump(zq_server_config, f, indent=4)

    os.replace(tmp_path, config.ZQ_CONFIG_ZQ_SERVER_FILE)
