...
def update_cluster_status(cluster_ip: str, status: str):
    with Spinner(text=f"Updating {cluster_ip} to {status}..."):
        success, message = change_cluster_status(cluster_ip, status)
...
def create_cluster():
    with Spinner(text="Creating cluster..."):
        success, message = create_new_cluster()
...
def main():
    ...
    off_parser.add_argument("cluster_ip", help="IP address of the cluster to turn off")
    ...
    #
        ...
        if args.cluster_command == "list":
            ...
            return update_cluster_status(args.cluster_ip, "OFF")
