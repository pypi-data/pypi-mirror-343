...
from fabric_cli.core.fab_hiearchy import FabricElement, Item, OneLakeItem, Workspace


def exec_command(
    args: Namespace, from_context: FabricElement, to_context: FabricElement
) -> None:
    args.from_path = from_context.get_path()
    args.to_path = to_context.get_path()
...
def _move_workspace_items_if_types_match(
    ...
    wstype_from = from_context.get_ws_type()
    wstype_to = to_context.get_ws_type()
...
def _move_item_if_types_match(
    ...
    if from_context.get_item_type() == to_context.get_item_type():
