...
from fabric_cli.core.fab_hiearchy import FabricElement, Item, OneLakeItem
...
def extract_paths(context: FabricElement) -> tuple:
    path_id = context.get_path_id().lstrip("/")
    path_name = context.get_path().lstrip("/")
...
def sort_items_by_config(ws_items: list) -> list:
    if (
        ...
        return sorted(
            ws_items,
            key=lambda item: (str(item.get_item_type().name), item.get_short_name()),
        )
    return sorted(ws_items, key=lambda item: item.get_short_name())
