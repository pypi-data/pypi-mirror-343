...
def add_item_props_to_args(args: Namespace, context: Item) -> None:
    ...
    args.item_id = context.get_id()
    args.item_type = context.get_type()
    args.item = context.get_name()
    args.jobType = context.get_job_type().value
...
def _process_config(args: Namespace, item: Item) -> None:
    if getattr(args, "config", None):
        if item.get_job_type() != FabricJobType.RUN_NOTEBOOK:
            raise FabricCLIError(
                f"Configuration payload not supported for job type: {item.get_job_type().value}.",
                fab_constant.ERROR_NOT_RUNNABLE,
            )
