...
def get_command_context(local_path, raise_error=True, supports_local_path=False):
    ...
    try:
        ...
        if path == "/" or path == "/..":
            ...
            local_context = process_relative_path(
                Context().get_context(), path, raise_error)
        else:
        ...
        if supports_local_path and e.status_code in [fab_constant.ERROR_NOT_FOUND, fab_constant.ERROR_INVALID_PATH, fab_constant.ERROR_NOT_SUPPORTED]:
            utils_ui.print_grey(
            ...
            if os.path.exists(local_path) and os.access(local_path, os.R_OK):
                return LocalPath(None, None, local_path)
            # IF the parent directory of the local path exists in the local file system, return the local path
            elif not raise_error and os.path.exists(os.path.dirname(local_path) and os.access(os.path.dirname(local_path), os.W_OK)):
                return LocalPath(None, None, local_path)
    ...
    match path_part:
        ...
        case x if re.match("\\.\\.\\.+", x):
            ...
            raise FabricCLIError(
                "Invalid path", fab_constant.ERROR_INVALID_PATH)
        case _:
...
def process_context_path(
    ...
    match cur_ctxt.get_type():
        ...
        case FabricElementType.ONELAKE:
...
def process_absolute_path(path: str, raise_error=True) -> FabricElement:
    ...
    path_parts = deque(x.replace("\\", "").strip()
                       for x in re.split(r"(?<!\\)/", path))
...
def process_relative_path(context: FabricElement, path: str, raise_error=True) -> FabricElement:
    """Process relative path and return the corresponding context."""

    # Get the size of the context stack
    _ctxt_size = 1
    _ctxt = context
    while _ctxt.get_type() != FabricElementType.TENANT:
        _ctxt = _ctxt.get_parent()
        _ctxt_size += 1
    ...
    context_stack: deque[FabricElement] = deque(
        maxlen=_ctxt_size+len(path_parts))
    ...
    while context.get_type() != FabricElementType.TENANT:
...
def _handle_path_in_item(
    ...
    if folder is None:
        raise FabricCLIError(
            f"Folder '{path_part}' not found in item '{item.get_name()}'. Valid folders are: {item_folders}",
...
def _handle_path_in_onelake(
    path_part, path_parts, onelake_resource: OneLakeItem, context: deque[FabricElement], raise_error
):
    _path_part = utils.remove_dot_suffix(path_part)
    _path = f"{onelake_resource.get_path_id()}/{_path_part}".lstrip("/")
    ...
    if response is None:
        ...
        if not raise_error and len(path_parts) == 0:
            ...
            return OneLakeItem(_path_part, None, onelake_resource, OneLakeItemType.UNDEFINED)
        else:
            raise FabricCLIError(
    ...
    if response.status_code == 200:
        ...
        match response.headers.get("x-ms-resource-type", "undefined"):
            case "directory":
                ...
                elif onelake_resource.get_local_path() == LakehouseFolders.TABLES.value:
...
def _get_onelake_details(path: str):
    ...
    match item_type:
        case VirtualWorkspaceItemType.CAPACITY:
            _capacity_id = _get_capacity_id(
                cur_ctxt.get_tenant(
                ), f"{item_name}.{str(item_type)}", raise_error
            )
    ...
    match item_type:
        ...
        case VirtualItemType.MANAGED_PRIVATE_ENDPOINT:
            ...
            managed_private_endpoint = VirtualItem(
                item_name, _managed_private_endpoint_id, cur_ctxt, str(
                    item_type)
            )
