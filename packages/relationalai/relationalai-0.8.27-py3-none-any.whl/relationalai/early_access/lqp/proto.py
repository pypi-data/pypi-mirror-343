...
__pb_RelationSig = getattr(logic_pb2, "RelationSig")

# Converts an LQP program to an LQP proto Transaction.
def lqp_program_to_proto(defs: Sequence[ir.Def]):
            ...
            __pb_Epoch(
                    ...
                    __pb_Write(
                        define = __pb_Define(
                            fragment = __pb_Fragment(
                                ...
                                declarations = [
                                    __as_declaration(lqp_node_to_proto(d)) for d in defs
                                ],
                            )
                        )
                    )
                ...
                reads = [
                    __pb_Read(
                        output = __pb_Output(
                            name = "output",
                            relation_id = lqp_node_to_proto(decl).name,
                        )
                    ) for decl in defs
                ],
            )
...
def lqp_node_to_proto(node: Union[ir.LqpNode, ir.PrimitiveType]):
    ...
    #     Ignore: we match on the "concrete" children of Formula.
        ...
        return __pb_RelAtom(
            sig = lqp_node_to_proto(node.sig),
            terms = [__as_term(lqp_node_to_proto(term)) for term in node.terms],
        )
    ...
    #     Ignore: we match on the "concrete" children of Term.
        ...
        return __pb_Var(
            name = node.name,
            type = lqp_node_to_proto(node.type),
        )
        ...
        return __pb_RelationSig(
            name = node.name,
            types = [
                lqp_node_to_proto(t) for t in node.types
            ],
        )
