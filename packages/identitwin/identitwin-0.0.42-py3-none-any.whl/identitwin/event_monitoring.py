...
# Replace GPIO import with conditional import
try:
    ...
    print("Running in development mode - using GPIO mock")
...
class EventMonitor:
    ...
    def __init__(self, config, data_queue, thresholds, running_ref, event_count_ref):
        ...
        self.activity_led_pin = 18  # Activity LED on pin 18
        self.setup_led()
    ...
    def setup_led(self):
        ...
        try:
            ...
            GPIO.setup(self.activity_led_pin, GPIO.OUT)
            # Ensure activity LED is off at startup
            GPIO.output(self.activity_led_pin, GPIO.LOW)
            # Add a state variable to track LED status
            state.set_event_variable("activity_led_active", False)
        except Exception as e:
            logging.error(f"Error setting up LED control: {e}")
    ...
    def _handle_event_trigger(self, sensor_data, current_time, accel_magnitudes, disp_values):
        ...
        try:
            ...
            if not self.in_event_recording:
                ...
                # Activate activity LED after trigger
                state.set_event_variable("activity_led_active", True)
                # Start a thread to blink the activity LED
                threading.Thread(
                    target=self._blink_activity_led,
                    daemon=True
                ).start()
    ...
    def _blink_activity_led(self):
        """Blink the activity LED while event recording is active"""
        try:
            while state.get_event_variable("is_event_recording") and state.get_event_variable("activity_led_active"):
                GPIO.output(self.activity_led_pin, GPIO.HIGH)
                time.sleep(0.5)
                GPIO.output(self.activity_led_pin, GPIO.LOW)
                time.sleep(0.5)
        except Exception as e:
            logging.error(f"Error in LED blinking: {e}")
        finally:
    ...
    def _finalize_record_event(self, complete_event_data, start_time):
        ...
        try:
            ...
            self.in_event_recording = False
            state.set_event_variable("is_event_recording", False)
            # Turn off activity LED blinking
            state.set_event_variable("activity_led_active", False)
