...
class MonitoringSystem:
    ...
    def _print_status(self, sensor_data):
        ...
        if self.config.enable_accel:
            ...
            if accel_list and any(isinstance(a, dict) for a in accel_list):
                for i, accel in enumerate(accel_list):
                    if isinstance(accel, dict):
                        ...
                        if not (np.isnan(x) and np.isnan(y) and np.isnan(z)):
                            print(f"  Accel{i+1}: X={x:.3f} Y={y:.3f} Z={z:.3f} (Mag: {mag:.3f}) m/s²")
                        else:
                            print(f"  Accel{i+1}: No valid reading")
            elif hasattr(self, 'accelerometers') and not self.accelerometers:
                print("  No accelerometer sensors initialized")
            else:
                print("  Waiting for next accelerometer reading...")
