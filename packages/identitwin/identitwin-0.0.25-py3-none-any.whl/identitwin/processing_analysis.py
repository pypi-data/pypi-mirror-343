...
def calculate_fft(data, sampling_rate):
    ...
    if not all(key in data for key in ['x', 'y', 'z']):
        print("Warning: Missing acceleration components for FFT calculation.")
        return np.array([]), np.array([]), np.array([]), np.array([])

    # Get lengths and ensure data exists
    lengths = [len(data[key]) for key in ['x', 'y', 'z']]
    if not all(lengths):
        print("Warning: Empty acceleration data for FFT calculation.")
        return np.array([]), np.array([]), np.array([]), np.array([])

    n = lengths[0]  # Use length of x component
    n_fft = 2**int(np.ceil(np.log2(n)))  # Next power of 2

    # Create window for the FFT length, not the original data length
    window = np.hanning(n_fft)
    
    fft_results = {}
    for axis in ['x', 'y', 'z']:
        # Zero pad data to n_fft length first
        padded_data = np.zeros(n_fft)
        padded_data[:n] = data[axis]
        
        # Then apply window to the padded data
        windowed_data = padded_data * window

        # Calculate FFT and normalize
        fft_result = np.fft.rfft(windowed_data)
        # Normalize by window sum and n_fft
        fft_magnitude = 2.0 * np.abs(fft_result) / (np.sum(window) * n_fft)
