...
def calculate_fft(data, sampling_rate):
    ...
    n = len(data['x'])  # Use length of x component
    window = np.hanning(n)
    
    fft_results = {}
    for axis in ['x', 'y', 'z']:
        # Apply window to the data
        data_windowed = data[axis] * window
        # Calculate FFT
        fft_result = np.fft.rfft(data_windowed)
        # Get magnitude spectrum, properly normalized
        fft_magnitude = 2.0 * np.abs(fft_result) / n
        fft_results[axis] = fft_magnitude

    # Calculate frequency array only up to Nyquist frequency
        ...
        for accel_idx in range(config.num_accelerometers):
            ...
            try:
                ...
                if fft_results_list and len(fft_results_list) > accel_idx:
                    ...
                    # Plot FFT data in log scale with appropriate filtering
                    for data, color, label in [
                        (current_fft['fft_x'], 'r', 'X'),
                        (current_fft['fft_y'], 'g', 'Y'),
                        (current_fft['fft_z'], 'b', 'Z')
                    ]:
                        # Filter out frequencies below 0.1 Hz and add small offset to avoid log(0)
                        mask = freqs > 0.1
                    ...
                    ax_fft.set_xlim(0.1, config.sampling_rate_acceleration/2)  # From 0.1 Hz to Nyquist frequency
                    ax_fft.set_ylim(1e-6, 1e2)  # Reasonable amplitude range
