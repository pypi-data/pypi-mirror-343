...
# Modify print based on I2C_AVAILABLE
print(f"Hardware detection: {'Raspberry Pi/Hardware Mode' if I2C_AVAILABLE else 'Simulation Mode / Missing Libraries'}")
...
class SystemConfig:
    ...
    def __init__(
        ...
        # Force disable sensors if hardware libs are missing but they were requested
        if not I2C_AVAILABLE:
            if self.enable_lvdt:
                print("Warning: LVDT enabled but hardware libraries are missing. Disabling LVDT.")
                self.enable_lvdt = False
            if self.enable_accel:
                print("Warning: Accelerometer enabled but hardware libraries are missing. Disabling Accelerometer.")
                self.enable_accel = False

        self.num_lvdts = num_lvdts if self.enable_lvdt else 0 # Adjust count if disabled
        self.num_accelerometers = num_accelerometers if self.enable_accel else 0 # Adjust count if disabled
    ...
    def initialize_leds(self):
        ...
        # Add early return if I2C/gpiozero is not available
    ...
    def create_ads1115(self):
        ...
        # Add early return if I2C or required libs are not available
        ...
        try:
            ...
            print("Attempting to initialize I2C bus for ADS1115...") # Added log
            i2c = busio.I2C(board.SCL, board.SDA)
            print("I2C bus initialized successfully for ADS1115.")
            # Create ADS object using the initialized bus
            print(f"Attempting to create ADS1115 object at default address 0x48...") # Added log
            ads = ADS.ADS1115(i2c)
            ...
            try:
                # Attempt to read a value (e.g., from P0) to confirm communication
                # This might raise an OSError if the device is not responding
                print("Attempting test read from ADS1115 channel P0...")
                test_channel = AnalogIn(ads, ADS.P0)
                _ = test_channel.value # Read the value
                print("ADS1115 communication check successful.")
            except Exception as comm_err:
                 print(f"Error during ADS1115 communication check: {comm_err}. Device might not be responding correctly.", file=sys.stderr)
                 # Optionally return None here if communication fails
                 # return None
            # *** End communication check ***
    ...
    def create_lvdt_channels(self, ads):
        ...
        try:
            ...
            for i in range(self.num_lvdts):
                ...
                print(f"  Creating AnalogIn for pin {ch_pin}...") # Added log
                channel = AnalogIn(ads, ch_pin)
                # *** Add communication check for each channel ***
                try:
                    print(f"    Attempting test read from LVDT channel {i+1} (pin {ch_pin})...")
                    _ = channel.value # Read the value
                    print(f"    LVDT channel {i+1} communication check successful.")
                    channels.append(channel)
                    print(f"  LVDT channel {i+1} created on pin {ch_pin}.")
                except Exception as ch_err:
                    print(f"    Error during LVDT channel {i+1} communication check: {ch_err}. Skipping this channel.", file=sys.stderr)
                    # Continue to try creating other channels
                # *** End communication check ***

            print(f"Successfully created {len(channels)} LVDT channel objects.")
            return channels if channels else None # Return None if no channels were created
        except NameError as e:
    ...
    def create_accelerometers(self):
        ...
        for i in range(self.num_accelerometers):
            ...
            try:
                ...
                try:
                    print(f"    Attempting test read (temperature) from MPU6050 at {hex(addr)}...") # Added log
                ...
                # *** Add check for get_accel_data method ***
                except AttributeError as attr_err:
                    print(f"  Error: The mpu6050 object at {hex(addr)} does not have the expected 'get_temp' method: {attr_err}. Check library version.", file=sys.stderr)
                    continue # Skip this sensor
                # *** End check ***
