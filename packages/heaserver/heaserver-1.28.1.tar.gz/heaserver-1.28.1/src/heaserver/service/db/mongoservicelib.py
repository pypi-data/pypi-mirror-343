    ...
    if cached_value is not None:
        ...
        async with MongoContext(request, volume_id) as mongo:
            ...
            result = await mongo.get(request, collection, var_parts='id',
                                     sub=sub, groups=await context.get_groups())
            ...
            if result is not None:
                ...
                permitted = await obj_.has_permissions(perms=RESTPermissionGroup.GETTER_PERMS, context=context)
                if not permitted:
                    return None
...
async def get_content(request: Request, collection: str, volume_id: str | None = None) -> StreamResponse:
    ...
    async with MongoContext(request, volume_id) as mongo:
        context = mongo.get_default_permission_context(request)
        result = await mongo.get(request, collection, var_parts='id', sub=sub, groups=await context.get_groups())

        if result is not None:
            obj = heaobject.root.desktop_object_from_dict(result)
            permitted = await obj.has_permissions(perms=RESTPermissionGroup.GETTER_PERMS, context=context)
            if not permitted:
                return response.status_not_found()
        out = await mongo.get_content(request, collection, var_parts='id', sub=sub)
        if out is not None:
...
async def delete(request: Request, collection: str, volume_id: str | None = None) -> Response:
    ...
    async with MongoContext(request, volume_id) as mongo:
        sub = request.headers.get(SUB, NONE_USER)
        context = mongo.get_default_permission_context(request)
        obj = await mongo.get(request, collection, var_parts='id', sub=sub, groups=await context.get_groups())
        ...
        if obj is not None:
            ...
            if not permitted:
                return response.status_forbidden()
            delete_result = await mongo.delete(request, collection, var_parts='id', sub=sub,
                                               groups=await context.get_groups())  # if lacks permissions, then deletes no records.
...
async def _get_dict(request: Request, collection: str, volume_id: str | None = None, mongo: Mongo | None = None,
    ...
    if cached_value is not None:
        ...
        async def fetch_from_mongo(mongo: Mongo) -> DesktopObjectDict | None:
            ...
            result = await mongo.get(request, collection, var_parts='id',
                                     sub=context_.sub, groups=await context_.get_groups())

            if result is not None:
                obj = heaobject.root.desktop_object_from_dict(result)
                permitted = await obj.has_permissions(perms=RESTPermissionGroup.GETTER_PERMS, context=context_)
                if not permitted:
                    return None
                request.app[HEA_CACHE][cache_key] = result
                return obj.to_dict()
...
async def _get_all_desktop_objects_gen(request: Request, collection: str, *,
    ...
    async def fetch_objects(mongo: Mongo) -> AsyncGenerator[DesktopObject, None]:
        context_ = mongo.get_default_permission_context(request) if context is None else context
        gen = mongo.get_all(request, collection, mongoattributes=mongoattributes, sub=context_.sub, sort=sort,
                            groups=await context_.get_groups())
        try:
            async for desktop_object_dict in gen:
                obj = desktop_object_from_dict(desktop_object_dict, type_=type_)
                if await obj.has_permissions(perms=RESTPermissionGroup.GETTER_PERMS, context=context_):
                    yield obj
        finally:
            await gen.aclose()
