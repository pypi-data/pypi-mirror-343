...
class DataFrameFilter:
    ...
    @staticmethod
    def match_common_column(filter_df, target_df, slice_length=6):
        col_mapping = {}
    ...
    @classmethod
    def by_boolean(cls, boolean_df, target_df, tobe_masked, mode='include', slice_length=6):
        ...
        if not cls.compare_column_length(boolean_df, target_df):
            matched_mask = cls.match_common_column(result_mask, target, slice_length)
            result_mask = cls.reindex_df(matched_mask, target)
        else:
            result_mask = cls.reindex_df(result_mask, target)
        
        return result_mask * target
    ...
    def by_gics(cls, gics_df, target_df, tobe_masked, mode='include', slice_length=6):
        ...
        if not cls.compare_column_length(gics_df, target_df):
            matched_mask = cls.match_common_column(result_mask, target, slice_length)
            result_mask = cls.reindex_df(matched_mask, target)
        else:
            final_mask = cls.reindex_df(result_mask, target)
