...
def fetch_rpm_header(url: str, end: str, retries: int = 2, backoff_factor: int = 2):
    """
    ...
    """
    logger.debug(f"++ CALLED fetch_rpm_header(url={url}, end={end})")
    for attempt in range(retries):  # [0 1] for retries = 2
        try:
            # TODO: enable auth via password-store for Stable updates.
...
def get_zypper_changelog(repo_list=[], pkg_list=[], only_updates: bool = True,
    ...
    for cached_repo_alias, zypp_cache_file in cache_files_dict.items():
        ...
        for pkg_name, pkg_info in cached_pkg_info.items():
            ...
            rpm_header_content = fetch_rpm_header(rpm_header_url, pkg_info["header-end"])
...
def main():
    ...
    parser.add_argument(
        '-re', '--regex', dest='reges', default=False, action='store_true',
    ...
    get_zypper_changelog(repo_list=repo_list, pkg_list=pkg_list, only_updates=not args.all,
                         titles_only=args.titles_only, regex_enabled=args.reges)
