...
import subprocess
import cloudscraper
from miyuki.logger import logger  # Reverted
from miyuki.config import MOVIE_SAVE_PATH_ROOT, RECORD_FILE, MAGIC_NUMBER # Reverted
# from miyuki.http_client import HttpClient # Reverted
from miyuki.url_sources import SingleUrlSource, PlaylistSource, AuthSource, SearchSource, FileSource, AutoUrlSource # Reverted
from miyuki.video_downloader import VideoDownloader # Reverted
from miyuki.utils import delete_all_subfolders, ThreadSafeCounter # Reverted
...
def main():
    ...
    logger.info(f"Parsed arguments: {args}") # Slightly more descriptive log
    validate_args(args)
    ...
    scraper = cloudscraper.create_scraper(
        ...
        delay=10 # Add a small delay between requests to Cloudflare if needed
    )
    logger.info("Initialized cloudscraper")

    # http_client = HttpClient()
    movie_counter = ThreadSafeCounter()
    # --- Logging Source Instantiation ---
    source = None
    if args.auto:
        logger.info(f"Creating AutoUrlSource with URLs: {args.auto}, Limit: {args.limit}")
        # AutoUrlSource creates its own client internally, need to modify it too (see below)
        # OR pass the scraper in
        source = AutoUrlSource(movie_counter=movie_counter, auto_urls=args.auto, limit=args.limit, scraper=scraper) # Pass scraper
    elif args.auth:
        logger.info(f"Creating AuthSource with User: {args.auth[0]}, Limit: {args.limit}")
        # AuthSource creates its own client, needs modification or passing scraper
        source = AuthSource(movie_counter=movie_counter, username=args.auth[0], password=args.auth[1], limit=args.limit, scraper=scraper) # Pass scraper
    elif args.search:
        logger.info(f"Creating SearchSource with Key: {args.search}")
        # SearchSource creates its own client, needs modification or passing scraper
        source = SearchSource(movie_counter=movie_counter, key=args.search, scraper=scraper) # Pass scraper
    elif args.file:
        logger.info(f"Creating FileSource with Path: {args.file}, Limit: {args.limit}")
        # FileSource uses AutoUrlSource, so the change there will cover it if scraper is passed.
        source = FileSource(movie_counter=movie_counter, file_path=args.file, limit=args.limit, scraper=scraper) # Pass scraper

    if not source:
        logger.error("No valid source mode specified or source instantiation failed.") # Changed error message
        exit(MAGIC_NUMBER)
    else:
        logger.info(f"Successfully instantiated source: {type(source).__name__}")

    # --- Logging URL Fetching ---
    logger.info("Attempting to fetch URLs using source.get_urls()...")
    movie_urls = source.get_urls()
    logger.info(f"source.get_urls() returned: {movie_urls} (Count: {len(movie_urls)})")

    if not movie_urls:
        logger.error("No URLs to download after calling source.get_urls().") # Changed error message
        exit(MAGIC_NUMBER)

    # --- Logging Download Options ---
    ...
    logger.info(f"Downloader options set: {options}")

    # --- Loop and Download ---
    logger.info(f"Starting download loop for {len(movie_urls)} URLs...")
    for url in movie_urls:
        ...
        try:
            ...
            # Pass the scraper instead of the old http_client
            downloader = VideoDownloader(url, scraper, options) # <-- Use scraper here
            downloader.download()
