...
class PublishHistory:
    ...
    def click_publish_tab(self) -> None:
        ...
        self.publish_tab = all_publish.click.for_new_tab()
        # 隐藏浏览器窗口:pip install pypiwin32
        if self.hide_browser:
            print('隐藏浏览器窗口...')
            self.publish_tab.set.window.hide()
    ...
    def parse_articles(self) -> List[dict]:
        ...
        while True:
            ...
            print(f'\n====================第 {page_num} 页====================')
            for div in self.publish_tab.s_eles('@class=weui-desktop-block__main'):
                try:
                    # 跳过广告文章
            ...
            try:
                with open(self.save_file, 'w', encoding='utf-8') as f:
                    json.dump(publish_articles, f, ensure_ascii=False, indent=4)
            except Exception as e:
                print(f"保存文章数据出错: {e}")

            try:
                if self.publish_tab.ele('下一页'):
                    page_num += 1
                    self.publish_tab.ele('下一页').click()
                    time.sleep(0.5)
                else:
                    break
            except Exception as e:
        ...
        print(f'共爬取 {len(publish_articles)} 篇文章!')
