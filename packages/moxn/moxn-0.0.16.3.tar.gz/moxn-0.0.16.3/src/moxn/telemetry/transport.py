...
class TelemetryTransportBackend(Protocol):
    ...
    async def log_telemetry_event_and_get_signed_url(
...
class APITelemetryTransport:
    ...
    async def _send_log_with_external_attributes(
        ...
        signed_url_response = await self.backend.log_telemetry_event_and_get_signed_url(
            signed_url_request
        )
    ...
    async def send_telemetry_log_and_get_signed_url(
        ...
        """Passthrough for the signed-URL handshake."""
        return await self.backend.log_telemetry_event_and_get_signed_url(
            signed_url_request
        )
