...
def calculate(*args):
    ...
    # 执行计算
    x = np.array(nums).reshape(-1, feature_num())
    x_std = sc_X['std_param'].transform(x)
    y_std = svr_model['model'].predict(x_std)
    y = sc_y['std_param'].inverse_transform(y_std.reshape(1, -1))[0][0]
    if y <= 330:
        predict_vol.set(f"预测电压：{round(y)} 伏")
        print(f'{svr_model['name']}模型预测值: {y}伏')
    else:
        predict_vol.set(f"{svr_model['name']}模型预测失败！！！")
...
# 加载支持向量机模型
with open('../../res/opto_predict_svr.pkl', 'rb') as f:
    svr_model = pickle.load(f)

# 创建主窗口
root = tk.Tk()
root.title('北扶-电压预测小工具')
...
entry_input = tk.Entry(root, textvariable=input_var, width=50, justify='center', font=('Arial', 12))
...
tk.Label(fr_model_info, text='Author: '+svr_model['author']).grid(row=0, column=0)
tk.Label(fr_model_info, text='Algorithm: '+svr_model['name']).grid(row=0, column=1, padx=30)
tk.Label(fr_model_info, text='Update: '+str(svr_model['timestamp'].replace(microsecond=0))).grid(row=0, column=2)
