...
class ReplayBuffer:
    def __init__(
        ...
        if not self.namespace:
            ...
            self.namespace = user_profile.handle

            if not self.namespace:
                self.namespace = user_profile.email.replace("@", "-").replace(".", "-")
        ...
        self.buffer = next(
            (
                ...
                if b.metadata.name == self.name
                and b.metadata.namespace == self.namespace
            ),
            None,
        )
