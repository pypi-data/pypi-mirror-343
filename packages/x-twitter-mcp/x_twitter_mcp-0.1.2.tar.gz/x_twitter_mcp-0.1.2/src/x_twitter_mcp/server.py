...
async def run_server():
    ...
    await server.run()
    logger.info(f"{server.name} finished.")

def main():
    """Entry point for running the server, handling asyncio event loop correctly."""
    try:
    ...
    # If we're in an Emscripten environment or the loop is already running, schedule the coroutine
    import platform
    if platform.system() == "Emscripten" or loop.is_running():
        logger.info("Scheduling run_server on existing loop")
        asyncio.ensure_future(run_server())
    else:
        logger.info("Running run_server in new loop")
