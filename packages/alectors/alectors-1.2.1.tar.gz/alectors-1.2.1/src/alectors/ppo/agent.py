...
class Agent:
    ...
    def learn(self):
        ...
        for _ in tqdm(range(self.epochs), disable=not self.verbose, desc="Learning Epoch", leave=False):
            state_arr, action_arr, old_probs_arr, vals_arr, reward_arr, truncated_arr, terminated_arr, batches = self.memory.recall()
            ...
            for t in range(len(reward_arr)-1):
                ...
                for k in range(t, len(reward_arr)-1):
                    a_t += discount*(reward_arr[k]+self.gamma*vals_arr[k+1]\
                            *(1-int(truncated_arr[k] or terminated_arr[k]))-vals_arr[k])
