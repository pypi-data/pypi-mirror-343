...
from ._ErrorHandler import _check_shape_, WrongParameter
import numpy as np
import warnings

from ._ResultsClasses import DesaturationsMeasuresResults, DesatMethodEnum
...
class DesaturationsMeasures:
    ...
    def __desaturation_detector(self, signal):
        ...
        table_desat_aa_new = table_desat_aa - 1
        self.begin = table_desat_aa_new
                    ...
                    if data[bb] <= data[bb - 1]:
                        if data[aa] - data[bb] >= thres:
                            ...
                            while cc < lg_dat and out_c == 0:
                                if ((data[aa] - data[cc]) <= 0 or (data[cc] - data[bb]) >= thres) and cc - aa >= 10:
