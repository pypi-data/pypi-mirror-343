...
app = typer.Typer(help="TLOPack - A command-line interface for TLO searches")
console = Console()

def get_admin_key() -> str:
    key_file = os.path.expanduser("~/.tlopack")
...
def search(
    ...
    with console.status("[bold green]Searching..."):
        try:
            ...
            if not key:
                console.print("[red]Error: Admin key not provided. Either use --admin-key or run 'tlopack config' first.")
                raise typer.Exit(1)
