...
def get_approximate_xray_total_rdf(
    ...
    # Form factors
    element_electrons: dict[str, float] = {}
    for element in unique_elements:
        a = torch.tensor(form_factor_params[element]["a"], device=r_bins.device)
        c = form_factor_params[element]["c"][0]

        element_electrons[element] = float(c + torch.sum(a))

    sum_of_electrons = sum(element_fractions[element] * element_electrons[element] for element in unique_elements)

    scaled_form_factors = {
        element: element_fractions[element] * element_electrons[element] / sum_of_electrons for element in unique_elements
    }

    total_rdf = torch.zeros_like(r_bins)
    for element_1, element_2 in combinations_with_replacement(unique_elements, 2):
        positions_1 = positions[element_masks[element_1]]
        scaled_form_factor_1 = scaled_form_factors[element_1]
        ...
        coefficient = scaled_form_factor_1 * scaled_form_factor_2
