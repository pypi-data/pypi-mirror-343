...
from servermind_cli.translator import translate_to_command
...
def main(input_text, preview, yes, explain, show_info, show_command, show_result, show_error, show_log, show_all):
    ...
    try:
        ...
        # 사용자 요청 출력
        if show_info:
            print(f"[bold blue]INFO[/bold blue] 요청: '{text}'")
        ...
        if not commands:
            ...
            print(f"[bold red]ERROR[/bold red] {error_msg}", err=True)
            sys.exit(1)
        ...
        if show_info:
            if len(commands) == 1:
                print(f"[bold blue]INFO[/bold blue] 변환된 명령어: '{commands[0]}'")
            else:
                print(f"[bold blue]INFO[/bold blue] 변환된 명령어 {len(commands)}개:")
