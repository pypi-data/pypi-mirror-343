...
@LastEditTime: 2025-04-25 10:07:57
...
def filter_check_current_limit(handler_name=None, current_limit_count=0, limit_params_dict={}):
    ...
    :param limit_params_dict: 限制扩展信息{"limit_type":限制类型：1-每分钟UV计数, 2-每分钟计数}
    :last_editors: HuangJianYi
    """
    def check_current(handler):
        def wrapper(self, **args):
            ...
            if safe_config.get("is_current_control", False) == True: # 是否开启流量控制 0-关闭 1-开启
                ...
                limit_type = limit_params_dict.get("limit_type", 1)
                if limit_count == 0:
                ...
                if limit_count > 0:
                    if limit_type == 1:
                        if SafeHelper.check_current_limit(app_id, limit_count, handler_name=handler_name, object_id=object_id) == True:
                            self.response_json_error("current_limit", "当前人数过多,请稍后再试")
                            return
                        if handler_name:
                            SafeHelper.add_current_limit_count(app_id, object_id, handler_name)
                    else:
                        request_queue_name = f"request_queue:appid_{app_id}"
                        if handler_name:
                            request_queue_name += f":handlername_{handler_name}"
                        if SafeHelper.check_current_limit_by_time_window(request_queue_name, limit_count, 60) == True:
                            self.response_json_error("current_limit", "当前人数过多,请稍后再试")
                            return
