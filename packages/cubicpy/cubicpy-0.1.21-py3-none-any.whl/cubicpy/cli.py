...
MESSAGES = {
    'ja': {
        ...
        'external_help': '外部接続モードで起動',
        'room_help': 'WebSocketサーバーのルーム名（デフォルト: 1000）',
    ...
    'en': {
        ...
        'external_help': 'Start in external connection mode',
        'room_help': 'WebSocket server room name (default: 1000)',
...
def main():
    ...
    parser.add_argument('--room', default="1000",
                        help=msgs['room_help'])
    ...
    # 外部接続モードの場合
    if args.external:
        from cubicpy.websocket import start_websocket_server
        ...
        print(f"外部接続モードで起動します (ルーム: {args.room})")
        start_websocket_server(DummyPhysicsEngine(), default_room=args.room)
        return 0
