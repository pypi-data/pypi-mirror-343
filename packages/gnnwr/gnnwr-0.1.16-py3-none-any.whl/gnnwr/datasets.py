...
def MinkowskiDistance(x, y, p=1.45):
    """
    ...
    :return: distance matrix
    """
    return np.float32(np.sum(np.abs(x[:, np.newaxis, :] - y) ** p, axis=2) ** (1 / p))
...
def init_dataset_split(train_data, 
    ...
    if use_model == "gnnwr":
        train_dataset.distances, val_dataset.distances, test_dataset.distances = _init_gnnwr_distance(
            ...
            test_data[spatial_column].values, spatial_fun, params
        )
...
# TODO: fix comment
def _init_gnnwr_distance(refer_data, train_data, val_data, test_data, spatial_fun=BasicDistance, params=None):
    r"""
    Parameters
    ----------
    ...
    """
    if spatial_fun is MinkowskiDistance:
        assert params is not None, "params must be a dict for MinkowskiDistance"
        train_distance = spatial_fun(train_data, refer_data, params['p'])
        val_distance = spatial_fun(val_data, refer_data, params['p'])
        test_distance = spatial_fun(test_data, refer_data, params['p'])
    else:
