...
class Tilemap:
    def __init__(self, app, size: list[int], tilesize: int=32, grid_color: list[int]=[10, 10, 10]) -> None:
        ...
        self.image = pg.Surface(self.sizep)
    ...
    def export_data(self, path: str) -> bool:
        ...
        for layer in self.layers:
            ...
            for tile in self.layers[layer][1]:
                ...
                index = pos[1] * self.sizet[0] + pos[0]
                pos = f"{pos[0]},{pos[1]}"
                save_data["layers"][layer][pos] = self.layers[layer][0][index]
            ...
            for layer in layers:
                for pos, data in layers[layer].items():
                    ...
                    if data[0] != -1 and data[0] != None:
                        self.set_tile(layer, data[0], data[1], [pos[0] * tilesize, pos[1] * tilesize])
        return True
    ...
    def set_data(self, layer: str, tileset: int, tile_id: int, pos: list[int]) -> None:
        if tileset < 0 or tileset >= len(self.tilesets): return
        if not self.layers.get(layer, False): return
        pos = [
            int(pos[0] - self.pos[0]),
            int(pos[1] - self.pos[1])
        ]
        mapx = pos[0] // self.tilesize
        mapy = pos[1] // self.tilesize
    ...
    def get_data(self, layer: str, pos: list[int]) -> int|str|None:
        if not self.layers.get(layer, False): return
        pos = [
            int(pos[0] - self.pos[0]),
            int(pos[1] - self.pos[1])
    ...
    def get_tile(self, layer: str, pos: list[int]) -> int:
        if not self.layers.get(layer, False): return
        pos = [
            int(pos[0] - self.pos[0]),
            int(pos[1] - self.pos[1])
    ...
    def set_tile(self, layer: str, tileset: int, tile_id: int, pos: list[int]) -> None:
        if tileset < 0 or tileset >= len(self.tilesets): return
        if not self.layers.get(layer, False): return
        if self.get_tile(layer, pos) is not None: return
        ...
        mapx = (pos[0] * self.tilesize) // self.tilesize
        mapy = (pos[1] * self.tilesize) // self.tilesize
        ...
        tile = Object(
            size=[self.tilesize, self.tilesize], color=[
                ...
                (pos[0] // self.tilesize) * self.tilesize,
                (pos[1] // self.tilesize) * self.tilesize
            ]
        )
    ...
    def get_region(self, layer:str, size:list[int], pos:list[int]) -> list[Object]|None:
        if not self.layers.get(layer, False): return
