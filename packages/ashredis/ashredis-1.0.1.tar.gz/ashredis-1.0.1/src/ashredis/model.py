...
class RedisObject:
    ...
    def __init__(self, host: str = None, port: int = None, password: str = None, db: int = None, key: str | int = None,
                 path: list[str] = None):
    ...
    def get_dict(self) -> dict:
        ...
        return {k: json.loads(v) if self._fields[k] is dict else v for k, v in self._data.items()}
    ...
    def copy(self, category: RedisObject):
        """Copy data from another category."""
        if not isinstance(category, RedisObject):
    ...
    async def save(self, key: str | int = None, stream: bool = False, ttl: timedelta = None):
        ...
        if stream:
            pipeline.xadd(
                f"{self.__category__}{self._path_str}{STREAM_KEY}",
