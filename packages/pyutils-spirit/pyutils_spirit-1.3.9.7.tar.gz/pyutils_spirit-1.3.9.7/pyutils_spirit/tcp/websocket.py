...
class WebSocket(ABC):
        ...
        while True:
            try:
                ...
                if cls.__handshaking(socket_client=socket_client, ws_headers=ws_headers):
                    endpoint = ws_endpoint_cls()
                    cls.__container.set_holder(key=str(addr), value=endpoint)
                    session = cls.Session(socket_client=socket_client,
                                          address=addr,
                                          headers=ws_headers,
                                          params=params,
                                          onclose_func=endpoint.ws_funcs["onclose"],
                                          partial_data=endpoint.partial_data,
                                          message_buffer=endpoint.message_buffer)
                    info(f"Session: {session} connection successful")
                    cls.__executor.execute(task=cls.__listener_message, endpoint=endpoint, session=session)
                    if endpoint.ws_funcs["onopen"] is not None:
                        endpoint.ws_funcs["onopen"](endpoint, session=session)
            except Exception as e:
                exception(e)

    @classmethod
    def __receive_message(cls, endpoint, session: Session):
        # 接收消息
        socket_client: socket.socket = session.get_socket_client()
        session_id = id(socket_client)
        data = endpoint.partial_data.get(socket_client, b'') + socket_client.recv(cls.__buffer_size)
            ...
            while len(data) >= 2:
                ...
                if opcode in (0x8, 0x9, 0xA):
                    ...
                    if opcode == 0x8:  # Close帧
                        ...
                        continue
                # 7. 处理数据帧
                if session_id not in endpoint.message_buffer:
                    endpoint.message_buffer[session_id] = {'fragments': [], 'opcode': opcode}
                else:
                    if opcode != 0x0:
                        raise ValueError("分片连续性错误")
                endpoint.message_buffer[session_id]['fragments'].append(decoded)
                if fin:
                    full_data = b''.join(endpoint.message_buffer[session_id]['fragments'])
                    original_opcode = endpoint.message_buffer[session_id]['opcode']
                    del endpoint.message_buffer[session_id]
                    # 处理数据
                    if original_opcode == 0x1:
                        ...
                        result = full_data
                    data = data[mask_start + 4 + payload_len:]
                    endpoint.partial_data[socket_client] = data
                    return result
                else:
                    data = data[mask_start + 4 + payload_len:]
    ...
    @classmethod
    def __listener_message(cls, endpoint, session: Session) -> None:
        while session.get_is_connected():
            try:
                message: str = cls.__receive_message(endpoint=endpoint, session=session)
                if message is None:
                    continue
                elif message == "ping":
                    ...
                    __onmessage: callable = endpoint.ws_funcs["onmessage"]
                    if __onmessage is not None:
                        __onmessage(self=endpoint, message=message)
            except Exception as e:
                exception(e)
                __onerror: callable = endpoint.ws_funcs["onerror"]
                if __onerror is not None:
                    try:
                        __onerror(endpoint, session=session, ex=e)
                    except Exception as e:
                        exception(e)
