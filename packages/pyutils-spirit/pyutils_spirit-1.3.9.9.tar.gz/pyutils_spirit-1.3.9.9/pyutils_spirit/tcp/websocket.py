...
class WebSocket(ABC):
            ...
            try:
                ...
                try:
                    ...
                    if ws_endpoint_cls is None:
                        raise ValueError(f"WebSocket URL({lines[0].split()[1]}) cannot match endpoint signature")
                ...
                if cls.__handshaking(socket_client=socket_client, ws_headers=ws_headers):
                    ...
                    cls.__executor.execute(task=cls.__listener_message, __endpoint=__endpoint, session=session)
    ...
    @classmethod
    def __receive_message(cls, __endpoint, session: Session):
        # 接收消息
        socket_client: socket.socket = session.get_socket_client()
        session_id = id(socket_client)
        data = __endpoint.partial_data.get(socket_client, b'') + socket_client.recv(cls.__buffer_size)
            ...
            while len(data) >= 2:
                ...
                if opcode in (0x8, 0x9, 0xA):
                    ...
                    if opcode == 0x8:  # Close帧
                        ...
                        continue
                # 7. 处理数据帧
                if session_id not in __endpoint.message_buffer:
                    __endpoint.message_buffer[session_id] = {'fragments': [], 'opcode': opcode}
                else:
                    if opcode != 0x0:
                        raise ValueError("分片连续性错误")
                __endpoint.message_buffer[session_id]['fragments'].append(decoded)
                if fin:
                    full_data = b''.join(__endpoint.message_buffer[session_id]['fragments'])
                    original_opcode = __endpoint.message_buffer[session_id]['opcode']
                    del __endpoint.message_buffer[session_id]
                    # 处理数据
                    if original_opcode == 0x1:
                        ...
                        result = full_data
                    data = data[mask_start + 4 + payload_len:]
                    __endpoint.partial_data[socket_client] = data
                    return result
                else:
                    data = data[mask_start + 4 + payload_len:]
    ...
    @classmethod
    def __listener_message(cls, __endpoint, session: Session) -> None:
        while session.get_is_connected():
            try:
                message: str = cls.__receive_message(__endpoint=__endpoint, session=session)
                if message is None:
                    continue
                elif message == "ping":
                    ...
                    __onmessage: callable = __endpoint.ws_funcs["onmessage"]
                    if __onmessage is not None:
                        __onmessage(self=__endpoint, message=message)
            except Exception as e:
                exception(e)
                __onerror: callable = __endpoint.ws_funcs["onerror"]
                if __onerror is not None:
                    try:
                        __onerror(self=__endpoint, session=session, ex=e)
                    except Exception as e:
                        exception(e)
