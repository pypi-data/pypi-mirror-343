...
class ChatManagerClient(AbstractClient):
    ...
    def get_domains(self, request_id: str = "") -> List[DictStr]:
        response: DomainsResponse = self._stub.GetDomains(DomainsRequest(RequestId=request_id))
        res = [{"DomainId": di.DomainId, "Name": di.Name} for di in response]
        return res

    def get_tracks(self, request_id: str = "") -> List[DictStr]:
        response: TracksResponse = self._stub.GetTracks(TracksRequest(RequestId=request_id))
