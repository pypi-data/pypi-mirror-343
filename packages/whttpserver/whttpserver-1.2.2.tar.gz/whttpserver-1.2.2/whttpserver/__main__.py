...
def list_directory_contents(req_path, directory):
    ...
    for entry in entries:
        ...
        try:
            ...
            print("Failed to get owner name for uid: " + str(stat_info.st_uid))
        ...
        if file_type == "dir":
            ...
            if entry.split('.')[-1] in ['html', 'txt', 'sh', 'scala', 'py', 'js', 'css', 'json', 'md', 'yml', 'yaml', 'ini','log']:
                action = "edit"
            else:
                action = "download"
...
def dir_listing(req_path):
    ...
    return render_template('index.html', data=data, the_path=the_path, req_path=req_path)
