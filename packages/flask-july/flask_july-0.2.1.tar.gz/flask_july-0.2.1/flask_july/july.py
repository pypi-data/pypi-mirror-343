from flask import Response, request, session, redirect, Flask, render_template_string, jsonify, send_file
import hashlib
import random
import string
...
<body class="anti-bot">
    ...
    <p>If you're <i>still</i> not redirected, please disable all browser extensions for this site. Should that not work, please contact the site administrator.</p>
    <p>Scraper-Protection by <a href="/July.x">July</a> from Within</p>
</body>
</html>
"""
...
class Flask_July:
    ...
    def __init__(self, app = None, backend=CryptoBackend(), *, enable_robots = False, robotsfile = "robots.txt", create_route = False):
    ...
    def init_app(self, app: "Flask"):
        ...
        if self.app:
            ...
            if self.enable_robots:
                ...
                self.robotparser.parse(open(self.robotsfile, 'r').readlines())
            if self.create_route:
                @self.app.route("/robots.txt")
                def robots_txt():
                    return send_file(self.robotsfile, mimetype="text/plain")
    ...
    def check(self):
        if self.enable_robots:
            print(request.url, request.headers.get("User-Agent"), self.robotparser.can_fetch(request.headers.get("User-Agent"), request.url))
            if self.robotparser.can_fetch(request.headers.get("User-Agent"), request.url):
                return None
