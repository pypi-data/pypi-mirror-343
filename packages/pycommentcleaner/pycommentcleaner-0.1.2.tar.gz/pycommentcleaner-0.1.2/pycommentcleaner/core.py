...
def clean_file(file_path: Union[str, Path], newfile: bool = False) -> Tuple[bool, str]:
    """
    Remove comments from a Python file and save the result.

    Args:
        file_path: Path to the Python file
        newfile: If True, saves the cleaned file as a new file with '_cleaned' suffix.
                 If False, overwrites the original file.
    ...
    try:
        ...
        if newfile:
            output_path = file_path.parent / f"{file_path.stem}_cleaned.py"
        else:
