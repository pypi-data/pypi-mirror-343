...
def ProcesarSubDatos(SubColumnasDatos, countCol, col, row):
    """
    Procesa y extrae datos anidados de estructuras complejas con optimización de rendimiento.
    
    Función altamente optimizada para acceder a datos anidados en estructuras complejas
    (diccionarios y listas anidadas). Utiliza múltiples estrategias para maximizar
    el rendimiento, incluyendo accesos directos para casos comunes, caché de metadatos,
    y mecanismos de fallback para casos excepcionales.
    
    Args:
        SubColumnasDatos: Configuración para acceso a datos anidados (lista/tupla de tuplas)
        countCol: Índice de la columna actual en SubColumnasDatos
    ...
    """
    if not SubColumnasDatos or not isinstance(SubColumnasDatos, (list, tuple)):
        return row.get(col, "")

    if countCol >= len(SubColumnasDatos):
        return row.get(col, "")
    ...
    # get tuple info
    result = {
        'length': len(tup) if isinstance(tup, (list, tuple)) else 0,
        'has_special': False,
        'alt_field': None
    }
    ...
    for field in tup:
        if isinstance(field, str) and '<' in field:
            ...
            result['alt_field'] = field.split('<')[1]
            break
    ...
    try:
        if info.get('has_special') and info.get('alt_field') in row:
            return row[info.get('alt_field')]
        
        if length == 4 and tup[0] in row:
            if isinstance(row[tup[0]], dict) and tup[1] in row[tup[0]]:
                if isinstance(row[tup[0]][tup[1]], dict) and tup[2] in row[tup[0]][tup[1]]:
                    return row[tup[0]][tup[1]][tup[2]]
        
        elif length == 3 and tup[0] in row:
            if isinstance(row[tup[0]], dict) and tup[1] in row[tup[0]]:
                return row[tup[0]][tup[1]]
        
        elif length == 2 and tup[0] in row:
            return row[tup[0]]
    except Exception:
        pass
    ...
    if length > 1:
        try:
            keys = [k for k in tup if isinstance(k, (str, int)) and k != ''][:3]
            return _acceso_seguro(row, keys)
        except Exception:
            pass
...
def TableVacia(NombreColumnas=('Id', 'Email', 'Usuario', 'Clave', 'Edad'), AgregarClass=False,
               ClassColumnas=('Id', 'Email', 'Usuario', 'Clave', 'Edad'), Titulo="Detalle", idtable="table", show_message=True):
    """
    Genera una tabla HTML vacía con las columnas especificadas.
    
    Crea una estructura de tabla HTML sin datos pero con encabezados de columna
    definidos, útil para mostrar cuando no hay resultados o como plantilla inicial.
    
    Args:
        NombreColumnas: Tupla con los nombres de las columnas a mostrar
        AgregarClass: Si se deben agregar clases CSS a los encabezados
        ClassColumnas: Tupla con las clases CSS para cada columna
        Titulo: Título de la tabla
        idtable: ID HTML para la tabla
        show_message: Si se debe mostrar el mensaje "NO SE ENCONTRARON DATOS"
        
    Returns:
        str: HTML generado para la tabla vacía
    """
    ColumnasTH = StringIO()
    
    for countCol, col in enumerate(NombreColumnas):
        if AgregarClass:
            ColumnasTH.write(f'<th class="{ClassColumnas[countCol]}">{col}</th>')
        else:
            ColumnasTH.write(f'<th>{col}</th>')
    ...
    Html = f"""
        ...
        <table class="table table-bordered table-hover table-condensed table-striped" id='{idtable}'>
        <thead>
            <tr>{ColumnasTH.getvalue()}</tr>
        </thead>
        <tbody class="text-center">
            <tr><td colspan="{len(NombreColumnas)}" class="text-center">{'NO SE ENCONTRARON DATOS.' if show_message else ''}</td></tr>
        </tbody>
        </table>
        </div>
        </div>
