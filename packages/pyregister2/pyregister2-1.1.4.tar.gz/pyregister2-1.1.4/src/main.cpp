...
public:
    ...
    std::pair<std::vector<Types>, std::vector<std::string>> execute() {
        ...
        for (auto &op : ops) switch (op.kind) {
            ...
            case OpKind::WriteRSI: asm volatile("movq %0,%%rsi" :: "r"(std::get<uint64_t>(op.value)) : "rsi"); break;
            case OpKind::ReadRSI:  asm volatile("movq %%rsi,%0" : "=r"(t64) : : ); results.push_back(t64); order.push_back("RSI"); break;
            case OpKind::WriteRDI: asm volatile("movq %0,%%rdi" :: "r"(std::get<uint64_t>(op.value)) : "rdi"); break;
            case OpKind::ReadRDI:  asm volatile("movq %%rdi,%0" : "=r"(t64) : : ); results.push_back(t64); order.push_back("RDI"); break;
            case OpKind::WriteRSP: asm volatile("movq %0,%%rsp" :: "r"(std::get<uint64_t>(op.value)) : "rsp"); break;
            case OpKind::ReadRSP:  asm volatile("movq %%rsp,%0" : "=r"(t64) : : ); results.push_back(t64); order.push_back("RSP"); break;
            case OpKind::WriteRBP: asm volatile("movq %0,%%rbp" :: "r"(std::get<uint64_t>(op.value)) : "rbp"); break;
            case OpKind::ReadRBP:  asm volatile("movq %%rbp,%0" : "=r"(t64) : : ); results.push_back(t64); order.push_back("RBP"); break;
            #endif
        }
        ops.clear();
