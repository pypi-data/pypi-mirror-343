...
class ExplainerController:
    ...
    async def get_started(self, user_prompt, file_attachments, focused_files):
        ...
        while True:
            if not isFirst:
                ...
                try:
                    if last_log:
                        ...
                        user_prompt, tier, file_attachments, focused_files = parse_payload(self.repo.get_repo_path(), user_permission)
                        ...
                        user_prompt, _, file_attachments, focused_files = parse_payload(self.repo.get_repo_path(), user_permission)
                        user_prompt = user_prompt.lower()
            ...
            if user_prompt == "h":
                await self.coder.explainer_code_task_pipeline(tier, last_log, "Top notch software engineer", "English", file_attachments, assets_link)
                ...
                if pipeline == "1" or pipeline == 1:
                    ...
                    self.conversation_history = await self.get_file_answer(user_prompt, working_files, role, file_attachments, focused_files, assets_link, crawl_logs)
                    ...
                    self.conversation_history = await self.get_normal_answer(user_prompt, role, file_attachments, focused_files, assets_link, crawl_logs)
