...
class ControllerAgent:
    ...
    async def get_coding_requests(self, instructions, context, file_lists, context_files, role, crawl_logs, file_attachments, assets_link):
        ...
        commits = plan.get('commits', "")
        logger.info("-------------------------------------------------")
        logger.info("#### `SnowX` is starting to code. This may take a few moments...")
        for step in plan.get('steps', []):
            ...
            if self.is_coding_file(file_name):
                ...
                if main_path:
                    ...
                    try:
                        result = await self.coder.get_coding_request(file_name, techStack)
    ...
    async def get_pro_coding_requests(
        ...
        plan = await self.get_taskPlanner_pro(instructions['Implementation_plan'], file_lists)
...
"""
    ...
    async def get_taskPlanner_pro(self, instruction, file_lists):
        """Generate idea plans based on user prompt and available files."""
        return await self.taskPlannerPro.get_task_plans(instruction, file_lists)
    ...
    async def codingProgress(self, tier, finalPrompt, role, file_attachments, focused_files):
        ...
        while True:
            ...
            idea_plan = await self.get_idea_plans(finalPrompt + (f" crawled data: {crawl_logs}" if crawl_logs else ""))
    ...
    async def get_started(self, user_prompt, tier, file_attachments, focused_files):
        ...
        if pipeline in ["2", 2]:
            ...
            await self.image.get_started(tier, user_prompt, file_attachments, focused_files)
