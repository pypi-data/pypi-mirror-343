...
class GitRepo:
    ...
    def __init__(self, repo_path):
        ...
        try:
            # Ensure .gitignore exists at the beginning
            gitignore_path = os.path.join(repo_path, ".gitignore")
            if not os.path.exists(gitignore_path):
                try:
                    with open(gitignore_path, "w") as f:
                        pass
                    logger.info("Created empty .gitignore file during initialization")
                except Exception as e:
                    # Log but don't raise, as we'll try to handle this gracefully
                    logger.error(f"Failed to create .gitignore file: {str(e)}")
            
            # Ensure .zinley folder exists
            zinley_path = os.path.join(repo_path, ".zinley")
            if not os.path.exists(zinley_path):
                try:
                    os.makedirs(zinley_path, exist_ok=True)
                    logger.info("Created .zinley directory")
                except Exception as e:
                    # Log but don't raise, as we'll try to handle this gracefully
                    logger.error(f"Failed to create .zinley directory: {str(e)}")
            ...
            try:
                # Check again because we might have just created it
                if os.path.exists(os.path.join(self.repo_path, ".zinley")):
                    # If we just created the .zinley directory, we need to add all files
                ...
                if need_add_all_files:
                    try:
                        # Check if .gitignore exists first
            ...
            # If it's a gitignore error, try to create the file
            if str(e) == "'gitignore'":
                try:
                    gitignore_path = os.path.join(repo_path, ".gitignore")
                    with open(gitignore_path, "w") as f:
                        pass
                    logger.info("Created empty .gitignore file after initialization error")
                    
                    # Try to initialize again, but catch any errors silently
                    try:
                        self.repo = git.Repo(repo_path, odbt=git.GitDB)
                        self.root = utils.safe_abs_path(self.repo.working_tree_dir)
                        logger.info("Successfully recovered from gitignore error")
                    except Exception as inner_e:
                        logger.error(f"Failed to recover from gitignore error: {str(inner_e)}")
                except Exception as create_e:
                    logger.error(f"Failed to create .gitignore file after error: {str(create_e)}")
    ...
    def append_ignore_file(self, fname):
        ...
        try:
            ...
            self.create_gitignore_safe() and self.append_ignore_file_safe(fname)
        except Exception as e:
    ...
    def ignored_file_update_gitignore(self, fname):
        try:
            ...
            # Check if .gitignore exists first and create it if needed
            gitignore_path = os.path.join(self.root, ".gitignore")
            need_create = not os.path.exists(gitignore_path)
            
            # Add to cache regardless of file existence
            if fname in self.ignore_file_cache:
                result = self.ignore_file_cache[fname]
            else:
                result = self.ignored_file_raw(fname)
                self.ignore_file_cache[fname] = result
            
            # Only try to modify the file if it exists or we successfully create it
            if not need_create or os.path.exists(gitignore_path) or self.create_gitignore_safe():
                self.append_ignore_file_safe(fname)
            
            return result
        except Exception as e:
            logger.error(f"Error in ignored_file_update_gitignore for {fname}: {str(e)}")
            # Even if we encounter an error, still return the result if available
            if fname in self.ignore_file_cache:
                return self.ignore_file_cache[fname]
            return False
    
    def create_gitignore_safe(self):
        """Safely create a .gitignore file without throwing exceptions"""
        try:
            if not hasattr(self, 'root') or not self.root:
                logger.error("Root directory not properly set")
                return False
    ...
    def append_ignore_file_safe(self, fname):
        """Safely append to .gitignore file without throwing exceptions"""
        try:
            if not hasattr(self, 'root') or not self.root:
                logger.error("Root directory not properly set")
                return False
