...
def _get_system_name() -> Optional[str]:
    ...
    if "Linux" == platform_name:
        return f"{platform.system()}-{platform.machine()}-{platform.libc_ver()[0]}"
...
def _get_architecture_name() -> Optional[str]:
    platform_name = platform.system()
    if "Linux" == platform_name:
        return f"{platform.machine()}-{platform.libc_ver()[0]}"
    elif "Darwin" == platform_name:
        return platform.machine()
    elif "Windows" == platform_name:
        return f"{platform.machine()}-{platform.win32_ver()[0]}"
