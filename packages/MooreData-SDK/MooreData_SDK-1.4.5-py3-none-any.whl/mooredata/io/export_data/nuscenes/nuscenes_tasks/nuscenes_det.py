...
class nuscenes_detection():
    ...
    def _get_sensor_mapping_from_moore(self):
        ...
        if 'data' in self.moore_data and len(self.moore_data['data']) > 0:
            if 'info' in self.moore_data['data'][0] and 'info' in self.moore_data['data'][0]['info']:
                if 'imgUrls' in self.moore_data['data'][0]['info']['info'] and len(
                        self.moore_data['data'][0]['info']['info']['imgUrls']) > 0:
    ...
    def _process_camera_data(self, sample: Dict, sensor_token: str, img_url: str, ego_pose_token: str, frame_idx: int,
                             camera_idx: int):
        """处理相机数据"""
    ...
    def _process_single_sequence(self, seq_idx: int, sequence: Dict):
        ...
        for frame_idx in range(num_frames):
            ...
            self._process_lidar_data(sample, self.lidar_sensor_token, info['pcdUrl'][frame_idx], ego_pose_token,
                                     frame_idx)
