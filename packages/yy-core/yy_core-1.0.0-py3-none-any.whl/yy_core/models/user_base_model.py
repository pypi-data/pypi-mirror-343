...
@LastEditTime: 2025-04-23 18:14:01
@LastEditors: yy
@Description: 
"""
from emoji import unicode_codes
from yy_core.libs.customize.cryptography_helper import CryptographyHelper
...
from yy_core.models.db_models.user.user_account_model import *
from yy_core.models.db_models.user.user_info_model import *
from yy_core.models.db_models.user.user_black_model import *
from yy_core.models.db_models.act.act_module_model import *
from yy_core.models.db_models.act.act_prize_model import *
from yy_core.models.db_models.prize.prize_roster_model import *
from yy_core.models.db_models.user.user_address_model import *
...
class UserBaseModel(FrameBaseModel):
    ...
    def __init__(self, context=None, logging_error=None, logging_info=None):
        ...
        self.db_connect_key, self.redis_config_dict = CoreHelper.get_connect_config("db_user","redis_user")
    ...
    def save_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id="", login_ip="", is_update_user_nick=True, mix_nick='', plat_type=0, continue_request_expire=1, is_acquire_lock=False, is_refresh_token=True):
        ...
        if continue_request_expire > 0 and CoreHelper.is_continue_request(f"continue_request:save_user_by_openid:{app_id}_{open_id}", expire=continue_request_expire * 1000) == True:
            invoke_result_data.success = False
        ...
        try:
            ...
            if not user_info:
                ...
                user_info.login_token = CoreHelper.get_random(16)
    ...
    def update_user_info(self, app_id, act_id, user_id, open_id, user_nick, avatar, is_member_before=-1, is_favor_before=-1):
        ...
        try:
            ...
            if not user_info_dict:
                ...
                user_info_dict["modify_date"] = CoreHelper.get_now_datetime()
    ...
    def update_user_state(self, app_id, act_id, user_id, user_state):
        ...
        if user_info.user_state == user_state:
            ...
            return invoke_result_data
        modify_date = CoreHelper.get_now_datetime()
        if user_state == 0:
    ...
    def update_user_state_by_black(self, app_id, act_id, user_id, reason=""):
        ...
        try:
            ...
            if not user_black:
                ...
                user_black.create_date = CoreHelper.get_now_datetime()
    ...
    def audit_user_black(self, app_id, black_id, audit_status, audit_remark=""):
        ...
        if not user_info:
            ...
            invoke_result_data.error_message = "用户信息不存在"
            return invoke_result_data
        audit_date = CoreHelper.get_now_datetime()
    ...
    def get_black_list(self, app_id, act_id, page_size=20, page_index=0, audit_status=-1, user_id=0, start_date="", end_date="", user_nick="", open_id="", order_by="id desc"):
        ...
        for user_black in page_list:
            user_black["refund_order_data"] = CoreHelper.json_loads(user_black["refund_order_data"]) if user_black["refund_order_data"] else []
            ...
            if user_info_dict["user_state"] == 0 and is_black == 1 and refund_count > 0:
                ...
                if len(refund_order_data) >= refund_count:
                    ...
                    if user_black:
                        ...
                        all_refund_order_data = CoreHelper.json_loads(user_black.refund_order_data)
                        if len(refund_order_data) > 0:
                            for item in refund_order_data:
                                all_refund_order_data.append(item)
                        user_black.refund_order_data = CoreHelper.json_dumps(all_refund_order_data)
                        user_black_model.update_entity(user_black)
                    else:
                        user_black = UserBlack()
                        user_black.app_id = user_info_dict["app_id"]
                        ...
                        user_black.audit_remark = ""
                        user_black.refund_count = len(refund_order_data)
                        user_black.refund_order_data = CoreHelper.json_dumps(refund_order_data)
                        user_black.create_date = CoreHelper.get_now_datetime()
                        user_black_model.add_entity(user_black)
                    self._delete_user_info_cache(user_info_dict["act_id"], user_info_dict["id_md5"], open_id=user_info_dict["open_id"])
        except Exception as ex:
    ...
    def save_user_address(self, app_id, act_id, user_id, open_id, user_address_id, real_name, telephone, province, city, county, street, address, is_default, remark=""):
        ...
        if user_address_id > 0:
            ...
            user_address.is_default = is_default
            user_address.remark = remark
            user_address.create_date = CoreHelper.get_now_datetime()
