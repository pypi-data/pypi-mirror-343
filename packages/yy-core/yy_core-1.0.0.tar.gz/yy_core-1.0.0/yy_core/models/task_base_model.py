...
@LastEditTime: 2025-04-23 17:48:02
...
from yy_core.models.db_models.task.task_info_model import *
from yy_core.models.db_models.task.task_count_model import *
from yy_core.models.db_models.invite.invite_log_model import *
from yy_core.models.db_models.collect.collect_log_model import *
from yy_core.models.db_models.task.task_gear_count_model import *
from yy_core.models.db_models.task.task_log_model import *
...
class TaskBaseModel(FrameBaseModel):
    ...
    def __init__(self, context=None, logging_error=None, logging_info=None, check_user_type=UserType.act.value):
        ...
        self.db_connect_key, self.redis_config_dict = CoreHelper.get_connect_config("db_task","redis_task")
    ...
    def get_task_info_dict_list(self, app_id, act_id, module_id, is_release, is_cache=True, source_object_id='', is_del=-1, order_by="sort_index desc,id asc"):
        ...
        for task_info in dict_list:
            task_info["config_json"] = CoreHelper.json_loads(task_info["config_json"]) if task_info["config_json"] else {}
        return dict_list
    ...
    def get_task_asset_type(self, task_asset_type_json, task_type):
        ...
        if task_asset_type_json == "":
            return asset_type
        task_asset_type_dict = CoreHelper.json_loads(task_asset_type_json)
    ...
    def check_task_info(self,act_id,module_id,task_type=0,task_id=0):
        ...
        try:
            ...
            config_json = CoreHelper.json_loads(task_info_dict["config_json"])
            if not config_json:
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
    ...
    def init_task_count(self, task_info_dict, task_count_list, task_sub_type=""):
        ...
        if task_info_dict["complete_type"] == TaskCompleteType.day.value and task_count.modify_day != CoreHelper.get_now_day_int():
    ...
    def add_task_log(self, task_type, task_sub_type, app_id, act_id, module_id, user_id, open_id, user_nick, task_name, log_title, reward_type, reward_object_id, reward_name, reward_num, handler_name, request_code, info_json={}):
        ...
        if task_config.get("is_task_log",False) == True:
            try:
                ...
                task_log.info_json = CoreHelper.json_dumps(info_json) if info_json else {}
                task_log.create_date = CoreHelper.get_now_datetime()
                task_log.create_day = CoreHelper.get_now_day_int()
    ...
    def add_gear_task_count(self, app_id, act_id, module_id, user_id, open_id, task_type, value, remark='', is_async=True):
        ...
        if is_async == True:
            ...
            task_gear_count.create_day = CoreHelper.get_now_day_int()
            task_gear_count.create_month = CoreHelper.get_now_month_int()
            task_gear_count.create_date = CoreHelper.get_now_datetime()
            task_gear_count.remark = remark
            redis_init = CoreHelper.redis_init(config_dict=self.redis_config_dict)
            redis_init.rpush(f"task_gear_list:{str(user_id % 10)}", CoreHelper.json_dumps(task_gear_count))
        else:
    ...
    def add_gear_task_count_to_db(self, app_id, act_id, module_id, user_id, open_id, task_type, value, remark=''):
        ...
        try:
            create_day = CoreHelper.get_now_day_int()
            create_month = CoreHelper.get_now_month_int()
            ...
            acquire_lock_status, task_count_identifier = CoreHelper.redis_acquire_lock(task_count_acquire_lock_name)
                ...
                if config_json and config_json.__contains__("gear_list"):
                    ...
                    for sub_config_json in config_json:
                        ...
                        if now_count >= decimal.Decimal(sub_config_json["gear"]):
                            ...
                            task_count.create_date = CoreHelper.get_now_datetime()
                            task_count.modify_date = CoreHelper.get_now_datetime()
                            task_count.modify_day = CoreHelper.get_now_day_int()
            ...
            if task_count_acquire_lock_name and task_count_identifier:
                CoreHelper.redis_release_lock(task_count_acquire_lock_name,task_count_identifier)
    ...
    def add_task_count(self, app_id, act_id, module_id, user_id, open_id, task_type, value, task_info_dict=None, task_sub_type=''):
        ...
        try:
            ...
            if task_info_dict:
                ...
                now_day = CoreHelper.get_now_day_int()
                task_count.id_md5 = task_count_id_md5
                task_count.app_id = app_id
                task_count.act_id = act_id
                task_count.module_id = module_id
    ...
    def get_gear_task_count(self, app_id, act_id, module_id, user_id, task_type, complete_type):
        ...
        if complete_type == TaskCompleteType.day.value:
            ...
            params.append(CoreHelper.get_now_day_int())
        elif complete_type == TaskCompleteType.week.value:
            ...
            params.append(CoreHelper.get_now_month_int())
        if module_id:
            condition_where.add_condition("module_id=%s")
            params.append(module_id)
    ...
    def convert_gear_task_node(self, app_id, act_id, module_id, user_id, task_info_dict, config_json, task_count_list):
        ...
        :last_editors: yy
        """
        result = deepcopy(self.convert_task_node(task_info_dict["id"],task_info_dict["sort_index"],task_info_dict["task_type"],CoreHelper.get_enum_key(TaskType, task_info_dict["task_type"]),task_info_dict["task_name"],get_ways=1,route_url="receive_reward"))
    ...
    def get_client_task_list(self, app_id, act_id, module_id, user_id, task_types="", app_key="", app_secret="", is_log=False, daily_repeat_browse=False, mix_nick="", order_by="sort_index desc,id asc", browse_site_task_types=None):
        ...
        for task_info_dict in task_info_list:
            ...
            #邀请助力
                ...
                invite_help_list = invite_help_model.get_cache_dict_list("id_md5=%s and user_id=%s and create_day=%s", "", order_by="id desc", field="invited_avatar", params=[help_id_md5, user_id, CoreHelper.get_now_day_int()], dependency_key=DependencyKey.invite_user_help_list(help_id_md5,user_id))
    ...
    def get_client_task_list_v2(self, app_id, act_id, module_id, user_id, task_types="", daily_repeat_browse=False, order_by="sort_index desc,id asc", param_dict={}):
        ...
        for task_info_dict in task_info_list:
            ...
            result = self.convert_task_node(task_info_dict["id"],
                                       ...
                                       CoreHelper.get_enum_key(TaskType, task_info_dict["task_type"]),
            ...
            #邀请新用户
                ...
                invite_log_list = invite_log_model.get_cache_dict_list("act_id=%s and user_id=%s and create_day=%s", "", order_by="id desc", field="invite_avatar", params=[act_id, user_id, CoreHelper.get_now_day_int()], dependency_key=DependencyKey.invite_log_list(act_id,user_id))
            ...
            #邀请加入会员
                ...
                invite_log_list = invite_log_model.get_cache_dict_list("act_id=%s and user_id=%s and create_day=%s", "", order_by="id desc", field="invite_avatar", params=[act_id, user_id, CoreHelper.get_now_day_int()], dependency_key=DependencyKey.invite_log_member_list(act_id,user_id))
                result["data"] = invite_log_list if len(invite_log_list) > 0 else []
            ...
            #购买指定商品
                ...
                limit_num = int(config_json["limit_num"]) if config_json.__contains__("limit_num") else 1
                satisfy_num = int(config_json["satisfy_num"]) if config_json.__contains__("satisfy_num") else 0
                remark = CoreHelper.json_loads(task_count.remark) if task_count.remark else {"goods_id_list":[]}
                result["data"] = config_json["goods_list"] if config_json.__contains__("goods_list") else []
                    ...
                    if acquire_lock_status == False:
                        ...
                        if reward_value <= 0 or task_count.now_count <= 0:
                            ...
                            if task_type == TaskType.invite_new_user.value:
                                invite_log_model = InviteLogModel(db_transaction=db_transaction,context=self.context)
                                #获取邀请用户数
                                if task_info_dict["complete_type"] == 1:
                            ...
                            elif task_type == TaskType.collect_goods.value:
