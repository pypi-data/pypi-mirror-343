...
@LastEditTime: 2025-04-27 16:28:01
...
from yy_core.handlers.filter_base import *
...
from yy_core.models.db_models.operation.operation_log_model import *
...
class FrameBaseApiHandler(BaseApiHandler):
    ...
    def get_now_datetime(self):
        ...
        :last_editors: yy
        """
        return CoreHelper.get_now_datetime()
    ...
    def create_order_id(self, ran=5):
        ...
        :last_editors: yy
        """
        return CoreHelper.create_order_id(ran)
    ...
    def json_dumps(self, rep_dic):
        ...
        :last_editors: yy
        """
        return CoreHelper.json_dumps(rep_dic)
    ...
    def json_loads(self, rep_str):
        ...
        :last_editors: yy
        """
        return CoreHelper.json_loads(rep_str)
    ...
    def get_param_int(self, param_name, default=0, strip=True, filter_sql=False, filter_special_key=False, throw_response_error=True, contain_list=[], range_list=[]):
        ...
        param, status = CoreHelper.to_int(param, default, True)
        if status == False:
    ...
    def get_param_decimal(self, param_name, default=0, strip=True, filter_sql=False, filter_special_key=False, throw_response_error=True):
        ...
        if not hasattr(self, "param_error_list"):
            self.param_error_list = []
        param = self.get_param(param_name, default, strip, filter_sql, filter_special_key)
        param, status = CoreHelper.to_decimal(param, default, True)
        if status == False:
    ...
    def get_param_datetime(self, param_name, default='1900-01-01 00:00:00', strip=True, filter_sql=False, filter_special_key=False, throw_response_error=True, fmt='%Y-%m-%d %H:%M:%S'):
        ...
        if not hasattr(self, "param_error_list"):
            self.param_error_list = []
        param = self.get_param(param_name, default, strip, filter_sql, filter_special_key)
        param, status = CoreHelper.to_date_time(param, fmt, default, True)
        if status == False:
...
class FrameBaseHandler(FrameBaseApiHandler):
    ...
    def create_operation_log(self, operation_type=1, model_name="", handler_name="", old_detail=None, update_detail=None, operate_user_id="", operate_user_name="", operate_role_id="", title="", operation_desc=""):
        ...
        request_dict = {"request_params": self.request_params, "request": self.request}
        ...
        operation_log.request_code = self.request_code
        operation_log.operation_type = operation_type
        operation_log.model_name = model_name
        operation_log.handler_name = handler_name
        operation_log.detail = CoreHelper.json_dumps(old_detail) if old_detail else {}
        operation_log.update_detail = CoreHelper.json_dumps(update_detail) if update_detail else {}
        ...
        if operation_type == 1:
            ...
            operation_type_name = "还原"
        elif operation_type == 5:
            operation_type_name = "复制"
        elif operation_type == 6:
            operation_type_name = "导出"
        elif operation_type == 7:
            operation_type_name = "导入"
        elif operation_type == 8:
            operation_type_name = "上架"
        elif operation_type == 9:
            operation_type_name = "下架"
        elif operation_type == 10:
            operation_type_name = "操作"
        else:
            operation_type_name = ""
        operation_log.title = operation_type_name + title if operation_type != 10 else title
        operation_log.operation_desc = operation_desc
        operation_log.create_date = TimeHelper.get_now_format_time()
        operation_log.create_day = CoreHelper.get_now_day_int()
    ...
    def check_continue_request(self, handler_name, app_id, object_id, expire=100):
        ...
        if object_id and handler_name and app_id:
            ...
            if CoreHelper.is_continue_request(f"continue_request:{handler_name}_{app_id}_{object_id}_{sign}", expire) == True:
