...
class MembershipStorage(Storage):
    ...
    def refresh_memberships(self) -> None:
        ...
        if self.first_load:
            default_start_time = self.storage.mandate_start_time.isoformat()
        else:
        ...
        for org_id, org_data in self.temporary_data.items():
            for single_org_membership in org_data:
                ...
                if end_time := single_org_membership.get("end_time"):
                    ...
                    end_time = default_start_time - timedelta(seconds=1)
    ...
    def fix_user_membership(self, person, organization_id, on_behalf_of) -> None:
        ...
        """
        person_voter_memberships = self.active_voters.get(organization_id).get(
            person.id
        )
        if person_voter_memberships and len(person_voter_memberships) > 1:
            # person has change a club
            for m in person_voter_memberships:
                if m.on_behalf_of and m.on_behalf_of != on_behalf_of:
