    ...
    def _trigger_start_for_user(self, username):
        ...
        try:
            ...
            try:
                ...
                # Build arguments as a PowerShell array string
                arguments_for_ps_array = f'@("--port", "{calculated_port}", "--target_user", "\'{user}\'")' 

                powershell_command = (
                    f'Start-Process -FilePath "{target_exe_for_ps}" '
                    f'-ArgumentList {arguments_for_ps_array} -Verb RunAs' # Pass the array syntax directly
                )
