    ...
    def _trigger_start_for_user(self, username):
        ...
        try:
            ...
            try:
                ...
                # Build the arguments for the target executable itself
                target_arguments = f'--port {calculated_port} --target_user \'{user}\'' # Single quotes around user

                # Build the arguments for cmd.exe /K
                # Needs outer quotes for /K, inner quotes around exe path
                # Format: /K ""C:\Path\To\Exe" --arg1 val1 --arg2 'val2'"
                cmd_k_arguments = f'/K """{target_exe_for_ps}"" {target_arguments}"'
                # Escape inner quotes for PowerShell string evaluation if needed, though often works directly
                cmd_k_arguments_escaped = cmd_k_arguments.replace('"', '""' ) # Double up double quotes for PS

                # Build the Start-Process command to launch cmd.exe elevated
                start_process_cmd = (
                    f'Start-Process -FilePath cmd.exe '
                    f'-ArgumentList "{cmd_k_arguments_escaped}" '
                    f'-WorkingDirectory "{target_exe_dir_for_ps}" -Verb RunAs'
                )
