...
try:
    ...
    from achatbot.thirdparty.qwen2_code2wav.engine import Code2WavEngine
    from achatbot.thirdparty.qwen2_code2wav import Code2WavEngineConfig, Code2WavGenerationConfig
    from achatbot.core.llm.transformers.models.qwen2_5_omni import (
...
from achatbot.common.utils.helper import ThreadSafeDict, get_device
        ...
        for step in range(self.args.warmup_steps):
            ...
            for i, chunk in enumerate(streamer):
                ...
                if "audio_wav" in chunk:
                    logging.info(
                        f"{i} chunk: {chunk['text']} {chunk['audio_wav'].shape} warmup time: {times[i]} s"
                    )
                else:
                    logging.info(f"{i} chunk: {chunk['text']} warmup time: {times[i]} s")
                start_time = time.perf_counter()
            if len(times) > 0:
                logging.info(
