...
class AudioCameraOutputProcessor(OutputProcessor):
    def __init__(
        ...
        **kwargs,
        ...
        self._audio_out_thread = threading.Thread(target=self._audio_out_task_handler)
        self._audio_out_thread.daemon = True
    ...
    async def stop(self, frame: EndFrame):
        ...
        if self._params.audio_out_enabled:
            self._audio_out_queue.put(None)  # Signal the thread to stop
    ...
    async def _handle_audio(self, frame: AudioRawFrame):
        ...
        for i in range(0, len(audio), self._audio_chunk_size):
            ...
            self._audio_out_queue.put(chunk)
    ...
    def _audio_out_task_handler(self):
        while True:
            try:
                chunk = self._audio_out_queue.get()
                if chunk is None:
                    break
                asyncio.run(self.write_raw_audio_frames(chunk))
