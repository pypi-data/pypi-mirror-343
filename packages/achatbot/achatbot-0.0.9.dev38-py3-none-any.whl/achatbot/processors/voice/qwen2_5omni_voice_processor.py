...
class Qwen2_5OmniVoiceProcessor(VoiceProcessorBase):
    ...
    async def start(self, frame: StartFrame):
        ...
        self._gen_task = self.get_event_loop().create_task(self._gen())
    ...
    async def gen(self) -> AsyncGenerator[Frame, None]:
        await self._input_queue.put(self._session)
        while True:
            ...
            if tensor_audio is not None:  # don't use if tensor_audio to check
                ...
                logging.info(f"audio tensor:{tensor_audio.shape},push audio len:{len(audio_bytes)}")
    ...
    async def _gen(self):
        while True:
            session = await self._input_queue.get()
            if session is None:
                break
            try:
                ...
                logging.info(f"Cancelled generating audio, stopping generation")
