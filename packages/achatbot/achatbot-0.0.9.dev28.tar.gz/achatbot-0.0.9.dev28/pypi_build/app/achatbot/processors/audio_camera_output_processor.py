...
class AudioCameraOutputProcessor(OutputProcessor):
    ...
    async def _handle_audio(self, frame: AudioRawFrame):
        ...
        for i in range(0, len(audio), self._audio_chunk_size):
            ...
            await asyncio.to_thread(self._sync_write_raw_audio_frames, chunk)
            # await self.write_raw_audio_frames(chunk)
            await self.push_frame(BotSpeakingFrame(), FrameDirection.UPSTREAM)
        # self._audio_out_buff.clear()

    def _sync_write_raw_audio_frames(self, frames: bytes):
        """
        Synchronous wrapper to call the asynchronous write_raw_audio_frames method.
        """
        asyncio.run_coroutine_threadsafe(
            self.write_raw_audio_frames(frames), asyncio.get_event_loop()
        )
