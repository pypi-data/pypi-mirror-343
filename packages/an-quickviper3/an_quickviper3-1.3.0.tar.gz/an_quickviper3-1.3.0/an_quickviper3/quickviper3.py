...
class QuickViper3:
    ...
    def list( self ):
        ...
        """
        self.toolkit.executor_sync( cmd = ["conda", "list", "-p", self.lcl_path], shell = False )
        self.toolkit.executor_sync( cmd = [ str( self.bin_path.joinpath( "pip" ) ), "list"], shell = False )

    def update(self, env_file: str = 'environment.yml', prune: bool = True ) -> bool:
        """
        Conda 環境を environment.yml から更新します。
        args:
            env_file( str ): 環境定義ファイルのパス
            prune( bool ): --prune オプションを付与して不要パッケージを削除するか
            return( bool ): 成功時 True、失敗時 False
        """
        cmd = ['conda', 'env', 'update', '-q', '-f', env_file]
        if prune:
            cmd.append('--prune')
        ...
        try:
            ...
            self.debug.log_step( f"Conda env update failed:\n{result.stderr}", success = False )
            return False
