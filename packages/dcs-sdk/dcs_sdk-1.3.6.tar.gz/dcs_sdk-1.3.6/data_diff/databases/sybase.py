...
class Sybase(ThreadedDatabase):
    ...
    _args: Dict[str, Any]
    _sybase: Any
    sybase_driver_type: Any
    query_config_for_free_tds: Dict[str, bool]
    _conn: Any
    ...
    def __init__(self, host, port, user, password, *, database, thread_count, **kw) -> None:
        ...
        self._sybase = None
        self._conn = None
        self.query_config_for_free_tds = {
    ...
    def select_table_schema(self, path: DbPath) -> str:
        ...
        if self.sybase_driver_type.is_iq:
            ...
            if self.query_config_for_free_tds["ase_query_chosen"]:
                return ase_query
            elif self.query_config_for_free_tds["freetds_query_chosen"]:
                return iq_query
            try:
                self._query_cursor(self._conn.cursor(), ase_query, test_query=True)
                self.query_config_for_free_tds["ase_query_chosen"] = True
                return ase_query
            except Exception as e:
                logger.error(f"ASE query failed: {e}")
                logger.info("Falling back to FreeTDS query.")
                self._query_cursor(self._conn.cursor(), iq_query, test_query=True)
                self.query_config_for_free_tds["freetds_query_chosen"] = True
                return iq_query
        else:
            ValueError(
    ...
    def _query_cursor(self, c, sql_code, test_query: bool = False):
        if test_query:
            try:
                ...
                raise QueryError(f"Query failed: {sql_code}") from e
        try:
            c.execute(sql_code)
