...
# key we chose based on the packet information would also be a duplicate). If we add a differentiator like a timestamp or packet_num, then we can't remove it by key because we wouldn't know when an ack comes in what the original values were
# Another issue is imagine I get an unwanted packet, and then 100 packets later the exact same one. The python dictionary would not be able to keep track of order in this case, it would refresh.
...
class Ports:
    def __init__(self,report_output_buffer,internalip,externalip,verbose,database):
        ...
        self.internal_ip_equals_external_ip=externalip
...
def send_data(consumer_upload_conn):
    ...
    # prepare a Session with connection pooling
    ...
    while True:
        # 1) wait for either:
        #    a) new data arrives
        #    b) idle timeout expires
        elapsed = time.monotonic() - last_activity
        timeout = max(0, IDLE_FLUSH_SEC - elapsed)
        if consumer_upload_conn.poll(timeout):
            # got something
            try:
        ...
        #    flush whenever we have at least a full batch,
        #    or we've been idle long enough and have anything queued
        while queue and (len(queue) >= BATCH_SIZE
                         or (time.monotonic() - last_activity) >= IDLE_FLUSH_SEC):
            ...
            try:
                ...
                print("Upload error, dropping batch:", e, flush=True)
                # if you wanted retry logic, you could reappend here

        # loop back and wait/poll again
