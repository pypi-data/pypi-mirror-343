...
class Ports:
    def __init__(self,report_output_buffer,internal_ip,external_ip,verbose,database,internal_is_private,external_is_private,internal_ip_equals_external_ip,interface,external_network_information,internal_network_information):
        print(f"Monitoring {internal_ip}")
        ...
        self.internal_ip_randomized=self.randomize_ip(internal_ip)
        self.external_ip_randomized=self.randomize_ip(external_ip)
...
def choose_windows_interface():
    ...
    # 2) Grab the raw device list from Npcap
    raw_devs = pcap.findalldevs()  # [r"\Device\NPF_{GUID}", ...]

    # 3) Build a GUID - adapter info map via Win32_NetworkAdapter
    c = wmi.WMI()
    adapter_map = {}
    for nic in c.Win32_NetworkAdapter():
        ...
        adapter_map[gid] = {
            'netid': nic.NetConnectionID,    # e.g. "Ethernet", "Wi-Fi"
            'name':  nic.Name,               # usually same as netid
    ...
    # 4) Open a tiny sniff handle on each device for stats
    devices = []
    for dev in raw_devs:
        ...
        speed    = info.get('speed') or 'N/A'
        status   = info.get('status')
        status_str = {
            0:'Unknown',1:'Other',2:'Disabled',3:'Enabled',
            4:'Testing',5:'Unknown',6:'Dormant',
            7:'NotPresent',8:'LowerLayerDown'
        }.get(status, 'N/A')

        # try opening for stats
        try:
    ...
    # 5) Let stats accumulate
    time.sleep(0.4)

    # 6) Collect stats for each
    for d in devices:
        h = d['handle']
        if h:
            try:
                seen, drop, ifdrop = h.stats()
            except Exception:
                seen = drop = ifdrop = None
        else:
            seen = drop = ifdrop = None
        d.update({'seen': seen, 'drop': drop, 'ifdrop': ifdrop})

    # 7) Print everything nicely
    print("\nAvailable capture devices:\n")
    for i, d in enumerate(devices, 1):
        live = 'Live' if d['seen'] and d['seen'] > 0 else 'Dead'
        print(f"{i:2d}. [{live}] {d['friendly']}  (status={d['status']}, speed={d['speed']})")
        print(f"      NPF: {d['npf']}")
        print(f"      MAC: {d['mac']}")
        if d['seen'] is not None:
            print(f"      stats: seen={d['seen']}, drop={d['drop']}, ifdrop={d['ifdrop']}")
        else:
            print(f"      stats: unavailable")
        print()

    # 8) Prompt and return the chosen NPF name
    choice = input(f"Pick an interface [1-{len(devices)}] (default 1): ").strip()
    idx = int(choice) - 1 if choice.isdigit() and 1 <= int(choice) <= len(devices) else 0
    return devices[idx]['npf']
...
def choose_mac_linux_interface():
    # 1) Snapshot I/O counters
    io_before = psutil.net_io_counters(pernic=True)

    # 2) Gather static info
    addrs = psutil.net_if_addrs()
    stats = psutil.net_if_stats()

    # 3) Sleep briefly so we can compute "live" packets
    time.sleep(0.1)
    io_after = psutil.net_io_counters(pernic=True)

    # Determine the link-layer family for MAC addresses
    link_family = getattr(psutil, 'AF_LINK', None) or getattr(socket, 'AF_PACKET', None)
    ...
    for iface, addr_list in addrs.items():
        ...
        macs = [a.address for a in addr_list
                if link_family is not None and a.family == link_family]
        ...
        duplex = duplex_map.get(st.duplex, "Unknown")

        # live packet count
        before = io_before.get(iface).packets_recv if iface in io_before else 0
        after  = io_after.get(iface).packets_recv  if iface in io_after  else 0
        seen   = after - before
    ...
    for i, d in enumerate(devices, 1):
        live   = 'ALIVE' if d['seen'] > 0 else 'DEAD'
        status = "UP" if d['isup'] else "DOWN"
        print(f"{i:2d}. [{live}] {d['iface']}  ({status})")
        print(f"      Speed: {d['speed']}  Duplex: {d['duplex']}  MTU: {d['mtu']}")
        print(f"      MAC:   {d['mac']}")
        if d['ips']:
            print(f"      IPs:   {', '.join(d['ips'])}")
        print(f"      Seen:  {d['seen']} pkts in last 0.1 s\n")

    # 6) Prompt user
    choice = input(f"Pick an interface [1-{len(devices)}] (default 1): ").strip()
    idx = int(choice) - 1 if choice.isdigit() and 1 <= int(choice) <= len(devices) else 0
    print(f"{devices[idx]['iface']} selected",flush=True)
    return devices[idx]['iface']
...
def lightscope_run():
    ...
    print(f"To view your logs, please visit https://lightscope.isi.edu/tables/{config_reader.database}")
...
class configuration_reader:
    def __init__(self, config_file='config.ini'):
        ...
        self.collection_ip="uninitialized"
        self.external_ip="uninitialized"
        self.internal_ip="uninitialized"
        self.interface = False
        self.gui = False
        self.verbose = 6
        self.pcap = False
        self.database = False
        self.readfile = "uninitialized"
        self.internal_ip_equals_external_ip="uninitialized"
        self.internal_is_private="uninitialized"
        self.external_is_private="uninitialized"
        self.interface="uninitialized"
        ...
        print(self.print_args(),flush=True)
    ...
    def load_config(self, config_file):
        ...
        if 'Settings' in config:
            ...
            self.collection_ip=config.get('Settings', 'collection_ip', fallback="self").lower()
    ...
    def pre_run_setup(self):
        self.internal_ip=self.get_internal_host_ip()
