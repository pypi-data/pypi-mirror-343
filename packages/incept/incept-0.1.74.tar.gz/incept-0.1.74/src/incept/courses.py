...
def addChapters(payload_data: dict,
    ...
    for chapter_payload in local_chapters:
        ...
        # ensure chapter_payload.video_path is always present
        chapter_payload["video_path"] = chapter_payload.get("video_path") if include_video else "NA"
...
def addLessons(lesson_payload: dict, course_filter: str, templates_dir: Path, db=DEFAULT_DB, include_video: bool = False, **kwargs):
    ...
    # Ensure lesson-level video_path is set or NA
    lesson_payload["video_path"] = lesson_payload.get("video_path") if include_video else "NA"
...
if __name__ == "__main__":
    ...
    def test_add_courses_with_video():
        ...
        # ─── DUMP TEST PAYLOAD ─────────────────────────────
        test_payload_path = Path("test_payload.json")
        with open(test_payload_path, "w", encoding="utf-8") as f:
            json.dump(payload_data, f, indent=2)
        print(f"📝  Test payload written to {test_payload_path}")
        print(templates_dir)
        print(NOTION_API_KEY)
        print(NOTION_COURSE_DATABASE_ID)
        # ────────────────────────────────────────────────────
