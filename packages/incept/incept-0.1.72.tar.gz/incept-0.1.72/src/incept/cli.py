...
def cli_add_course(api_key, database_id, data_file_path, name, description, link,
    ...
    # ─── DUMP CLI PAYLOAD ──────────────────────────────
    import json
    cli_payload_path = Path.home() / ".incept" / "payload" / "cli_payload.json"
    with open(cli_payload_path, "w", encoding="utf-8") as f:
        json.dump(payload_data, f, indent=2)
    click.echo(f"📝  CLI payload written to {cli_payload_path}")
    print(Path.home() / ".incept" / "templates")
    print(include_video)
    print(api_key)
    print(database_id)
    # ────────────────────────────────────────────────────
