...
from incept.dl_rebelway import cli_download_rebelway
from incept.dl_rebelway import report_broken_sources
...
@click.option("--excel", "--input",  "excel_path", required=True, help="path to lessons.xlsx")
@click.option("--output",            "out_dir",    required=True, help="where to save MP4s")
@click.option("--skip-first",        default=0,     help="rows to skip (zero-based)")
@click.option("--chrome-port",       default=9222,  help="Chrome debug port")
def dl_rebelway(excel_path, out_dir, skip_first, chrome_port):
    """Download Rebelway SOURCE videos from an Excel sheet."""
    cli_download_rebelway.callback(excel_path, out_dir, skip_first, chrome_port)

@main.command("report-broken")
@click.option("--excel",  "excel_path", required=True, help="path to lessons.xlsx")
@click.option("--output", "out_csv",    required=True, help="where to write the report CSV")
@click.option("--chrome-port", default=9222, help="Chrome debug port")
def report_broken(excel_path, out_csv, chrome_port):
    """Scan each lesson for missing SOURCE links and write a CSV report."""
    # call the same routine you wrote in report_broken_sources()
