...
class NotionDB:
    ...
    def __init__(self, api_key, database_id, mapping_config: dict = None):
        ...
        print(f"→ NotionDB.__init__: loading mapping from {mapping_file_path}")
        print(f"→ forward_mapping['Video'] = {forward_mapping.get('Video')!r}")
        print(f"→ forward_mapping['Video Path'] = {forward_mapping.get('Video Path')!r}")
        print(f"→ back_mapping['video']    = {back_mapping.get('video')!r}")
        print(f"→ back_mapping['video_path']    = {back_mapping.get('video_path')!r}")
