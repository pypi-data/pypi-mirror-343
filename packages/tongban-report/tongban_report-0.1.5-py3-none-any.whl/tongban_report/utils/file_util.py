...
base_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "output", current_time)
...
class FileUtil(object):
    ...
    def double_folder_functional(self):
        report_path = os.path.join(base_path, f"{self.project_num}", f"{self.year}年{self.month}月实施", self.title, "功能测试报告")
        os.makedirs(report_path)
        save_path = os.path.join(report_path, "一网通办标准政务服务事项自测报告.docx")
        return save_path

    def double_folder_safety(self):
        report_path = os.path.join(base_path, f"{self.project_num}", f"{self.year}年{self.month}月实施", self.title, "安全测试报告")
        os.makedirs(report_path)
