...
from agptools.helpers import parse_uri, build_uri, parse_uri
...
class BrowserLogicSession(iBrowserSession):
    ...
    async def _save_step_training_attibutes(self, element, ctx):
        keyword = ctx.get("keyword", ctx["step"])
        selector = ctx["current_selector"]
        ...
        for idx in range(0, 3):
            ...
            if not attributes:
                break
            if attributes.get("tag") in (
                "body",
                "html",
            ):
                break

            _type = ctx["info"].get("type")
            if _type and _type not in ("meta",):
                attributes["xpath"] = await self._get_xpath(element)

            attributes = {key: tokens(value) for key, value in attributes.items()}
            print(f"{idx}: {attributes}")
            data[f"{idx}"] = attributes
            element = element.locator("..")

        storage = self.bot.parent.syncmodel[0].storage[0].storage

        # uri = f"pricemon://crawler/learning:step_{keyword}"
        uri = URI_AI_LEARNING_STEPS.format(**locals())
        current = await storage.get(uri) or {}
        # merge
        for idx, attributes in data.items():
            for fullkey, values in attributes.items():
                for key in re.findall(r"\w+", fullkey):
                    if re.match("action|content|href|style|text|title|xpath", key):
                        continue

                    if not (key := is_common_word(key)):
                        continue

                    base = current.setdefault(idx, {}).setdefault(key, {})
                    for value in values:
                        if not re.match("id", value) and re.match(".{0,2}$", value):
                            # too short
                            continue

                        if not (value := is_common_word(value)):
                            continue
                        base[value] = base.get(value, 0) + 1
        result = await storage.put(uri, current)
        return data
