...
from xorq.vendor.ibis.expr.rewrites import rewrite_window_input
...
from xorq.vendor.ibis.util import deprecated, promote_list, warn_deprecated
...
class Value(Expr):
    ...
    def isin(self, values: Value | Sequence[Value]) -> ir.BooleanValue:
        ...
        └───────────────────┘
        """
        from xorq.vendor.ibis.expr.types import ArrayValue
    ...
    def identical_to(self, other: Value) -> ir.BooleanValue:
        ...
        except (com.XorqTypeError, NotImplementedError):
...
class Scalar(Value):
    ...
    def as_table(self) -> ir.Table:
        ...
        True
        """
        from xorq.vendor.ibis.expr.types.relations import unwrap_alias
...
class Column(Value, _FixedTextJupyterMixin):
    ...
    def topk(
        ...
        └────────┴───────────┘
        """
        from xorq.vendor.ibis.expr.types.relations import bind

        try:
            (table,) = self.op().relations
        except ValueError:
            raise com.XorqTypeError("TopK must depend on exactly one table.")

        table = table.to_expr()

        if by is None:
            by = lambda t: t.count()  # noqa: E731
