...
def path_pretzel(T, t_dur=3600.0, radius=1000.0, height=100.0,
        cycles=1.0, twists=1):

def path_grid(T, velocity, spacing=300.0, length=1600.0, rows=6):

def path_spiral(T, velocity, spacing=300.0, cycles=3):

def path_clover(T, velocity, radius=1000.0, cycles=3):

class waypoints:
    def __init__(self, T, points, velocity,
            R_min=100.0, plot=True, ax=None, res=1.0,
...
def path_grid(T, velocity, spacing=300.0, length=1600.0, rows=6):
    ...
    points[:, 1:4] = np.array([[0.0, 0.0, spacing], [spacing, 2*spacing, 2*spacing]])
    ...
    p_t = waypoints(T, points, velocity, 0.01, True).B
    return p_t
...
def path_spiral(T, velocity, spacing=300.0, cycles=3):
    ...
    # Convert waypoints to a path.
    p_t = waypoints(T, points, velocity, 0.01, False).B
    return p_t


def path_clover(T, velocity, radius=10000.0, cycles=3):
    """
...
class waypoints:
    ...
    Bezier curves. This path will be defined such that the given velocity and
    sampling period are observed.

    Click and drag near a control point to move it.
    Shift click (or right-click) and drag to pan.
    Scroll to zoom.
    ...
    def __init__(self, T, points, velocity,
        ...
        # Initialize the selected control point and pressed keys.
        self.sel = None
        self.shift = False
    ...
    def home(self):
        """ Set the view to the limits of the data. """
    ...
    def on_press(self, event):
        """
        Shift, left-mouse click or right-mouse click:
            Remember the mouse location and view.
        ...
        if event.button == 1: # left mouse button
            if self.shift: # shift key is down
                # Remember where the pan click started.
            ...
            if self.shift: # shift key is down
                ...
                # Find the midpoint closest to the mouse.
                mp = (self.p[:, :-1] + self.p[:, 1:])/2
                dx = event.ydata - mp[0]
                dy = event.xdata - mp[1]
                r = np.sqrt(dx**2 + dy**2)
                i = np.argmin(r)

                # Insert new point before the next control point.
    ...
    def on_motion(self, event):
        """
        Shift, left-mouse drag or right-mouse drag:
            Pan the view.

        Left-mouse drag with selected control point:
    ...
    def on_release(self, event):
        ...
        # Reset the view.
        if (self.xlim_home == self.ax.get_xlim()) \
                and (self.ylim_home == self.ax.get_ylim()):
            self.home()
            self.canvas.draw_idle()

    def on_scroll(self, event):
        """ Scroll centered on the mouse location. """
