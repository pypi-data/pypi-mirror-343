...
class WagtailFilterPersistenceMiddleware(MiddlewareMixin):
    def process_request(self, request):
        # Skip middleware for unauthenticated users or login/logout pages
        ...
        try:
            ...
            if request.session.get('post_save_redirect', False):
                # Use get() instead of pop() for safer access
                request.session.pop('post_save_redirect', None)
            ...
            # If anything goes wrong, don't break the site - just continue normally
            pass
