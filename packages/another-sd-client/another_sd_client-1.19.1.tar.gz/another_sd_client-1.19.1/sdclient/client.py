...
class SDClient:
    ...
    def get_department_parent_history(
        ...
        """
        path_param = (
            "api-gateway" if not self.use_test_env else "organization-8s-stable-gw"
        )
        try:
            response = httpx.get(
                f"https://service.sd.dk/{path_param}/organization/api/v1/organizations/uuids/{str(org_unit_uuid)}/department-parent-history",
                auth=(self.username, self.password),
                timeout=self.timeout,
            )
