...
from ..contentpatcher import ToArea, Position
...
class Eventscripts:
    ...
    def move (self, actor:str, x:int, y:int, speed:int=4) -> None:
        self.commands.append(f"move {actor} {x} {y} {speed}")
    ...
    def quickQuestion(self, question: str, answer1: str, answer2: str, answer3: str, answer1script: str, answer2script: str, answer3script: str) -> None:
        self.commands.append(f"quickQuestion {question}#{answer1}#{answer2}#{answer3}\\{answer1script}\\{answer2script}\\{answer3script}")
    ...
    def temporaryAnimatedSprite(self, texture: str, rectangle:ToArea, interval:int, frames: int, loops:int, tile:Position, flicker:bool, flip:bool, sorttileY:int, alphafade: float, scale:float, scalechange:float, rotation:float, rotationchange:float, flags:list[str]):
    ...
    def viewportmove(self, code:str):
        self.commands.append(f"viewportmove {code}")
