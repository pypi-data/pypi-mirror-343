...
def generate_query_result_table(base_url, queries):
    ...
    if not nl_queries:  # the llm may have failed
    ...
    for query, nl_query in zip(queries, nl_queries):
        ...
        button = RQL_BUTTON.format(base_url, urllib.parse.urlencode(button_query))
    ...
    def call(self):
        ...
        if question:
            ...
            table = generate_query_result_table(
                cnx.build_url("view"), ask(cnx, question)
            )
            output = """
            ...
            self.w(SPACY_QUERY_TEMPLATE.format(output="Aucun résultat", q=question))
