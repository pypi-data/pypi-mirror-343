...
class GraphQuery(nx.Graph):
    ...
    @property
    def weight(self):
        return sum(w for _, _, w in self.edges.data("weight"))

    def __hash__(self):
        return hash(self.to_rql())

    def __eq__(self, other: "GraphQuery"):
        return self.to_rql() == other.to_rql()

    def get_node_var_name(self, node):
        if node not in self.type_to_var:
            var_name = self.to_variable(node)
            self._definitions.add(f"{var_name} is {node}")
    ...
    def convert_attr_into_rql(self, attr_type):
        ...
        if len(descr) == 2:
            ...
            self._definitions.add(f"%({instance_type})s {attr_name} {attr_code}")
        elif len(descr) == 3:
            instance_type, attr_name, expected_value = descr
            self.get_node_var_name(instance_type)
            self._definitions.add(f"%({instance_type})s {attr_name} '{expected_value}'")
        elif len(descr) == 4:
...
class QueryExtractor:
    ...
    def compute_weight(self, subj, pred, obj):
        if self.weight_df is None:
            if subj.startswith("CW") or obj.startswith("CW"):
                return 4
            if {subj, pred, obj}.intersection(CW_INTERNAL_ENTITIES):
                return 3 * len({subj, pred, obj}.intersection(CW_INTERNAL_ENTITIES))
            return 1
    ...
    def _get_subgraphs(self, subgraph, max_radius=9):
        ...
        for current_node in not_connected_nodes:
            ...
            for radius in range(1, max_radius + 1, 1):
    ...
    def get_queries(self, target_entities, target_attributes) -> list[GraphQuery]:
        ...
        for te in target_entities:
            if "|" in te:
                # in case the user ask for a specific relation
                # make sure we visit the subject, the object of the relation
                # too.
                subj, _, obj = te.split("|")
                target_nodes.append(subj)
                target_nodes.append(obj)
            target_nodes.append(te)
        ...
        all_subgraphs = set(
            ...
            for sg in self._get_subgraphs(subgraph)
        )

        return sorted(all_subgraphs, key=lambda sg: sg.weight)
...
def llm_rewrite_queries(queries: list[GraphQuery], max_queries=10):
    ...
    for i in range(0, len(queries) // max_queries + 1):
        some_queries = queries[i * max_queries : (i + 1) * max_queries]
        queries_and_explanation = llm(
            system_prompt=(
                "Rephrase those sentences in a concise and clear manner, "
                "without unnecessary complexity, while maintaining the meaning "
                "and accuracy. Reply in french."
            ),
            user_prompt=(
                "QUERIES:\n"
                + "- "
