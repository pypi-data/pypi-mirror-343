...
def get_preprocessed_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df[config.model_config.cabin_var[0]] = df[config.model_config.cabin_var[0]].\
        apply(get_first_cabin)
    df[config.model_config.title_var[0]] = df[config.model_config.name_var[0]].apply(get_title)
