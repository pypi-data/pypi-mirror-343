...
# Import CNN function directly to handle training parameters
from .modules.predict.cnn_model import predict_drowsiness_with_cnn
            ...
            try:
                if force_train_cnn:
                    if not train_dir or not test_dir:
                        return {'error': "train_dir and test_dir must be provided when force_train_cnn=True"}
                    
                    # Use the CNN model function directly with training parameters
                    prediction_result, confidence = predict_drowsiness_with_cnn(
                    ...
                    # For normal prediction without training, use predict_drowsiness_internal
                    prediction = predict_drowsiness_internal(image_data=img_array, force_train=force_train_cnn)
