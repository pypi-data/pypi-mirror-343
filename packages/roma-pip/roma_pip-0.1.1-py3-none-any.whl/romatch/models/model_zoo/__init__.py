...
def tiny_roma_v1_outdoor(device, weights = None, xfeat = None):
    ...
    if xfeat is None:
        xfeat = torch.hub.load(
            ...
            'XFeat', 
            # '/home/cxb/.cache/torch/hub/checkpoints/xfeat.pt')
            pretrained = True, 
            top_k = 4096).net

    return tiny_roma_v1_model(weights = weights, xfeat = xfeat).to(device) 

def roma_outdoor(device, weights=None, dinov2_weights=None, coarse_res = 560, upsample_res = 864, amp_dtype = torch.float16):
    ...
    if weights is None:
        ...
        #                                              map_location=device)
        weights = torch.load('./weights/roma_outdoor.pth', map_location=device, weights_only=True)
...
def roma_indoor(device, weights=None, dinov2_weights=None, coarse_res = 560, upsample_res = 864, amp_dtype = torch.float16):
    if isinstance(coarse_res, int):
        coarse_res = (coarse_res, coarse_res)
    if isinstance(upsample_res, int):    
        upsample_res = (upsample_res, upsample_res)
