...
class TestConfig:
    ...
    MODEL = os.getenv("LLM_MODEL", "gemini-2.0-flash-exp")  # Read from env with fallback
...
def initialize_llm():
    ...
    try:
        return ChatGoogleGenerativeAI(
            ...
            google_api_key=api_key,  # Using google_api_key parameter
            temperature=float(os.getenv("LLM_TEMPERATURE", "0.2")),
            max_tokens=int(os.getenv("LLM_MAX_TOKENS", "4096")),
        )
    except Exception as e:
        raise Exception(f"Failed to initialize Gemini LLM: {str(e)}. Please verify your API key and permissions.")
