...
def cleanup_old_artifacts(request, metrics_reporter_fixture):
    ...
    if all_tests:
        try:
            # Export individual test metrics - Use the properly injected fixture
            reporter = metrics_reporter_fixture
            
            # Check if metrics_reporter is callable (function) instead of an object
            if callable(reporter) and not hasattr(reporter, 'export_metrics'):
                # If it's a function that returns the actual reporter, call it
                reporter = reporter()
                
            # Now use the reporter object to export metrics
            if hasattr(reporter, 'export_metrics'):
                for test_name in all_tests:
                    reporter.export_metrics(test_name)
                    
                # Generate combined performance report
                if hasattr(reporter, 'save_execution_summary'):
                    reporter.save_execution_summary()
                
                # Check for performance regressions
                if hasattr(reporter, 'get_trend_analysis'):
                    for test_name in all_tests:
                ...
                logger.warning("Metrics reporter does not have export_metrics method")
