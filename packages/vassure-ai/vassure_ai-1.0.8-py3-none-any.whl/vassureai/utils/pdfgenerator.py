...
class TestReport:
    def __init__(self, filename_prefix=None, output_dir=None):
        """Initialize consolidated report"""
        # Use developer's project directory for reports if provided
        self.report_dir = output_dir or 'reports/pdf'
        
        # Ensure the report directory exists
        os.makedirs(self.report_dir, exist_ok=True)
        
        # Create PDF filename with prefix if provided
        prefix = f"{filename_prefix}_" if filename_prefix else ""
        self.filename = os.path.join(self.report_dir, f"{prefix}report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf")
