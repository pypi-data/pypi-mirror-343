...
def browser_config(request):
    ...
    # Check for network sensitive tests
    if request.node.get_closest_marker('network_sensitive'):
        config.update({
...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == "call":
        ...
        test_name = item.name
        if test_name in monitor.metrics_history:
            ...
            trends = reporter.get_trend_analysis(test_name)
            if trends.get('needs_attention'):
                report.performance_warning = True
                report.trend_analysis = trends
                
        # Add load test results if applicable
        if hasattr(call, 'result') and isinstance(call.result, LoadTestResult):
            report.load_test_result = call.result