...
def codebook_to_audio(generated_codes, delay_pattern, B=1, T=2600, C=9):
    ...
    return audio.get_audio_values(np.array(codebook))
...
class Decoder(nnx.Module):
    ...
    # @nnx.jit
    def __call__(self, tgt_ids_Bx1xC, self_rope_cos, self_rope_sin, cross_rope_cos, cross_rope_sin, cross_attn_mask, self_kv_caches, cross_kv_caches):
        x = None
...
def generate(model, config, text, max_tokens=None, cfg_scale=3.0, temperature=0.7, top_p=0.95, use_cfg_filter=True, cfg_filter_top_k=35, seed=0):
    ...
    for step in range(400): # DEBUG
        ...
        logits_Bx1xCxV, self_kv_caches = model.decoder(tgt_ids_Bx1xC=tgt_ids_Bx1xC, self_rope_cos=self_rope_cos, self_rope_sin=self_rope_sin, cross_rope_cos=cross_q_cos, cross_rope_sin=cross_q_sin, cross_attn_mask=decoder_cross_attn_mask, self_kv_caches=self_kv_caches, cross_kv_caches=cross_kv_caches)
        ...
        pred_C = sample_next_token(logits_CxV/temperature, top_p=top_p, cfg_filter_top_k=cfg_filter_top_k, rng_key=subkey) if temperature > 0 else jnp.argmax(logits_CxV, axis=-1)
        print(f'{step=}/{max_tokens}: {pred_C}')
...
def main():
    ...
    parser.add_argument('--text', type=str, default="[S1] Dear Jacks, to generate audio from text from any machine. [S2] Really? How! (screams) [S1] With flakes and an axe. (chuckles)",
