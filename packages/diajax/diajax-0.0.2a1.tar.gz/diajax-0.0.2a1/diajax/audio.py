...
@torch.no_grad()
@torch.inference_mode()
def get_audio_prompt(audio_prompt_path, *, channels=9, pad_value=1025, bos_value=1026, delay_pattern=[0, 8, 9, 10, 11, 12, 13, 14, 15]):
    try:
        ...
        audio_prompt = audio_to_codebook(model, audio_prompt, channels, pad_value, bos_value, delay_pattern)
        return audio_prompt.cpu().numpy()
