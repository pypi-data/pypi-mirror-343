...
def objects():
    ...
    if dd.is_installed("uploads"):
        ...
        if DEMO_DATA:
            ...
            sports_options.update(image_file=Upload.objects.get(library_file="26bd_soccer.png"))
            french_options.update(image_file=Upload.objects.get(library_file="1f347_grapes.png"))
            german_options.update(image_file=Upload.objects.get(library_file="1f34e_apple.png"))
    ...
    yield create_skill(_("Dealing with texts and media"), True)  # Umgang mit Texten und Medien
    ...
    yield prjtemplate(_("Telling experiences in an exciting way"), "Erle", "red", "6")  # Erlebnisse spannend erzählen
    ...
    yield prjtemplate(_("Tell real-life stories "), "Leb", "red", "6")  # Geschichten aus dem Leben erzählen
    ...
    PERIODS = Cycler(StoredPeriod.objects.all())
    ...
    for a in "AB":
        for g in Grade.objects.filter(cert_template__isnull=False):
            yield add_remark(grp := Group(
                grade=g, designation=g.ref + a, year=PERIODS.pop().year))
            for i in range(18):
