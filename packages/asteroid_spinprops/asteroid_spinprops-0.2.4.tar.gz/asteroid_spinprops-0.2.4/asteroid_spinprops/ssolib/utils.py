...
def pq_foler_to_dictionary(path, save=False):
    sso_in_pq = {}
...
def estimate_sidereal_period(data, model_parameters, synodic_period):
    ...
    epoch1 = np.linspace(data["cjd"].values[0][0], data["cjd"].values[0][0] + 1000, 30)

    epoch2 = epoch1 + synodic_period

    ra1, dec1 = calc_radec_for_epoch(data, epoch1)
    ra2, dec2 = calc_radec_for_epoch(data, epoch2)

    atan_param_1 = calc_atan_parameter(ra1, dec1, ra0, dec0)
    atan_param_2 = calc_atan_parameter(ra2, dec2, ra0, dec0)

    ra_alt, dec_alt = flip_spin(ra0, dec0)

    atan_param_1alt = calc_atan_parameter(ra1, dec1, ra_alt, dec_alt)
    atan_param_2alt = calc_atan_parameter(ra2, dec2, ra_alt, dec_alt)
...
def read_clean_data(sso_name, pq_dictionary, clean_data_path, rejected_data_path):
