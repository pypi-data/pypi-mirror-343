...
import pytesseract
from robot.api.deco import keyword  # <-- Import the decorator
...
class DeskAutomation:
    ...
    @keyword  # <-- Expose this function as a keyword
    def Set_Execution_Speed(self, time_str):
        ...
        if isinstance(time_str, str):
            ...
            if time_str.endswith("ms"):
                ...
                execution_speed = float(time_str.replace("s", ""))
            else:
    ...
    def Open_Application(self, app_path):
        ...
        # Try to bring the app window to the front
        app_name = app_path.split("\\")[-1].replace(".exe", "").lower()
        time.sleep(2)
