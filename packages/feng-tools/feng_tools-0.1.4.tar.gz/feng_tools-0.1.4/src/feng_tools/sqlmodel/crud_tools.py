...
class CrudTools:
    ...
    def get_one(self, model:type[SQLModel], id_value: int)-> SQLModel:
        """ 获取一条数据
        :param model: 模型
        :param id_value: 主键值
        :return: 主键对应数据
        ...
        with Session(self.engine) as session:
            one_data = session.get(model, id_value)
        return one_data
    ...
    def query_by_page(self, model:type[SQLModel], page:int, page_size:int, *where_clause: Union[_ColumnExpressionArgument[bool], bool]) -> list:
        ...
        """
        with Session(self.engine) as session:
            statement = select(model).where(*where_clause).offset((page - 1) * page_size).limit(page_size)
            data_list = session.exec(statement).all()
        return data_list
    ...
    def update_one(self, model:type[SQLModel], data_dict:dict, id_value: int=None)->SQLModel:
        """更新一条数据
        :param model: 模型
        :param id_value: 主键值
        ...
        with Session(self.engine) as session:
            db_data = session.get(model, id_value)
            if db_data:
    ...
    def get_max(self, model:type[SQLModel], field:Field):
        ...
        """
        with Session(self.engine) as session:
            statement = select(model).order_by(field.desc())
    ...
    def get_min(self, model:type[SQLModel], field:Field):
        ...
        """
        with Session(self.engine) as session:
            statement = select(model).order_by(field.asc())
    ...
    def get_avg(self, model:type[SQLModel], field:Field):
        ...
        """
        with Session(self.engine) as session:
            statement = select(func.avg(field)).select_from(model)
    ...
    def get_sum(self, model:type[SQLModel], field:Field):
        ...
        """
        with Session(self.engine) as session:
            statement = select(func.sum(field)).select_from(model)
    ...
    def get_max_min_avg_sum(self, model:type[SQLModel], field:Field):
        ...
        """
        with Session(self.engine) as session:
            statement = select(func.max(field), func.min(field), func.avg(field), func.sum(field)).select_from(model)
    ...
    def get_max_min_avg_sum_by_page(self, model:type[SQLModel], page:int, page_size:int, field:Field):
        ...
        """
        with Session(self.engine) as session:
            statement = select(func.max(field), func.min(field), func.avg(field), func.sum(field)).select_from(model).offset((page - 1) * page_size).limit(page_size)
            max_min_avg_sum = session.exec(statement).one()
        return max_min_avg_sum
