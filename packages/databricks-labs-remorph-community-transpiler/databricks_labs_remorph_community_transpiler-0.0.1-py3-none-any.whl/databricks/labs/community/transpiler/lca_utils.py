...
from databricks.labs.community.transpiler.local_expression import AliasInfo
...
def check_for_unsupported_lca(
    ...
    try:
        all_parsed_expressions: Iterable[Expression | None] = parse(
            source_sql, read=transpile_options.dialect, error_level=ErrorLevel.RAISE
        )
        ...
        logger.warning(f"Error while preprocessing {file_name}: {e}")
        return []  # will fail later, then detailed diagnostics will be provided
    ...
    diagnostics = [
        Diagnostic(
            ...
            message=f"Unsupported operation found in file {file_name}. Needs manual review of transpiled query.",
        )
    ]
    if aliases_in_where:
        diagnostics.append(
            Diagnostic(
                ...
                code="ANALYSIS-UNSUPPORTED_LCA",
                severity=DiagnosticSeverity.Error,
                message=f"Lateral column aliases `{', '.join(aliases_in_where)}` found in where clause.",
            )
        )
    ...
    if aliases_in_window:
        diagnostics.append(
            Diagnostic(
                ...
                code="ANALYSIS-UNSUPPORTED_LCA",
                severity=DiagnosticSeverity.Error,
                message=f"Lateral column aliases `{', '.join(aliases_in_window)}` found in window expressions.",
            )
        )
    return diagnostics
