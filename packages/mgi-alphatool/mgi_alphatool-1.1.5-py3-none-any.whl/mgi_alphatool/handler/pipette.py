...
from ..utils.validation import (validate_location, validate_transfer_params, validate_mix_params, validate_wells, validate_tiprack, 
                                validate_mix_in_transfer_params, validate_delay, validate_air_gap, validate_multi_dispense_mode)
    ...
    def __execute_transfer(
        ...
        def _transfer_chunk(vol: Union[int, float], 
            ...
            while volume_remaining > 0:
                ...
                self.aspirate(volume=volume_chunk*len(dest),
                              location=src, 
                              position=aspirate_position,
                              offset=aspirate_offset, 
                              flow_rate=aspirate_flow_rate)
                ...
                for d in dest:
                    if isinstance(d._get_parent(), TrashBin):
                        self.dispense(volume_chunk, d._get_parent(), flow_rate=dispense_flow_rate)
                    else:
                        self.dispense(volume_chunk, d, position=dispense_position, offset=dispense_offset, flow_rate=dispense_flow_rate)
