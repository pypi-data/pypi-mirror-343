...
First, all of the existing water usage data for the upstream WAPs are extracted. Given that the raw water usage data has not had much quality controls, three filters are used to ensure that the usage values are “realistic”. The first are the usage/allocation ratios at the daily, monthly, and yearly scales. The defaults are 2, 3, and 2 respectively. These were found to be generous enough to retain real-looking usage values and exclude erroneous ones. Though it is still possible to get false positives and negatives using these filters.
...
If usage data already exists, then the daily values are used, if not then the monthly estimates are resampled to daily using the pchip interpolation method in Python. This method is used to make the result more smooth and consequently more natural looking.
