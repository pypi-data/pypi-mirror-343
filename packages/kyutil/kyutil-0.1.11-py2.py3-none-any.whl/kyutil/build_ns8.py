...
class ReleaseOSEl8(ReleaseOSBase):
    ...
    def mock_pungi_gather(self):
        ...
        minor = run_get_return(cmd)[1] or "7"
        ...
        for i, repo in enumerate(self.mash_httpd_path.strip().split("\n")):
            new_str = f"repo --name={self.params.get('tag')}-{i} --baseurl={repo}\n%packages\n"
            file_str_switch(f"{self.build_cfg_dir}/{self.ks_file}", "%packages", new_str)
