...
class EPManager:
    ...
    def __init__(self, idf, epw, output_dir=None, energyplus_dir=None):
        """Initialize the EnergyPlus simulator.
        
        Args:
            idf (str): Path to the EnergyPlus IDF file.
            epw (str): Path to the EnergyPlus weather file (EPW).
            output_dir (str, optional): Directory for EnergyPlus output files.
                Defaults to "EnergyPlus Output".
            energyplus_dir (str, optional): Path to the EnergyPlus installation directory.
    ...
    def timestep_callback(self, state):
        ...
        if not self.api.exchange.warmup_flag(state):
            ...
            setpoint = self.control_strategy.execute_control(state, self, current_time)
    ...
    def _validate_configuration(self):
        ...
        Raises:
            ValueError: If required controllers are not set.
        """
        # For the standard control strategy, we need high and low level controllers
        if isinstance(self.control_strategy, StandardControl):
            if self.low_level_control is None:
                raise ValueError("Low-level controller not set. Use set_low_level_control() before simulation.")
            
            if self.high_level_control is None:
                raise ValueError("High-level controller not set. Use set_high_level_control() before simulation.")
    ...
    def _create_data_dict(self):
        ...
        for key, value in self.high_level_control.data.items():
            data[key] = value
