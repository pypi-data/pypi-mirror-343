...
class RandomSetpoint(HighLevelController):
    ...
    time_step : int, optional
        Time step between control updates in minutes. Default is 60.
    min_setpoint : float, optional
        Minimum temperature setpoint in °C. Default is 19.
    max_setpoint : float, optional
        Maximum temperature setpoint in °C. Default is 21.
    random_seed : int, optional
        Seed for the random number generator. Default is 42.
    change_probability : float, optional
        Probability of changing the setpoint at each time step (0.0-1.0). Default is 0.25.
    """
    
    def __init__(self, time_step=60, min_setpoint=19, max_setpoint=21, random_seed=42, change_probability=0.25):
