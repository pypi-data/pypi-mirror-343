...
class StatsHandler:
    ...
    @property
    def _data(self):
        if self._identifier1 and self._identifier2 and self._test_objective == "paired" and not "Difference" in self.__data:
            self.__data['Difference'] = self.data2 - self.data1
    ...
    def _get_data(self, identifier):
        if self._query_on_column:
            data_here = self.__data.query(self._query_column + ' == "' + identifier + '"')
    ...
    def get_hypothesis_str(self, results, extra_info="", statistic_is_arr=False, use_df=False, assume_normality = False):
        ...
        assume_normality_str = ""
        if assume_normality:
            assume_normality_str = f"\nWe assume a normal distribution of the test statistic."
...
Alpha: {self._alpha}{assume_normality_str}
...
{self.get_conclusion(result.pvalue)}\n\n"""
    ...
    def get_correlation(self):
        return stats.pearsonr(self.data1, self.data2)
    ...
    def exec(self):
        if self._is_regression_spec:
            ...
            return self.get_hypothesis_str(results, extra_info=f"using t-test", use_df=True, assume_normality=True)
    ...
    def cohens_d(self):
        if self._override_cohens_d:
            ...
            if self._test_objective == 'correlation':
                ...
                return (xG - xP) / s
        elif self._override_n and self._override_n2 and self._override_t:
            return abs(self._override_t * ((self._override_n + self._override_n2) / (self._override_n * self._override_n2)) ** 0.5)
        elif self.n and self.n2 and self.t:
            return abs(self.t * ((self.n + self.n2) / (self.n * self.n2)) ** 0.5)
