...
class LangchainOpenai:
    ...
    def __init__(self, openai_api_base: str,  streaming: bool, model: str = "", *, openai_api_key: Optional[str] = "None",
        ...
        **kwargs  调用模型的其他参数，可以不传，使用字典形式传入
        ...
        if model is None:
            model = ""
        logger.info(f"call LangchainOpenai,openai_api_base:{openai_api_base},system:{system},prompt:{prompt}")
        if "dashscope.aliyuncs.com" in openai_api_base:
            config = get_system_config_from_nacos()
            openai_api_key = "sk-6911f75d58104540a0fa261f6f4386e1"
            model = "deepseek-r1"
        logger.info(f"call LangchainOpenai,openai_api_key:{openai_api_key},model:{model},openai_api_base:{openai_api_base}")
