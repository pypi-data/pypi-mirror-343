...
_u='table-keys'
_t='yangcore_meta'
_s='SELECT schema_name FROM information_schema.schemata;'
_r='postgresql'
_q='Cannot delete: '
_p='[^/]*=[^/]*'
_o='[^:]*:'
...
class DataAccessLayer:
	...
	async def handle_get_config_request(D,data_path,query_dict):
		E=data_path;A=re.sub(_F,'',E);F=await D._handle_get_data_request(E,A,ContentType.CONFIG_TRUE,query_dict);B=re.sub(_W,'',A)
	...
	async def _handle_get_data_request(E,data_path,schema_path,content_type,query_dict):
		...
		if I is ContentType.CONFIG_TRUE and any(F.startswith(A)for A in E.config_false_prefixes):raise NodeNotFound(P+G+_U)
		with E.engine.connect()as K:
			A=E._get_dbpath_for_data_path(G,I,K)
			if A is _A:raise NodeNotFound(P+G+_U)
	...
	async def handle_post_config_request(A,data_path,query_dict,request_body,create_callbacks,change_callbacks,opaque):
		...
		with A.engine.begin()as C:
			...
			if F is _A:raise ParentNodeNotFound(_d+B+_U)
	...
	async def _handle_post_config_request(B,parent_dbpath,query_dict,request_body,create_callbacks,change_callbacks,opaque,conn):
		...
		if O==_B:C=E;K=_B+C
		else:C=re.sub(_o,'',E);K=O+_B+C
		M=re.sub(_F,'',K);H=B._get_table_name_for_schema_path(M);assert H is not _A
		if H==A.table_name:
			if C in A.node_ptr:
				if not isinstance(A.node_ptr[C],list):raise NodeAlreadyExists(_e+C+_f)
				assert len(F[E])==1
				if F[E][0]in A.node_ptr[C]:raise NodeAlreadyExists(_e+F[E][0]+_f)
				if M not in B.config_true_obu_seq_nodes:A.node_ptr[C].append(F[E][0])
				else:
					G=_V
					if _P in J:G=J[_P]
					if G==_g:assert _N not in J;A.node_ptr[C].insert(0,F[E][0])
					elif G==_V:assert _N not in J;A.node_ptr[C].append(F[E][0])
					else:
			...
			if M in B.config_true_obu_seq_nodes:
				G=_V
				if _P in J:G=J[_P]
				if G==_g:D=0;T=[(B.table_keys[H],Q),(_J,D)];I.insert(0,T)
				elif G==_V:D=len(I);T=[(B.table_keys[H],Q),(_J,D)];I.append(T)
				else:
	...
	async def _recursively_post_subtable_data(C,parent_id,data_path,req_body_iter,jsob,jsob_data_path,create_callbacks,opaque,conn):
		...
		if isinstance(A,dict):
			...
			if B in C.table_keys:
				...
				while A:
					J=A.pop(0);assert isinstance(J,dict);V=re.sub(_W,'',B);O={};O[V]=J;K=C.table_keys[B];G={};G[_H]=P;G[K]=J[K]
					...
					except sa.exc.IntegrityError as Y:raise NodeAlreadyExists(_e+K+'" with value "'+G[K]+_f)from Y
	...
	async def _handle_put_config_request(B,data_path,query_dict,request_body,create_callbacks,change_callbacks,delete_callbacks,opaque,conn):
		...
		if A is _A:
			...
			if A is _A:raise ParentNodeNotFound(_d+G+_U)
		...
		if isinstance(E,list):
			if D in B.table_keys:
				...
				if D in B.config_true_obu_seq_nodes:
					j=B._get_obu_idx_list_for_list_path(C,I);c=[[(_h,A),(_J,V.index(A))]for A in V]
	...
	async def handle_put_opstate_request(D,data_path,request_body):
		...
		with D.engine.begin()as E:
			...
			if A is _A:raise ParentNodeNotFound(_d+C+_U)
	...
	async def _handle_delete_config_request(C,data_path,delete_callbacks,change_callbacks,opaque,conn):
		...
		if A is _A:raise NodeNotFound(_q+D)
		...
		if isinstance(B,list):
			if G in A.table_keys:
				...
				async def L(data_path,delete_callbacks,opaque,conn):
					...
					if C is _A:raise NodeNotFound(_q+B)
	...
	def _get_jsob_for_record_id_in_table(B,table_name,record_id,conn):
		...
		for E in A.c:
			if E.name not in(_O,_H,_b,_c):J[I][E.name]=H[E.name]
		return J
	...
	def _get_dbpath_for_data_path(B,data_path,content_type,conn):
		...
		else:M=re.findall(_p,A.jsob_data_path);assert len(M)!=0;E=M[-1].split(_D);A.inside_path=E[0];P=re.sub('^'+A.table_name,'',A.schema_path);assert P==L;A.inside_path+=L
	...
	def _init_from_existing_db(A,url,cacert_param,cert_param,key_param):
		K=key_param;J=cert_param;I=cacert_param;H=url
		if not(H.startswith('sqlite:///')or H.startswith(_Y)or H.startswith(_r)):raise SyntaxError('The database url contains an unrecognized dialect.')
		D={}
		if I is not _A:
			D[_M]={};D[_M]['ca']=I;D[_M]['mode']=6
			if J is not _A:D[_M]['cert']=J
			if K is not _A:D[_M][_h]=K
		A.engine=sa.create_engine(H,connect_args=D)
		try:
			if A.engine.url.database==_i or not db_utils.database_exists(A.engine.url,connect_args=D):raise NotImplementedError(str(A.engine.url))
		except sa.exc.OperationalError as G:
			if G.orig and'Access denied'in str(G.orig):N=re.sub('^.*"','',re.sub('")$','',str(G.orig)));raise AuthenticationFailed('Authentication failed: '+N)from G
			raise G
		...
		if A.engine.dialect.name==_j:A.schema_path_to_real_table_name[_B]=_G;A.schema_path_to_real_table_name[_G]=_G
		else:A.db_schema=A.engine.url.database;A.schema_path_to_real_table_name[_B]=A.db_schema.join(_Z);A.schema_path_to_real_table_name[_G]=A.db_schema+_Z;O=A.engine.execute(_s);L=O.fetchall();P=[L[A][0]for A in range(len(L))];assert A.db_schema in P
		A.metadata=sa.MetaData(bind=A.engine,schema=A.db_schema);A.metadata.reflect()
		for Q in A.metadata.tables.values():
			for E in Q.c:
				if isinstance(E.type,(sa.sql.sqltypes.BLOB,sa.sql.sqltypes.PickleType)):E.type=sa.PickleType()
				if A.engine.dialect.name==_Y and isinstance(E.type,sa.dialects.mysql.types.LONGTEXT):E.type=sa.JSON()
				elif isinstance(E.type,sa.sql.sqltypes.JSON):E.type=sa.JSON()
		B=A.metadata.tables[A.schema_path_to_real_table_name[_G]]
		with A.engine.connect()as F:
			C=F.execute(sa.select(B.c.jsob).where(B.c.name==_t));M=C.first()[0]
			if M[_k]!=1:raise AssertionError('The database version ('+M[_k]+') is unexpected.')
			C=F.execute(sa.select(B.c.record_id).where(B.c.name==_l));A.global_root_id=C.first()[0];C=F.execute(sa.select(B.c.jsob).where(B.c.name==_u));A.table_keys=C.first()[0];C=F.execute(sa.select(B.c.jsob).where(B.c.name==_v));A.schema_path_to_real_table_name=C.first()[0];C=F.execute(sa.select(B.c.jsob).where(B.c.name==_w));A.config_false_prefixes=C.first()[0];C=F.execute(sa.select(B.c.jsob).where(B.c.name==_x));A.config_true_obu_seq_nodes=C.first()[0]
	def _load_yang_dir_file(A,filename):
		if A.app_name==_n:B=importlib_resources.files(_m)
	...
	def _init_new_db_with_factory_default(A):
		...
		if os.environ.get('YANGCORE_TEST_DAL'):
			...
			with A.engine.begin()as E:G=E.execute(D.insert().values(name=_l,jsob={}))
		...
		with A.engine.begin()as E:G=E.execute(D.insert().values(name=_l,jsob=j))
	...
	def _create_new_db(A,url,yl_obj,cacert_param=_A,cert_param=_A,key_param=_A):
		K='_sztp_ref_stats_stmt';I=key_param;H=cert_param;G=cacert_param;F=yl_obj;assert isinstance(F,dict);B={}
		if G is not _A:
			B[_M]={};B[_M]['ca']=G;B[_M]['mode']=6
			if H is not _A:B[_M]['cert']=H
			if I is not _A:B[_M][_h]=I
		A.engine=sa.create_engine(url,connect_args=B)
		if A.engine.url.database!=_i and db_utils.database_exists(A.engine.url,connect_args=B):raise AssertionError('Database already exists (call init() first).')
		if A.engine.url.database!=_i:
			if A.engine.dialect.name==_Y:db_utils.create_database(A.engine.url,encoding='utf8mb4',connect_args=B)
			else:db_utils.create_database(A.engine.url,connect_args=B)
		A.db_schema=_A
		if A.engine.dialect.name in(_Y,_r):
			A.db_schema=str(A.engine.url.database);E=A.engine.execute(_s);J=E.fetchall();L=[J[A][0]for A in range(len(J))]
			if A.db_schema not in L:A.engine.execute(f"CREATE SCHEMA IF NOT EXISTS {A.db_schema};")
		...
		with A.engine.begin()as C:C.execute(D.insert(),{_I:_t,_K:{_k:1}});C.execute(D.insert(),{_I:'yang-library',_K:F})
		...
		if A.engine.dialect.name==_j:A.schema_path_to_real_table_name[_B]=_G;A.schema_path_to_real_table_name[_G]=_G
		else:A.schema_path_to_real_table_name[_B]=A.db_schema+_Z;A.schema_path_to_real_table_name[_G]=A.db_schema+_Z
		def M(self,stmt,sctx):assert stmt is not _A;assert sctx is not _A;self.globally_unique=_E
		yangson.schemanode.SchemaNode._sztp_globally_unique_stmt=M;yangson.schemanode.SchemaNode._stmt_callback['yangcore:globally-unique']='_sztp_globally_unique_stmt'
		def N(self,stmt,sctx):assert sctx is not _A;self.ref_stats=stmt.argument
		setattr(yangson.schemanode.SchemaNode,K,N);yangson.schemanode.SchemaNode._stmt_callback['yangcore:ref-stats']=K;O=yl_8525_to_7895(F);A.dm=yangson.DataModel(json.dumps(O),A.module_paths);A._recursive_gen_tables(A.dm.schema,_G)
		with A.engine.begin()as C:E=C.execute(D.insert(),{_I:_v,_K:A.schema_path_to_real_table_name});E=C.execute(D.insert(),{_I:_u,_K:A.table_keys});E=C.execute(D.insert(),{_I:_w,_K:A.config_false_prefixes});E=C.execute(D.insert(),{_I:_x,_K:A.config_true_obu_seq_nodes})
	...
	def _recursive_gen_tables(D,node,parent_table_name):
		...
		if issubclass(type(C),yangson.schemanode.ListNode):
			B=[];B.append(sa.Column(_O,sa.Integer,primary_key=_E));O=D.schema_path_to_real_table_name[G];B.append(sa.Column(_H,sa.Integer,sa.ForeignKey(O+'.record_id'),index=_E,nullable=_C));B.append(sa.Column(_b,sa.String(250),index=_E,nullable=_E))
			class S(enum.Enum):UNIVERSAL=0;OPERATIONAL=1;RUNNING=2;STARTUP=3;SYSTEM=4;INTENDED=5;FACTORY=6
			B.append(sa.Column(_c,sa.Integer,index=_E,nullable=_E))
			if C.config is _E:
