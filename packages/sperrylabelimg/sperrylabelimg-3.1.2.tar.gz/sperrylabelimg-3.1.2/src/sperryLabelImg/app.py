...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(
        ...
        add_actions(
            ...
            (open_dir, open_annotation, copy_prev_bounding,
             ...
             close, reset_all, delete_image, quit_)
        )
    ...
    # Uses:
    # self.active_dir_list_widget
    # self.add_active_dir_button
    # self.reset_active_dir_button
    # self.remove_active_dir_button
    # self.active_dir_container
    # self.active_dir_dock
    ...
    def move_image_to_active_dir(self):
        ...
        if not active_dir:
            QMessageBox.warning(
                self, self.tr("Error"), self.tr("No active directory selected")
            )
            return
    ...
    def add_active_dir(self):
        ...
        if self.cur_active_dir not in self.active_dirs:
            ...
            active_dir_items_list.extend(
                create_active_dir_item(active_dir, self.com_dir_path) for
                active_dir in self.active_dirs
            )

            self.active_dir_list_widget.clear()
            _ = [self.active_dir_list_widget.addItem(item) for item in
                 active_dir_items_list]

        # Select the last opened active directory
        cur_active_dir_widget_index = calculate_cur_active_dir_widget_index(
            self.active_dirs, self.cur_active_dir
        )
        ...
        rel_cur_active_dir_msg = calculate_rel_cur_active_dir_msg(
            self.cur_active_dir, self.com_dir_path
        )
        self.active_dir_dock.setWindowTitle(rel_cur_active_dir_msg)
    ...
    def remove_active_directory(self, directory: Path, directory_text):
        ...
        for active_dir in self.active_dirs:
            self.m_img_list.extend(scan_for_images(
                active_dir,
    ...
    def scan_all_images(self):
        ...
        images = []
        for active_dir in self.active_dirs:
            images.extend(scan_for_images(active_dir, self.show_elmer_images))
        return images
    ...
    def sort_img_list(self):
        ...
        self.m_img_list = list(
            filter(
                lambda img: re.match(
                    IMAGE_NAME_PATTERN, os.path.basename(img)
                ), self.m_img_list
            )
        )
        self.m_img_list.sort(
            key=lambda img:
            re.match(PULSE_COUNT_PATTERN, os.path.basename(img)).groups()[0]
        )
        self.m_img_list.sort(
            key=lambda img:
            re.match(RAIL_PATTERN, os.path.basename(img)).groups()[0]
        )
        self.m_img_list.sort(
            key=lambda img:
            re.match(DATASOURCE_PATTERN, os.path.basename(img)).groups()[0]
        )
