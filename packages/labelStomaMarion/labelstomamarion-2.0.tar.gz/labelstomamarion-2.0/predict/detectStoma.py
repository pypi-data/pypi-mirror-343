...
def mainDataset(imagesPath):
    ...
    #arg1 = sys.argv[1]
    # Invocamos a la funcion con dichos parametros y mostramos el resultado por pantalla
    #images = list(os.scandir(imagesPath, validExts=(".jpg", ".jpeg", ".png", ".bmp", ".tiff", ".tif")))
    images = list(os.scandir(imagesPath))
    for image in images:
        if image.name.endswith('.jpg') or image.name.endswith('.jpeg') or image.name.endswith('.tif')or image.name.endswith('.tiff')or image.name.endswith('.png')or image.name.endswith('.bmp'):
            generateXMLFromImage(imagesPath+"/"+image.name)
