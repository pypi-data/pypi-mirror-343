...
def download_if_changed(gcs_uri: str, local_path: str, client: storage.Client = None) -> None:
    """
    Download the object at gcs_uri to local_path if its MD5 differs from the local file (or if local is missing).
    """
    if client is None:
    ...
    if os.path.exists(local_path):
        ...
        with open(local_path, "rb") as f:
            data = f.read()
        local_md5 = base64.b64encode(hashlib.md5(data).digest()).decode("utf-8")
...
def localize_sample_files(samples_json_selected: list, localized_dir: str = "") -> list:
    """
    Take FISS sample list structure and localize all files from GCP,
    re-downloading only if the remote object has changed (MD5 mismatch).
    """
    ...
    for item in samples_json_selected:
        ...
        if out_count:
            ...
            download_if_changed(out_count, local_count, client)
            local_item["attributes"]["output_count_result"] = local_count

        # output_editing_efficiency_dict
        ...
        # output_supplementary_files_dict
        ...
        if supp_dict:
            for fid, uri in supp_dict.items():
                ...
                download_if_changed(uri, local_supp_path, client)

                root, ext = os.path.splitext(local_supp_path)
                if ext.lower() == ".pdf":
                    png_path = root + ".png"
                    download_pdf = not os.path.exists(png_path)
                    if download_pdf:
