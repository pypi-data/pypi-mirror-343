...
def conv2tflite(
    ...
    save_path,
    optimize_default,
    optimize_experimental_sparsity,
    verbose,
):
    if verbose:
        warnings.filterwarnings("always")
    ...
    op = []
    if optimize_default:
        op = [tflite.Optimize.DEFAULT]
    if optimize_experimental_sparsity:
        op.append(tflite.Optimize.EXPERIMENTAL_SPARSITY)
    dd.commands.convert_to_tflite_from_from_saved_model(
        project_path, model_path, save_path, op, verbose=verbose
    )
