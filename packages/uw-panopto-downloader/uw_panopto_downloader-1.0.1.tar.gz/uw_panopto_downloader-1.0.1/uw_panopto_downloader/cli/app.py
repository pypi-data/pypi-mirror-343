...
from .utils import print_header, print_info, print_warning, check_dependencies
...
def convert(
    ...
    output_path: Optional[str] = typer.Option(None, "--output", "-o", help="Output directory or file"),
    bitrate: Optional[str] = typer.Option(None, "--bitrate", "-b", help="Audio bitrate (e.g. 192k)"),
    threads: Optional[int] = typer.Option(None, "--threads", "-t", help="Number of FFmpeg threads (0=auto)")
) -> None:
...
def config_command(
    download_dir: Optional[str] = typer.Option(None, "--download-dir", help="Set default download directory"),
    max_workers: Optional[int] = typer.Option(None, "--max-workers", help="Set default number of concurrent downloads"),
    headless: Optional[bool] = typer.Option(None, "--headless", help="Set default headless browser mode"),
    audio_bitrate: Optional[str] = typer.Option(None, "--audio-bitrate", help="Set default audio bitrate"),
    ffmpeg_threads: Optional[int] = typer.Option(None, "--ffmpeg-threads", help="Set default FFmpeg threads"),
    ...
    if show or all(param is None for param in [download_dir, max_workers, headless, audio_bitrate, ffmpeg_threads]):
