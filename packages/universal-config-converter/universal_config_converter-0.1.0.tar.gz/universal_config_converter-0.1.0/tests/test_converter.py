...
def temp_files(tmp_path):
    ...
    paths = {
        ...
        "yaml_comments_in": tmp_path / "input_comments.yaml", # For comment test
...
def test_xml_to_toml(temp_files):
    ...
    output_path = temp_files["out"].with_suffix(".toml")
    # Note: XML structure needs to be convertible to TOML's structure
    # Using XML_SAMPLE_DATA_FROM_STR which has a root key
    input_xml_path = temp_files["xml_in"]
    convert(input_xml_path, 'xml', 'toml', output_path)
    assert output_path.exists()
    loaded_data = load_config(output_path, 'toml')
    # TOML doesn't support attributes like '@host', xmltodict structure needs care
    # We compare the structure derived by xmltodict parse
    # Need conversion for comparison, might need adjustment
    # For this test, let's compare against the dictionary derived from XML
    assert loaded_data == XML_SAMPLE_DATA_FROM_STR
