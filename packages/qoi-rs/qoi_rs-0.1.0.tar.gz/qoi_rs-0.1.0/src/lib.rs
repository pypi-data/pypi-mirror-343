        ...
        match data {
            Data::Channel4(items) => {
                if items.iter().all(|pixel| pixel.3 == 0) {
                    items.into_iter().flat_map(|(a, b, c, _)| [a, b, c]).collect()
                } else {
                    items.into_iter().flat_map(|(a, b, c, d)| [a, b, c, d]).collect()
                }
            },
...
mod _qoi {
    ...
    fn to_py_error(error: qoi::Error) -> PyErr {
        ...
        match &error {
            ...
            | InvalidImageLength { .. }
            | InvalidMagic { .. }
            | InvalidPadding => PyValueError::new_err(format!("{error:?}")),
            OutputBufferTooSmall { .. } | UnexpectedBufferEnd | IoError(_) => {
    ...
    impl Image {
        ...
        fn __repr__(&self) -> PyResult<String> {
            ...
            Ok(format!("<qoi_rs._qoi.Image color_space={color_space} mode={mode} size={width}x{height} at {id:?}>"))
        }
    }
