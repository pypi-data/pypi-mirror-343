...
class Ai:
    ...
    def GAI(cls, context, questions=None, summarize_text=False, summary_length=3):
        ...
        @classmethod
        def detect_question_type(cls, question):
            q = question.lower()
            if any(x in q for x in ['how old', 'age']):
                return 'age'
            if any(x in q for x in ['when', 'date', 'born']):
                return 'date'
            if any(x in q for x in ['where', 'place', 'located', 'born in']):
                return 'location'
            if any(x in q for x in ['who', 'person', 'individual']):
                return 'person'
            if any(x in q for x in ['what', 'define', 'meaning', 'explain']):
                return 'thing'
            if any(x in q for x in ['why', 'reason']):
                return 'explanation'
            if any(x in q for x in ['how']):
                return 'how'
            return 'unknown'
        cls.detect_question_type = detect_question_type
        ...
        def extract_answer(question, context):
            q_type = cls.detect_question_type(question)
            ...
            for s in sentences:
                ...
                if full_name and s_original.startswith(('He ', 'he ')):
                    s_resolved = s_original.replace('He ', f'{full_name} ').replace('he ', f'{full_name} ')
                else:
                    s_resolved = s_original
                ...
                if q_type in ['thing', 'definition']:
                    if any(x in s_resolved.lower() for x in ['is', 'are', 'means']):
                        return s_resolved

                if q_type == 'explanation':
                    if "because" in s_resolved or "due to" in s_resolved:
                        return s_resolved

                if q_type == 'how':
                    if any(w in s_resolved.lower() for w in ['by', 'through', 'using', 'with', 'due to']):
                        return s_resolved

            cm = get_close_matches(q_lower, sentences, n=1)
            if cm:
                return cm[0].strip().rstrip('.')
        ...
        def summarize(text, num_sentences=3):
            ...
            stopwords = {
                ...
                'he','she','they','at','by','an','this','from','be','or'
            }
...
def private(func):
    def wrapper(*args, **kwargs):
        ...
        if caller is not args[0]:
            raise AttributeError(f"'{func.__name__}' Cant reach from outside.")
