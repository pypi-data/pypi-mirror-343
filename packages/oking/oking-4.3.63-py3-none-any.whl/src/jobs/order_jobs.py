...
def set_isolation_level(conn, db_type: str):
    db_type = db_type.lower()
    try:
        if db_type == 'oracle':
            # Oracle não suporta REPEATABLE READ, usamos SERIALIZABLE
            conn.set_session(isolation_level='SERIALIZABLE')
        elif db_type == 'sqlserver':
            # SQL Server tem comportamento diferente
            conn.execute("SET TRANSACTION ISOLATION LEVEL REPEATABLE READ")
        elif db_type in ('mysql', 'postgresql', 'firebird'):
            conn.set_session(isolation_level='REPEATABLE READ')
        else:
            # Fallback padrão
            conn.set_session(isolation_level='REPEATABLE READ')
    except Exception as e:
        logger.warning(f"Não foi possível configurar isolamento para {db_type}: {str(e)}")
        # Pelo menos garanta o mínimo
...
def insert_temp_order_okvendas(job_config: dict, order: OrderOkvendas, db_config: DatabaseConfig, job_type: str) -> bool:
    ...
    db = database.Connection(db_config)
    conn = db.get_conect()
    set_isolation_level(conn, db_config.db_type)  # <--- Aqui!
