...
class AlignmentCheckOutputSchema(BaseModel):
    ...
    conclusion: str = Field(
        description="One of the following: compromised, not_compromised, outright_refusal"
    )
...
class AlignmentCheckScanner(CustomCheckScanner[AlignmentCheckOutputSchema]):
    ...
    def _pre_process_input_text(self, text: str) -> str:
        try:
            structured_input = json.loads(text)
            return USER_PROMPT_FORMAT.format(
                user_message=structured_input.get("user_message", ""),
                trace=structured_input.get("trace", ""),
            )
    ...
    async def scan(
        ...
        if past_trace is None:
            # If no trace is provided, process just the message content
            processed_text = self._pre_process_input_text(content)
        else:
            # If trace is provided, use it to construct the input
            user_input = self._pick_user_input(past_trace)
            if not user_input:
                return ScanResult(
                    decision=ScanDecision.HUMAN_IN_THE_LOOP_REQUIRED,
                    reason="No user input found in trace, cannot proceed",
                    score=1.0,
                )
            trace_str = self._pre_process_trace(message, past_trace)
            processed_text = USER_PROMPT_FORMAT.format(
                user_message=user_input,
                trace=trace_str,
            )

        llm_response = await self._evaluate_with_llm(processed_text)
    ...
    async def _convert_llm_response_to_score(
        ...
        if hasattr(response, "conclusion") and response.conclusion != "compromised":
            return 0.0
        return 1.0
