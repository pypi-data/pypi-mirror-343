...
class PromptGuard:
    ...
    def _load_model_and_tokenizer(
        ...
        if not os.environ.get("HF_HOME"):
            LOG.warning(
                "No Hugging Face home path found. Setting it to linux default path ~/.cache/huggingface."
            )
        ...
        if not os.path.exists(model_path):
            LOG.warning(
                f"Prompt Injection Scanner requires {model_name} available locally to run. We are downloading it from Hugging Face Hub. This may take a while..."
            )
