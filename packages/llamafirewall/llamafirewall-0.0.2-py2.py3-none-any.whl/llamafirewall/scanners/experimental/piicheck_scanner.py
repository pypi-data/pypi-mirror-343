...
class PIICheckScanner(CustomCheckScanner[PIICheckOutputSchema]):
    ...
    async def _convert_llm_response_to_score(
        ...
        if response.detected_pii_types and response.detected_pii_types != ["ERROR"]:
            return 1.0
...
- detected_pii_types: A list of PII types detected in the text. If no PII is detected, return an empty list.
