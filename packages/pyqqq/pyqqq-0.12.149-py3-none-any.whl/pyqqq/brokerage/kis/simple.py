...
class KISSimpleDomesticStock:
    ...
    @with_mock()
    def create_order(self, asset_code: str, side: OrderSide, quantity: int, order_type: OrderType = OrderType.MARKET, price: int = 0) -> str:
        """
        주문을 생성합니다.
    ...
    @with_mock()
    def update_order(self, org_order_no: str, order_type: OrderType, price: int, quantity: int = 0) -> str:
        """
        주문을 수정합니다.
        ...
        r = self.stock_api.order_rvsecncl(self.account_no, self.account_product_code, org_order_no, __get_order_type_code(), "01", quantity, price, qty_all_ord_yn)  # 정정
