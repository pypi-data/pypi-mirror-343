...
import traceback
import yaml
from ...infrastructure_pro.logging import get_system_logger, get_user_logger
from ...infrastructure_pro.exceptions import StorageError
from ...infrastructure_pro.config.decorators import config
...
class StorageService:
    ...
    def __init__(self, storage_config=None):
        ...
        if storage_config is None:
            from datawhale.infrastructure_pro.config.decorators import get_config
            config_manager = get_config()
            storage_config = config_manager.get("storage", {})
            
        # 规范化路径分隔符，确保跨平台兼容性
        self.data_dir = os.path.normpath(os.path.abspath(storage_config.get('data_dir')))
        
        # 规范化路径分隔符，确保跨平台兼容性
        runtime_dir = os.path.normpath(os.path.abspath(storage_config.get('runtime_dir')))
        self.metainfo_dir = os.path.normpath(os.path.join(runtime_dir, storage_config.get('metainfo_dir')))
    ...
    def save(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(f"StorageService开始保存数据: {data_name}, rows={len(data)}, field_values={field_values}, mode={mode}")
    ...
    def query(
        ...
        try:
            ...
            logger.info(f"StorageService开始查询数据: {data_name}, field_values={field_values}, sort_by={sort_by}, parallel={parallel}")
            ...
            try:
                ...
                data = dataset.query(field_values, sort_by, parallel, max_workers)
    ...
    def delete(self, data_name: str, field_values: Dict[str, str] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"StorageService开始删除数据: {data_name}, field_values={field_values}")
    ...
    def create_dataset(
        ...
        try:
            ...
            logger.info(f"StorageService开始创建数据集: name={name}, level={dataset_level}, format={format}, update_mode={update_mode}")
            ...
            dataset = Dataset.create_dataset(self.data_dir, self.metainfo_dir, meta_info)
