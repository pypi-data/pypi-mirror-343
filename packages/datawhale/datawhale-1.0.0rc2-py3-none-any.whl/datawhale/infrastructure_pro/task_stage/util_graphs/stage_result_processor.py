...
from datawhale.infrastructure_pro.task_stage.stage import Stage
...
class StageResultFlattener:
    ...
    def __init__(self, stage: Stage, node_name: str = "save_df_to_cache"):
        ...
        logger.debug(f"创建Stage结果展平器, stage_id={stage.stage_id}, node_name={node_name}")
    ...
    def process(self) -> List[Dict[str, Any]]:
        ...
        if not self.stage.registries:
            logger.warning(f"Stage {self.stage.stage_id} 没有注册表，无法处理结果")
            return []
        ...
        for i, registry in enumerate(self.stage.registries):
            ...
            if result_list:
                # 记录找到的结果项数量
                logger.debug(f"从registry #{i} 提取到 {len(result_list)} 个结果项")
                
                # 将当前registry的所有结果项添加到总列表中
                all_results.extend(result_list)
                
                # 记录每个项的格式，用于调试
                if result_list and logger.isEnabledFor(10):  # DEBUG level
                    sample = result_list[0]
                    logger.debug(f"结果项示例: {sample.keys() if isinstance(sample, dict) else type(sample)}")
            else:
                logger.warning(f"Registry #{i} 中没有找到节点 {self.node_name} 的结果")
        
        # 如果没有找到任何结果
        if not all_results:
            logger.warning(f"没有找到任何结果")
            return []
        ...
        logger.info(f"从 {len(self.stage.registries)} 个registry中总共收集到 {len(valid_results)} 个有效结果项")
...
class StageResultHashMerger:
    ...
    def __init__(self, stage: Stage, node_name: str = "extract_df_params"):
        ...
        self.node_name = node_name
        self._cached_results = None  # 缓存处理结果
        logger.debug(f"创建Stage结果Hash合并器, stage_id={stage.stage_id}, node_name={node_name}")
    ...
    def process(self) -> List[Dict[str, Any]]:
        ...
        for i, registry in enumerate(self.stage.registries):
            ...
            if not isinstance(result, dict) or 'params' not in result or 'data' not in result or 'hash' not in result:
                logger.warning(f"Registry #{i} 中的结果无效: 缺少必要的键")
                continue
            ...
            if item_hash in hash_dict:
                ...
                logger.debug(f"合并了hash值为 {item_hash} 的结果项，data列表长度现为 {len(existing_item['data'])}")
            else:
                ...
                logger.debug(f"添加了新的hash值 {item_hash}，data为单元素列表")
        
        # 如果没有处理任何结果
        if total_processed == 0:
            logger.warning(f"没有找到任何有效结果项")
            return []
        ...
        logger.info(f"处理了 {total_processed} 个原始结果项，合并后得到 {len(merged_results)} 个结果项")
