...
from datawhale.infrastructure_pro.storage import create_dataset, save, infer_dtypes
...
class Edge:
    ...
    def __init__(self, source: "Node", target: "Node", param_name: Optional[str] = None):
        """初始化边对象"""
        self.source = source
        self.target = target
...
class Node:
    ...
    def __init__(
        ...
        self.remove_from_namespace_after_execution = remove_from_namespace_after_execution
    ...
    def _add_downstream(self, downstream_node: "Node", param_name: Optional[str] = None) -> "Node":
        """添加下游节点并创建边"""
        # 检查边是否已存在
    ...
    def _update_param_mapping(self, target_node: "Node", param_name: str):
        """更新参数映射"""
        # 判断是否为列表参数（通过命名约定）
        is_list_param = param_name == "args" or param_name.endswith("[]") or param_name.endswith("_list")
        ...
        if is_list_param and base_param in target_node.inputs:
            ...
            if isinstance(target_node.inputs[base_param], list):
                ...
                # 如果是字符串，转换为列表
                target_node.inputs[base_param] = [target_node.inputs[base_param], self.name]
        else:
            # 普通参数，直接设置
    ...
    def _cache_result_to_file(self, result_value: Any, cache_dir: str) -> Optional[str]:
        ...
        try:
            ...
            system_logger.debug(f"[Node][{self.node_id}]：结果已缓存，path={cache_path}")
            return cache_path
        except Exception as e:
            system_logger.error(f"[Node][{self.node_id}]：缓存结果失败，错误={str(e)}")
            return None
    
    def execute(self, namespace_registry: Optional[NamespaceRegistry] = None, cache_dir: Optional[str] = None, *args, **kwargs) -> Any:
        """执行节点"""
        ...
        try:
            ...
            if namespace_registry and self.save_to_namespace:
                ...
                if self.cache_result and result is not None and cache_dir:
                    ...
                    if cache_path:
                        ...
                        system_logger.debug(f"[Node][{self.node_id}]：设置命名空间缓存路径，cache_path={cache_path}")
                    else:
                        # 缓存失败，只保存原始结果
                        namespace_registry.set_value(self.name, result)
                else:
                    # 不需要缓存，直接更新命名空间
                    namespace_registry.set_value(self.name, result)
                    system_logger.debug(f"[Node][{self.node_id}]：结果已保存到命名空间，name={self.name}, value={result}")
            elif not self.save_to_namespace:
                system_logger.debug(f"[Node][{self.node_id}]：结果不保存到命名空间，save_to_namespace={self.save_to_namespace}")
            ...
            system_logger.info(f"[Node][{self.node_id}]：执行完成，status={self.status}，耗时={execution_time:.2f}秒")
    
    def _can_execute(self) -> bool:
        """检查节点是否可以执行"""
        # 如果没有上游节点或所有上游节点都已成功完成，则可以执行
