...
class BatchTaskUnfinishedManager:
    ...
    def _init_storage_params(self, storage_dir: str = None) -> None:
        ...
        """
        self.storage_dir = storage_dir or config(
            "infrastructure.task.recovery_tasks_dir"
        )

        if self.storage_dir is None:
            error_msg = "配置项 infrastructure.task.recovery_tasks_dir 未设置"
            system_logger.error(error_msg)
            user_logger.error("任务恢复存储目录未配置，请检查配置文件")
