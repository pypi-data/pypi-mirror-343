...
class BatchTaskFailureManager:
    ...
    def __init__(self, task_type: str = "default", storage_dir: str = None):
        ...
        Args:
            ...
            storage_dir: 存储目录，如果为None则从配置文件读取
        """
    ...
    def _init_storage_params(self, storage_dir: str = None) -> None:
        ...
        Raises:
            ValueError: 当必要的配置项未设置时抛出
        """
        self.storage_dir = storage_dir or config("infrastructure.task.failed_tasks_dir")

        if self.storage_dir is None:
            error_msg = "配置项 infrastructure.task.failed_tasks_dir 未设置"
            system_logger.error(error_msg)
            user_logger.error("失败任务存储目录未配置，请检查配置文件")
    ...
    def record_task_failure(self, task: Task) -> None:
        """记录任务失败
