...
from datawhale.infrastructure_pro.task_gragh.node.node import Edge
from datawhale.infrastructure_pro.task_gragh.node.status import NodeStatus
from datawhale.infrastructure_pro.task_gragh.namespace import Namespace, NamespaceRegistry
from datawhale.infrastructure_pro.task_gragh.node.node import NodeProxy
...
class Graph:
    ...
    def __init__(self, name: str, nodes: List[Node] = None, cache_dir: Optional[str] = None):
        """初始化图对象
        ...
        if self.cache_dir is not None:
            ...
            system_logger.debug(f"[Graph][{self.graph_id}]：创建缓存目录，cache_dir={self.cache_dir}")
    ...
    def add_node_after(self, target_node_name: str, new_node: Node, 
                     connect_downstream: bool = True) -> Node:
        """在指定节点后添加一个新节点
        ...
        system_logger.debug(f"[Graph][{self.graph_id}]：在节点 '{target_node_name}' 后添加节点 '{new_node.name}'")
        ...
        if connect_downstream and target_node.downstream_edges:
            ...
            for edge in original_downstream_edges:
                ...
                system_logger.debug(f"[Graph][{self.graph_id}]：将节点 '{new_node.name}' 连接到下游节点 '{downstream_node.name}'")
        
        return new_node
    
    def add_node_before(self, target_node_name: str, new_node: Node, 
                      connect_upstream: bool = True) -> Node:
        """在指定节点前添加一个新节点
        ...
        system_logger.debug(f"[Graph][{self.graph_id}]：在节点 '{target_node_name}' 前添加节点 '{new_node.name}'")
        ...
        if connect_upstream and target_node.upstream_edges:
            ...
            for edge in original_upstream_edges:
                ...
                system_logger.debug(f"[Graph][{self.graph_id}]：将上游节点 '{upstream_node.name}' 连接到节点 '{new_node.name}'")
    ...
    def execute(self, start_params: Dict[str, Any] = None, check_dag: bool = False) -> Dict[str, str]:
        """执行图中的所有节点
        ...
        for node in execution_order:
            ...
            if node._can_execute():
                try:
                    ...
                    node.error = str(e)
                    self.results[node.name] = node.status
                    system_logger.error(f"[Graph][{self.graph_id}]：节点执行失败，node={node.name}，error={str(e)}")
            else:
                # 标记节点为失败状态
                node.status = NodeStatus.FAILED
                self.results[node.name] = node.status
                system_logger.warning(f"[Graph][{self.graph_id}]：节点依赖未满足，无法执行，node={node.name}")
                
        # 检查执行状态
        failed_nodes = [node.name for node in self.nodes if node.status == NodeStatus.FAILED]
        ...
        if failed_nodes:
            if len(failed_nodes) == len(self.nodes):
                ...
                system_logger.warning(f"[Graph][{self.graph_id}]：图执行部分失败，failed_nodes={failed_nodes}")
        else:
            self.status = GraphStatus.COMPLETED
        ...
        system_logger.info(f"[Graph][{self.graph_id}]：图执行完成，status={self.status}，耗时={elapsed_time:.3f}秒")
    ...
    def visualize(self, output_path=None) -> graphviz.Digraph:
        ...
        if not GRAPHVIZ_AVAILABLE:
            system_logger.warning(f"[Graph][{self.graph_id}]：graphviz未安装，无法执行可视化")
            return None
        ...
        if output_path:
            try:
                ...
                system_logger.debug(f"[Graph][{self.graph_id}]：图结构已保存到 {output_path}.png")
            except Exception as e:
