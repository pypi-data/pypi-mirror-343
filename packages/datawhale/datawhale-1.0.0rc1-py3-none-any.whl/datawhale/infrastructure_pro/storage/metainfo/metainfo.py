...
class MetaInfo:
    ...
    def is_valid_dict(meta_dict: Dict[str, Any]) -> bool:
        ...
        """
        # 检查必需的顶级键
        if MetaInfo.KEY_NAME not in meta_dict:
            return False
            
        # 检查dtypes是否存在、是字典且长度大于1
        ...
        dataset_info = meta_dict.get(MetaInfo.KEY_DATASET)
        ...
        if dataset_info is not None:
            ...
            if dataset_info and MetaInfo.KEY_STRUCTURE_FIELDS not in dataset_info:
                return False
    ...
    @classmethod
    def from_yaml(cls, yaml_path: Union[str, Path]) -> 'MetaInfo':
        """从YAML文件加载元数据信息
    ...
    def infer_dtypes_from_data(cls, data: pd.DataFrame) -> Dict[str, str]:
        ...
        for column in data.columns:
            ...
            # 先检查是否为字符串类型，防止字符串被误判为数值类型
            elif pd.api.types.is_string_dtype(col_type):
                dtypes[column] = 'string'
            else:
                # 对于对象类型，检查是否可以转换为更具体的类型
                if pd.api.types.is_object_dtype(col_type):
                    # 尝试推断更具体的类型
                    # 检查是否所有非空值都是数字
                    non_null = data[column].dropna()
                    if non_null.empty:
                        dtypes[column] = 'string'  # 空列默认为字符串
                    # 判断是否看起来像数字，但实际是作为字符串使用（如ID、代码等）
                    elif isinstance(non_null.iloc[0], str) and column.lower() in ['id', 'code', 'date', 'time', 'year', 'month', 'day', 'hour', 'minute', 'second']:
                        dtypes[column] = 'string'
                    elif non_null.str.isnumeric().all():
                        # 尝试转换为整数
