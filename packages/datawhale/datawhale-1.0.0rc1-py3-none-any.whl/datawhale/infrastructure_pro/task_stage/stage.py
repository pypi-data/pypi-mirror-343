...
from datawhale.infrastructure_pro.task_system.interface import execute_new_batch_tasks
from datawhale.infrastructure_pro.task_system.batch_task import BatchTask
from datawhale.infrastructure_pro.logging import get_user_logger, get_system_logger
from datawhale.infrastructure_pro.task_gragh.namespace import NamespaceRegistry
...
class Stage:
    ...
    def __init__(self, name: str, graph: Graph, params_list: List[Dict[str, Any]], runtime_dir: str = None, cache_registry: bool = False):
        """初始化阶段对象
        ...
        if runtime_dir is None:
            self.runtime_dir = os.path.join(os.getcwd(), f"stage_runtime_{self.stage_id}")
        else:
        ...
        system_logger.debug(f"[Stage][{self.stage_id}]：创建任务阶段，name={self.name}，params数量={len(self.params_list)}，runtime_dir={self.runtime_dir}，cache_registry={self.cache_registry}")
    ...
    def execute(self, 
                ...
                record_failed: bool = True) -> BatchTask:
        """执行阶段中的所有任务
        ...
        system_logger.info(f"[Stage][{self.stage_id}]：开始执行阶段，name={self.name}，任务数量={len(self.params_list)}")
        ...
        try:
            ...
            def execute_graph_task(params: Dict[str, Any]) -> Dict[str, Any]:
                ...
                if self.cache_registry:
                    try:
                        ...
                        system_logger.debug(f"[Stage][{self.stage_id}]：缓存了图 {self.graph.graph_id} 的注册表")
                    except Exception as e:
                        system_logger.error(f"[Stage][{self.stage_id}]：无法缓存图 {self.graph.graph_id} 的注册表: {str(e)}")
            ...
            batch_task = execute_new_batch_tasks(
            ...
            self.status = StageStatus.FAILED
            system_logger.error(f"[Stage][{self.stage_id}]：执行阶段失败，name={self.name}，error={str(e)}")
            raise
