...
def getlinkadjmat(fig, paths):
    ...
    for frozen_path in fig.board_config.board_paths:
        ...
        edgeidx2edge[edge_num - 1] = (frozen_path.start_point_num, frozen_path.end_point_num)

    num_nodes = len(fig.board_config.points)
    adj_mat = np.eye(num_nodes, dtype=bool)
...
def getcompletedroutes(fig, player_hand, log=False):
    ...
    visited_nodes = np.column_stack([
        np.eye(1, num_points, routes[route_num - 1].start_num, dtype=bool).flatten()
        for route_num in route_nums
    ])

    target_nodes = np.column_stack([
        np.eye(1, num_points, routes[route_num - 1].end_num, dtype=bool).flatten()
        for route_num in route_nums
    ])
