...
class BaseRepository:
    ...
    def _build_filter_conditions(self, filter_dict: Dict, operator: FilterOperator = FilterOperator.AND) -> List[expression.BinaryExpression]:
        conditions = []
        ...
        if "operator" in filter_dict and "conditions" in filter_dict:
            nested_conditions = []
            try:
                # Convert operator to lowercase before creating enum
    ...
    def _build_sort_orders(self, sort_orders: List[SortOrder]) -> List[expression.UnaryExpression]:
        ...
        for sort in sort_orders:
            ...
            if hasattr(self.model, field):
                ...
                # Optional: Handle cases where the field does not exist on the model.
                raise ValidationError(detail=f"Field '{field}' does not exist on model {self.model.__name__}")

        return orders

    def _build_dynamic_select(self,show_columns:List[str]) -> List[expression.ColumnClause]:
        """
        Builds the SELECT statement dynamically excluding hidden columns.
        """
        columns_to_select = []

        # Get all columns from the model
        for column in class_mapper(self.model).columns:
            if column.name in show_columns:
                columns_to_select.append(getattr(self.model, column.name))
    ...
    def read_by_options(
        ...
        with self.session_factory() as session:
            try:
                ...
                # Handle multiple sort orders
                sort_orders = schema_as_dict.get("sort_orders", [])
                if not sort_orders and schema_as_dict.get("ordering"):
                    # Backward compatibility for single ordering
                    ordering = schema_as_dict.get("ordering", DEFAULT_ORDERING)
                    sort_orders = [{
                        "field": ordering[1:] if ordering.startswith("-") else ordering,
                        "direction": SortDirection.DESC if ordering.startswith("-") else SortDirection.ASC
                    }]
                ...
                if "sort_orders" in filter_dict:
                    del filter_dict["sort_orders"]
                ...
                # Handle show columns
                show_columns = schema_as_dict.get("show_columns") or []
                query = query.with_entities(*self._build_dynamic_select(show_columns))
    ...
    def get_unique_values(self, schema: FindUniqueValues) -> dict:
        with self.session_factory() as session:
            try:
                ...
                if search_term:
                    count_query = count_query.filter(column.ilike(f"%{search_term}%"))
