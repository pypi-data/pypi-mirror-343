...
class CircuitState(Enum):
    ...
    HALF_OPEN = "half-open"  # Testing if Redis is available again


class CircuitBreakerFallbackMiddleware(StorageBackendMiddleware):
    """
    Circut Breaker + fallback middleware.

    - CLOSED: all ops go to default storage (usually Redis); Failures count up
    - OPEN: all ops go straight to fallback_storage cache until timeout
    - HALF OPEN: allow a few trial calls to default storage; If they succeed - CLOSE, on any fail - reopen
    """

    def __init__(
        self,
        fallback_storage: "StorageBackend",
    ...
    async def record_failure(self) -> None:
        async with self.lock:
            ...
            if self.state == CircuitState.CLOSED:
                ...
                self.last_failure_time = time.time()  # mb monotonic
                if self.failure_count >= self.failure_threshold:
                    print("Switching circuit breaker to OPEN")
        ...
        async with self.lock:
            if self.state == CircuitState.CLOSED:
                ...
                if self.success_count >= self.success_threshold:
                    print("Switching circuit breaker to CLOSE")
                    self.state = CircuitState.CLOSED
                    self.failure_count = 0
        ...
        async with self.lock:
            if self.state == CircuitState.CLOSED:
                ...
                if cooldown_elapsed:
                    ...
                    print("Switching circuit breaker to HALF_OPEN")
                ...
                return True
        return True  # Default to using backend
    ...
    async def _call_with_circuit(
        ...
        **kwargs: P.kwargs,
        ...
        if should_use_main:
            # CircutBreaker is closed - we can use main Redis
            try:
                result = await func_original(*args, **kwargs)
                await self.record_success()
                return result
            except CacheBackendInteractionError:
                # при запросе ошибка, записываем её и откатываемся на фоллбек
                await self.record_failure()
                return await func_fallback(*args, **kwargs)
        else:
            # circutbreaker открыт, цепь разорвана, нужно юзать фоллбек
            return await func_fallback(*args, **kwargs)
