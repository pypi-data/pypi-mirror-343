...
def rename(bot_id: str, new_id: str, version: str | None = None):
    if not re.match(r"^\d[A-Za-z]\d{3}$", bot_id):
        print(f"Skipping invalid target ID format: {bot_id}")

    bot_to_rename = get_device_by_id(bot_id)
    old_name = bot_id
    if bot_to_rename:
        old_name = bot_to_rename.get("device_name").removeprefix("kiwibot")
    else:
        exit(1)

    if click.confirm(
        f'Do you want to rename the old device {old_name} to {new_id}? ',
        default=True,
    ):
        bot_uuid = bot_to_rename.get("uuid")
        tags = balena_sdk.models.device.tags.get_all_by_device(bot_uuid)
        ...
        print(f"Device {bot_to_rename.get("device_name")} successfully renamed")