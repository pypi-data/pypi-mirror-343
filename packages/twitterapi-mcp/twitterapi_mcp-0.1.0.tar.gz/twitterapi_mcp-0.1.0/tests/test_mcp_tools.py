...
# Test monitor_ai_influencers tool
async def test_monitor_ai_influencers_tool():
    from twitterapi.tools.ai_tools import monitor_ai_influencers
    ...
    assert "@testuser" in result
    assert "High engagement test tweet" in result
    twitter_ctx.get_influencer_tweets.assert_called_once_with("testuser", 7)

# Test search_ai_development_topics tool
async def test_search_ai_development_topics_tool():
    from twitterapi.tools.ai_tools import search_ai_development_topics
    ...
    # Call tool function with a query that doesn't have AI or dev terms
    result = await search_ai_development_topics("transformers", "Top", 5, ctx)
    
    # Verify that the query was enhanced
    twitter_ctx.search_tweets.assert_called_once()
    # Check that AI and dev terms were added
