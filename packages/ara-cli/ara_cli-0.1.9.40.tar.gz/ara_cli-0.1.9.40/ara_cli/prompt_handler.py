...
class ChatOpenAISingleton:
    ...
    def __init__(self, model):
        ...
        selected_config = next((c for c in llm_config if c["model"] == model), None)

        if not selected_config:
            raise ValueError(f"No configuration found for the model: {model}")

        ChatOpenAISingleton._model = model

        model_name = ChatOpenAISingleton._strip_model_name(model)
    ...
    @staticmethod
    def get_instance():
        if ChatOpenAISingleton._instance is None and ChatOpenAISingleton._instance is None:
            config = ConfigManager().get_config()
            llm_config = config.llm_config
            first_model = llm_config[0]["model"]
            ChatOpenAISingleton(first_model)
        if ChatOpenAISingleton._instance is None:
            ChatOpenAISingleton(ChatOpenAISingleton._model)
        return ChatOpenAISingleton._instance

    @staticmethod
    def set_model(model_name):
        if model_name == ChatOpenAISingleton._model:
            return ChatOpenAISingleton._instance
        ChatOpenAISingleton._model = model_name
        ChatOpenAISingleton(ChatOpenAISingleton._model)
    ...
    def get_model():
        if ChatOpenAISingleton._instance is None:
            ChatOpenAISingleton.get_instance()
        return ChatOpenAISingleton._model
