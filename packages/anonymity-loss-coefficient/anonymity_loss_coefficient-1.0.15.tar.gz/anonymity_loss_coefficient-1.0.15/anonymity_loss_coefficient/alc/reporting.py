...
class Reporter():
    def __init__(self,
        ...
        if flush:
            # delete the summary_raw_path if it exists
            if os.path.exists(summary_raw_path):
                self.logger.info(f"Flushing {summary_raw_path}")
                try:
                    os.remove(summary_raw_path)
                except PermissionError:
                    self.logger.warning(f"Warning: The file at {summary_raw_path} is currently open in another application. You might want to make a copy of the file in order to view it while the attack is still executing.")
                except Exception as e:
                    self.logger.error(f"Error: Failed to delete {summary_raw_path}: {e}")
        else:
            ...
            if os.path.exists(summary_raw_path):
                ...
                self.list_results = df.to_dict(orient='records')
    ...
    def _get_results_df(self) -> None:
        self.df_results = pd.DataFrame(self.list_results)
    ...
    def summarize_results(self,
        ...
        if self.df_results is None:
            self._get_results_df()
        if complete:
            self.save_to_parquet(self.results_path, self.df_results, 'summary_raw.parquet')
        df_secret_known = self.alc_per_secret_and_known_df()
        self.save_to_csv(self.results_path, df_secret_known, 'summary_secret_known.csv')
        df_secret = self.alc_per_secret_df()
        self.save_to_csv(self.results_path, df_secret, 'summary_secret.csv')
        if with_text:
