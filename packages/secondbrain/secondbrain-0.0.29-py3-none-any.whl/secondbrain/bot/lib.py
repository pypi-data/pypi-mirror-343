...
def get_abilities(bot_setting):
    execution_config = bot_setting["executionConfig"]
    abilities = []
    
    for bot_id in bot_setting["botList"]:
        bot = ref_bot(bot_id)
        if bot:
            abilities.append(bot)

    tool_config = execution_config["toolConfig"]
    for tool_id in bot_setting["toolList"]:
        config = tool_config.get(tool_id, None)
        abilities.extend(ref_tools(tool_id), config)
    for workflow_id in bot_setting["workflowList"]:
