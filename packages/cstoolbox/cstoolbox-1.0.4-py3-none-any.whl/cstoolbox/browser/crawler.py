...
from .config import CrawlerConfig, CrewlerResult, FieldType, PageConfig
...
class Crawler:
    ...
    async def crawl(self, url: str, config: CrawlerConfig) -> CrewlerResult:
        ...
        Returns:
            Extracted data as dictionary with optional screenshot path if enabled
        """
        ...
        try:
            ...
            if config.screenshot:
                if config.screenshot_path:
                    sdir = Path(config.screenshot_path).parent
                    if not sdir.exists():
                        os.makedirs(str(sdir))
                else:
                    config.screenshot_path = 'screenshots'
                    if not os.path.exists(config.screenshot_path):
                        os.makedirs(config.screenshot_path)
                    config.screenshot_path = os.path.join(config.screenshot_path, "screenshot.png")
