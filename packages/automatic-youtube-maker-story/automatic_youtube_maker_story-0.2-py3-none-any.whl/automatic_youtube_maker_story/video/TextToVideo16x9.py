...
def setWorkflowFinish(len, wid, hei, fps, crf, cfg, prompt, id, num, steps, test_prompt):
    ...
    prompt = "The scene is animated in a children's cartoon style, with bright, cheerful colors and proportion ratio 16x9; " + prompt + "; The scene is animated in a children's cartoon style, with bright, cheerful colors and proportion ratio 16x9."
...
def testeMochi(prompt, cname):
    ...
    heights = [  384, 480, 480, 768, 720]
    widths = [  688, 848, 852, 1280, 1280]
    count = -1
    for width in widths:
      count = count + 1
      height = heights[count]
      for liberdade in [5,6]:
