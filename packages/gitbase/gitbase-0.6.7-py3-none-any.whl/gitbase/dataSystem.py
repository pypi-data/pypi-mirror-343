...
from .__config__ import show_logs, use_offline
        ...
        try:
            ...
            if is_online():
                ...
                if response_code in (200, 201):
                    if show_logs: cPrint("GREEN", f"Successfully saved online data for {key}.")
                else:
                    if show_logs: cPrint("RED", f"Error saving online data for {key}. HTTP Status: {response_code}")
            else:
        ...
        try:
            if is_online():
                ...
                if online_data:
                    ...
                    if show_logs: cPrint("RED", f"No online data found for {key}.")
            else:
    ...
    def use_offline_data(self, key: str, encryption: bool = False) -> Optional[KeyValue]:
        ...
        if use_offline:
            ...
            if not os.path.exists(path):
                if show_logs: cPrint("RED", f"No offline data found for key: {key}")
                return None
    ...
    def delete_data(self, key: str, path: str = "data", delete_offline: bool = False) -> None:
        ...
        try:
            ...
            if response_code in (200, 204):
                if show_logs: cPrint("GREEN", f"Successfully deleted online data for {key}.")
            elif response_code == 404:
                if show_logs: cPrint("RED", f"No online data found for {key}.")
            else:
                if show_logs: cPrint("RED", f"Error deleting online data for {key}. HTTP Status: {response_code}")
        except Exception as e:
            if show_logs: cPrint("RED", f"Error deleting online data: {e}")

        if delete_offline:
            offline_path: str = os.path.join("gitbase/data", f"{key}.gitbase")
            if os.path.exists(offline_path):
                os.remove(offline_path)
                if show_logs: cPrint("GREEN", f"Successfully deleted offline backup for {key}.")
            else:
                if show_logs: cPrint("RED", f"No offline backup found for {key}.")
    ...
    def get_all(self, encryption: bool, path: Optional[str] = "data") -> Dict[str, Any]:
        ...
        if is_online():
            try:
                ...
                if show_logs: cPrint("RED", f"Error loading online data: {e}")
        else:
            if use_offline:
                if show_logs: cPrint("YELLOW", "Network is offline, loading from offline backup.")
                ...
                for file in files:
                    if file.endswith(".gitbase"):
                        ...
                        try:
                            ...
                            if show_logs: cPrint("RED", f"Error loading offline data for {key}: {e}")
