...
from .__config__ import use_offline, show_logs
        ...
        try:
            ...
            if is_online():
                ...
                if response_code in (200, 201):
                    if show_logs: cPrint("GREEN", f"Successfully saved online data for {username}.")
                    self.use_offline_account(username, player_instance, attributes)
                else:
                    if show_logs: cPrint("RED", f"Error saving online data for {username}. HTTP Status: {response_code}")
            else:
                if show_logs: cPrint("YELLOW", "Network is offline, saving to offline backup version.")
                if use_offline:
                    self.use_offline_account(username, player_instance, attributes)
        except Exception as e:
            if show_logs: cPrint("RED", f"Error: {e}")
            if use_offline:
                if show_logs: cPrint("GREEN", "Attempting to save to offline backup version anyway.")
                try:
                    self.use_offline_account(username, player_instance, attributes)
                except Exception as e:
                    raise Exception(f"Error: {e}")

    def use_offline_account(self, username: str, player_instance: Any, attributes: Optional[List[str]] = None) -> None:
        """
        ...
        if use_offline:
            ...
            try:
                ...
                if show_logs: cPrint("GREEN", f"Successfully saved full offline backup for {username}.")
            except Exception as e:
        ...
        try:
            ...
            if is_online():
                ...
                if online_data:
                    ...
                    if offline_data_exists and offline_timestamp > online_timestamp and use_offline:
                        if show_logs: cPrint("GREEN", f"Loading offline backup for {username} (newer version found).")
                        ...
                        if show_logs: cPrint("GREEN", f"Loading online data for {username} (newer version).")
                        if encryption:
                ...
                elif offline_data_exists and use_offline:
                    if show_logs: cPrint("GREEN", f"Loading offline backup for {username} (no online data available).")
                    self.use_offline_account(username, player_instance)
                else:
                    if show_logs: cPrint("RED", f"No data found for {username}.")
            else:
                if use_offline:
                    if show_logs: cPrint("YELLOW", "Network is offline, loading from offline backup.")
                    self.use_offline_account(username, player_instance)
        except Exception as e:
        ...
        if use_offline:
            ...
            try:
                if os.path.exists(offline_path):
                    ...
                    if show_logs: cPrint("GREEN", f"Successfully loaded offline backup for {username}.")
                else:
                    if show_logs: cPrint("RED", f"No offline backup found for {username}.")
            except Exception as e:
    ...
    def delete_account(self, username: str, delete_offline: bool = False) -> None:
        ...
        try:
            ...
            if response_code == 204 or response_code == 200:
                if show_logs: cPrint("GREEN", f"Successfully deleted online account for {username}.")
            elif response_code == 404:
                if show_logs: cPrint("RED", f"No online account found for {username}.")
            else:
                if show_logs: cPrint("RED", f"Error deleting online account. HTTP Status: {response_code}")
        except Exception as e:
            raise Exception(f"Error deleting online account: {e}")

        if delete_offline and os.path.exists(offline_path) and use_offline:
            try:
                os.remove(offline_path)
                if show_logs: cPrint("GREEN", f"Successfully deleted offline backup for {username}.")
            except Exception as e:
        ...
        if is_online():
            try:
                ...
                if response.status_code == 200:
                    ...
                    if not files:
                        if show_logs: cPrint("YELLOW", "No player files found in the online repository.")
                        ...
                        if file.get('name', '').endswith('.json'):
                            ...
                            if online_data:
                                ...
                                try:
                                    ...
                                    if show_logs: cPrint("YELLOW", f"Decryption failed for {username}, falling back to plain text: {e}")
                                ...
                                try:
                                    ...
                                    if show_logs: cPrint("RED", f"Failed to parse JSON for {username}: {e}")
                else:
                    if show_logs: cPrint("RED", f"Error retrieving player files from online database. HTTP Status: {response.status_code}")
            except Exception as e:
                if show_logs: cPrint("RED", f"Error retrieving online player data: {e}")
        else:
            if use_offline:
                if show_logs: cPrint("YELLOW", "Network is offline, loading player data from local storage.")
                        ...
                        if filename.endswith('.gitbase'):
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if show_logs: cPrint("YELLOW", f"Decryption failed for {username} offline data, falling back to plain text: {e}")
                                    # Fallback if decryption fails
                                    decrypted_content = encrypted_data.decode('utf-8')
                                player_data = jsonpickle.decode(decrypted_content)
                                all_players[username] = player_data
                            except Exception as e:
                                if show_logs: cPrint("RED", f"Error loading offline data for {username}: {e}")
                else:
                    if show_logs: cPrint("YELLOW", f"Offline directory {offline_dir} does not exist.")

        if not all_players:
            if show_logs: cPrint("YELLOW", "No players found in either online or offline storage.")
