...
class DataSystem:
    ...
    def save_data(self, key: str, value: Any, path: str = "data", encryption: bool = False) -> None:
        ...
        try:
            ...
            if is_online():
                ...
                self.save_offline_data(key, value, encryption=encryption)
    ...
    def load_data(self, key: str, encryption: bool, path: str = "data") -> Optional[KeyValue]:
        ...
        try:
            if is_online():
                ...
                return self.load_offline_data(key, encryption=encryption)
        except Exception as e:
            raise Exception(f"Error loading data for key '{key}': {e}")

    def save_offline_data(self, key: str, value: Any, encryption: bool = False) -> None:
        """
        ...
        serialized_data = jsonpickle.encode(value)
        data: bytes = self.encrypt_data(serialized_data) if encryption else serialized_data.encode("utf-8")
    ...
    def load_offline_data(self, key: str, encryption: bool = False) -> Optional[KeyValue]:
        """
        ...
        if not os.path.exists(path):
            cPrint("RED", f"No offline data found for key: {key}")
            return None
