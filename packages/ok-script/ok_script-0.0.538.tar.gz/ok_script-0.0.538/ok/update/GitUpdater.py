            ...
            for item in os.listdir('repo'):
                ...
                if os.path.isdir(item_path) and item != current_version:
                    ...
                    try:
                        ...
                        cmd = ["cmd", "/c", "rd", item_path, "/S", "/Q", ]
                        result = subprocess.run(cmd, capture_output=True, text=True, check=False, shell=False) # shell=False is safer
                        logger.info(f"Successfully deleted: {item_path} result: {result}")
                    except Exception as e:
...
def copy_folder_with_exclusions(source_folder, destination_folder, exclude_folders=[".venv"]):
    ...
    excluded_patterns = load_gitignore(source_folder)
