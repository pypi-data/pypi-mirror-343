    ...
    with click.progressbar(imex_map.items()) as bar:
        for path, textgrid_uri in bar:
            ...
            with open(filename, 'rb') as the_data:
                ...
                elif is_portalconfig(metadata.object_value):
                    content = rewrite_portalconfig_for_update_imex(the_data, imex_map, path_0)
                    the_data = ET.tostring(content, encoding='utf8', method='xml')
...
def rewrite_portalconfig_for_update_imex(data, imex_map, ignore_warnings=True):
    """Same as in tgimport.py, but using path as string, not purepath.

    TODO: implement an Imex class for handling absolute and relative paths
    """
    parser = ET.XMLParser(target=ET.TreeBuilder(insert_comments=True))
    the_dataXML = ET.parse(data, parser).getroot()

    # modify avatar element
    avatar_location = the_dataXML.find('pc:avatar', PC_NAMESPACES)
    check_path_and_rewrite(imex_map, avatar_location, ignore_warnings)
    ...
    if xslt_tag:
        ...
        for html_xslt_location in xslt_tag.findall('pc:html', PC_NAMESPACES):
            check_path_and_rewrite(imex_map, html_xslt_location, ignore_warnings)

    return the_dataXML


def check_path_and_rewrite(imex_map, element, ignore_warnings):
    """Check if the path referenced in 'element' is already contained in 'imax_map'.

    Exchange element content if so, fail if not

    almost the same as in tgimport.py, but using path as string, not purepath
    TODO: move to Imex class
    """
    path = element.text
